...
try {
  ...
  await jump("line:4~blue");
  await ac.page.evaluate(() => window.acSEND({type:"jump", content:{piece:"prompt",ahistorical:false,alias:false}}));
  await ac.wait(1200);
  await ac.page.keyboard.type("done");
  await ac.page.keyboard.press("Enter");
  await ac.wait(6000);
  assert.equal((await uploads.wips.find({code:"test"})).status, "done", "normal AC Done seals the WIP");
