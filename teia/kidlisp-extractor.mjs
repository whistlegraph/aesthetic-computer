...
function generateCacheCode(codesMap) {
  const entries = Array.from(codesMap.entries()).map(([id, data]) => {
    return `  globalCodeCache.set("${id}", ${JSON.stringify(data.source)});`;
  }).join('\n');
  
  return `
// 🗂️ Pre-populated KidLisp code cache for offline execution
// Generated by kidlisp-extractor.mjs
if (typeof globalCodeCache !== 'undefined') {
${entries}
  console.log('📚 Pre-loaded ${codesMap.size} KidLisp codes for offline use');
}`;
}
