#!/usr/bin/env fish
# Render the B&W notepat keymap FIGURE (shaded variant) — piano + QWERTY +
# connectors only, on white. No QR codes, no portrait, no brand block.
# See figure-bw.html.
#
# Output: slides/notepat-keymap/notepat-keymap-figure-bw.png
...
set -l out (test -n "$argv[1]"; and echo $argv[1]; or echo "$here/notepat-keymap-figure-bw.png")
set -l scale (test -n "$argv[2]"; and echo $argv[2]; or echo 2)
set -l tmp (mktemp -d)

set -l template "$here/figure-bw.html"
...
open('$tmp/keymap-figure-bw.html', 'w').write(html)
"
...
"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
    ...
    --screenshot="$out" \
    "file://$tmp/keymap-figure-bw.html" 2>/dev/null
