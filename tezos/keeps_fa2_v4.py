"""
Keeps FA2 v4 - Aesthetic Computer NFT Contract (PRODUCTION)

This contract extends v3 with essential production features:
...
def keeps_module():
    ...
    class KeepsFA2(
        ...
        """
        FA2 NFT contract for aesthetic.computer Keeps (v4 - PRODUCTION).

        Extends v3 with:
        - 10% royalty support for creator monetization
        - Emergency pause for security incidents
        - Admin transfer for customer service
        ...
        def __init__(self, admin_address, contract_metadata, ledger, token_metadata):
            ...
            # v4: Emergency pause flag (NEW)
            # When true, minting and metadata edits are disabled
            self.data.paused = False

            # v4: Default royalty configuration (NEW)
        ...
        def keep(self, params):
            ...
            2. User calling: mints to sender, requires fee payment
               - USER becomes firstMinter for proper attribution
            ...
            sp.cast(params, sp.record(
                ...
                attributes=sp.bytes,
                creators=sp.bytes,
                royalties=sp.bytes,  # v4: NEW - royalty metadata
                rights=sp.bytes,
            ...
            # v4: Now includes royalties field
            token_info = sp.cast({
                "name": params.name,
                ...
                "royalties": params.royalties,  # v4: NEW
        ...
        def edit_metadata(self, params):
            ...
            v4: Respects pause flag (cannot edit when paused)
            """
            sp.cast(params, sp.record(
                token_id=sp.nat,
        ...
        # =====================================================================
        # ON-CHAIN VIEWS (v4)
        # Temporarily commented out - may have Michelson compatibility issues
        # =====================================================================
        ...
        # @sp.onchain_view()
        # def get_default_royalty(self):
        #     """Get default royalty in basis points."""
        #     return self.data.default_royalty_bps

        # Note: Views can be added via TzKT API queries instead:
        # - paused: storage query
        # - default_royalty_bps: storage query
        # - token_creators: bigmap query
...
@sp.add_test()
def test():
    """Minimal test to compile v4 contract."""
    scenario = sp.test_scenario("KeepsFA2v4")
    scenario.h1("Keeps FA2 v4 - Production Contract")
    ...
    scenario.p("✅ Contract compiled successfully")
