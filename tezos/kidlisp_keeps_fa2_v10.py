"""
KidLisp Keeps FA2 v10 - Aesthetic Computer NFT Contract

v10 CHANGES from v9:
- admin_transfer removed — admin cannot move tokens on behalf of owners
- lock_metadata restricted to owner-only — admin can no longer freeze
  a token's metadata without owner consent
- withdraw_fees kept — all fees accumulate in-contract and are pulled
  to treasury via withdraw_fees (no in-flight send at mint)
...
def keeps_module():
    ...
    class KidLispKeepsFA2v10(
        main.Admin,
        main.Nft,
        main.OnchainviewBalanceOf,
    ):
        """
        FA2 NFT contract for KidLisp Keeps (v10).

        v10 changes from v9:
        - admin_transfer removed: admin has no power over token ownership
        - lock_metadata is owner-only: admin cannot freeze someone else's metadata
        - fees accumulate in-contract; use withdraw_fees to pull to treasury
        """
        ...
        def keep(self, params):
            """
            Mint a new Keep token.

            Two modes:
            1. Admin calling: mints to specified owner (server-side path)
            2. User calling: mints to sender, fee forwarded to treasury immediately

            Requires a valid backend-issued keep permit (signed by KEEP_PERMIT_SIGNER).
            Permit binds: contract + owner + content_hash + deadline.
            """
        ...
        def withdraw_fees(self, destination):
            ...
            Covers both user-path and admin-path mints.
            Admin only.
            """
...
@sp.add_test()
def test():
    scenario = sp.test_scenario("KeepsFA2v10")
    scenario.h1("KidLisp Keeps FA2 v10")
    ...
    contract = keeps_module.KidLispKeepsFA2v10(
        admin.address,
        treasury.address,
        sp.big_map(),
        ledger,
