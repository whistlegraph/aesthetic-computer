"""
KidLisp Keeps FA2 v8 - Aesthetic Computer NFT Contract (AUTHOR-PERMITTED KEEP)

This contract is the v8 security release with signed keep permits.

v8 CHANGES from v7:
- keep requires backend-issued signature (author permit)
- keep signature binds: contract + owner + content_hash + deadline
- keep still preserves v7 creator refresh metadata policy
- immutable fields remain preserved: content_hash + royalties

v7/v6 features (preserved):
...
def keeps_module():
    ...
    class KidLispKeepsFA2v8(
        main.Admin,
        main.Nft,
        main.OnchainviewBalanceOf,
    ):
        """
        FA2 NFT contract for KidLisp Keeps (v8 - SIGNED PERMIT PRODUCTION).

        v8 changes from v7:
        - keep requires a valid backend signature ("keep permit")
        - keep permit is bound to owner + content_hash + contract + deadline
        - owner/creator edit_metadata model remains from v7
        """
        ...
        def __init__(self, admin_address, contract_metadata, ledger, token_metadata):
            ...
            # v5/v6/v7/v8: Default fee set to 2.5 XTZ for revenue activation
        ...
        def keep(self, params):
            ...
            if not is_admin:
                ...
                # User must mint to themselves (prevents delegated/spam flows)
                assert params.owner == sp.sender, "MUST_MINT_TO_SELF"

            # Verify signed keep permit from backend authorizer.
...
@sp.add_test()
def test():
    """Minimal test to compile v8 contract."""
    scenario = sp.test_scenario("KeepsFA2v8")
    scenario.h1("KidLisp Keeps FA2 v8 - Signed Permit Production Contract")
    ...
    # Deploy contract
    contract = keeps_module.KidLispKeepsFA2v8(
        admin.address,
        sp.big_map(),
        ledger,
        token_metadata
    ...
    scenario.p("v8: signed keep permits + owner self-mint enforcement")
