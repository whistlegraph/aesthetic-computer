"""
Keeps FA2 v6 - KidLisp Production Contract

Production deployment on keeps.tez. Contract-level metadata:
  name: "KidLisp"
  description: "https://keeps.kidlisp.com"
  homepage: "https://kidlisp.com"
  version: "6.0.0"

Identical contract logic to v5. v6 is a fresh deploy with:
- Admin: keeps.tez (tz1Lc2DzTjDPyWFj1iuAVGGZWNjK67Wun2dC)
- Clean token IDs starting from 0
- Production contract metadata
- 2.5 XTZ mint fee (revenue enabled)

v5 features (preserved):
- Default fee: 2.5 XTZ (revenue enabled by default)
- Immutable content_hash: edit_metadata preserves content_hash
...
def keeps_module():
    ...
    class KeepsFA2(
        ...
        """
        FA2 NFT contract for KidLisp Keeps (v6 - Production on keeps.tez).
        ...
        v5 features: 2.5 XTZ fee, immutable content_hash, improved errors
...
def test():
    ...
    scenario.h1("Keeps FA2 v6 - KidLisp Production Contract")
    ...
    scenario.p("v6: Production deploy on keeps.tez")
    scenario.p("Contract name: KidLisp")
    scenario.p("Fee: 2.5 XTZ, 10% royalties")
    scenario.p("All v3-v5 features preserved")
