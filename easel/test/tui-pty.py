...
with tempfile.TemporaryDirectory(prefix="easel-pty-") as temporary:
    ...
    try:
        ...
        for index in range(24):
            send(f"line {index}\r")
        result = send("\x1b[5~")
        assert "lines above" in result and "EASEL" in result
        result = send("\x1b[F")
        ...
        print("PTY passed: about/profile clicks, engine handoff/recovery, stale callbacks, internal scroll, benchmark, existing piece preservation, mouse cleanup")
    finally:
