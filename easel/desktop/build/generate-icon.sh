...
mkdir "$icon_tmp/Easel.iconset"
rsvg-convert -w 1024 -h 1024 "$here/icon.svg" -o "$here/icon.png"
for size in 16 32 128 256 512; do
  sips -z "$size" "$size" "$here/icon.png" --out "$icon_tmp/Easel.iconset/icon_${size}x${size}.png" >/dev/null
  double=$((size * 2))
  sips -z "$double" "$double" "$here/icon.png" --out "$icon_tmp/Easel.iconset/icon_${size}x${size}@2x.png" >/dev/null
done
iconutil -c icns "$icon_tmp/Easel.iconset" -o "$here/icon.icns"
