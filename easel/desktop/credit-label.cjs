...
function startCreditLabel({app, window, root, nativeRoot = process.env.AESEL_DEV_ROOT ? join(process.env.AESEL_DEV_ROOT,'desktop') : app.isPackaged ? join(process.resourcesPath,'app.asar.unpacked') : __dirname}) {
  ...
  async function refresh() {
    ...
    busy = true;
    offer = null;
    let balance = {remaining:null,purchased:null,dollars:null};
    ...
    try {
      ...
      if (!account.signedIn) { text = 'Sign in for credits'; description = 'Sign in to Aesthetic Computer to view your daily allowance.'; }
        ...
        offer = value.offer;
        balance = {remaining:value.remaining,purchased:value.purchased,dollars:value.dollars??null};
    ...
    if (text === lastText && lastPayload) { lastPayload={...lastPayload,...balance,description,offer}; send(lastPayload); busy=false; return; }
    execFile(join(nativeRoot,'native','credit-label'),[text,'14'],{encoding:'buffer',timeout:5000,maxBuffer:1024*1024},(error,png)=>{
      busy=false;
      if (disposed) return;
      lastText=text;
      lastPayload={text,...balance,description,offer,image:error?null:`data:image/png;base64,${png.toString('base64')}`};
      send(lastPayload);
    });
  }
