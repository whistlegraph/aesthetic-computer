...
export const API_WORKFLOW = `AC JavaScript workflow: Use the ac MCP tools directly. If discovery is necessary, match exact names; never dump ALL_TOOLS or broadly search tool descriptions for input/publish. aesel owns preview refresh and automatic publishing. lifecycle functions receive the AC API object; it is not CanvasRenderingContext2D. Before using an unfamiliar symbol, call ac_api with its exact name or a task such as "text fonts". Use the returned signatures and examples; do not invent browser-style overloads. If still unclear, use ac_examples for that symbol, then ac_outline/ac_symbol on one relevant file instead of repeatedly scanning the repository. ac_references finds identifier occurrences in its syntax tree; it does not resolve scope or types. Runtime references such as lib/cam-doll.mjs are bundled and can be passed directly to ac_outline/ac_symbol. After editing, call ac_preview and inspect the current revision before claiming it works. Missing feedback is not a successful test.`;
