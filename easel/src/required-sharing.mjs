...
import {MASCOT_SETTLED_MS,mascotRows} from './mascot.mjs';
...
export const DISCLOSURE_COLUMNS=64;
export function sharingScreen({columns=80,rows=24,useColor=true,signedIn=false}={}){
 ...
 const donkey=mascotRows(MASCOT_SETTLED_MS).rows.map(({text,tone})=>({
  ...
  painted:useColor?text.replace(/\s+$/,'').split('*').map(part=>ink(tone,part)).join(ink('highlight','*')):text.replace(/\s+$/,''),
 }));
 ...
 const block=[
  ...donkey.map(row=>({plain:row.text,painted:row.painted})),
  {plain:'',painted:''},
  {plain:'Aesel data sharing',painted:ink('highlight','Aesel data sharing')},
  {plain:'',painted:''},
  ...paragraph.map(line=>({plain:line,painted:ink('soft',line)})),
  {plain:'',painted:''},
