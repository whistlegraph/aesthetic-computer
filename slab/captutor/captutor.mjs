...
import { assertBookmarksHidden } from "./lib/chrome-bookmarks.mjs";
import { assertPresentationClean } from "./lib/presentation-check.mjs";
...
import { mux, writeVTT, probe, preventOverlappingCues } from "./lib/compose.mjs";
import { rhythmBedClip } from "./lib/rhythm-bed.mjs";
...
import {nativeDisplayFormat} from "./lib/native-display.mjs";
...
const NATIVE_DISPLAY = process.env.CAPTUTOR_NATIVE_DISPLAY ? JSON.parse(process.env.CAPTUTOR_NATIVE_DISPLAY) : null;
...
if (STAGE_MODE && !NATIVE_DISPLAY && process.env.CAPTUTOR_STAGE_GEOMETRY) {
...
if(NATIVE_DISPLAY){
 if(!STAGE_MODE)throw Error("Native display capture requires Stage Mode");
 Object.assign(FORMATS.docs,nativeDisplayFormat(NATIVE_DISPLAY));
}
...
async function cmdRender(sp, workDir, locale, format, attempt = 1) {
  ...
  if(NATIVE_DISPLAY && format!=="docs")throw Error("Native display capture currently requires docs format");
  ...
  const cdp = STAGE_MODE
    ? await connectWithModalPolice(() => attach(sp.match || sp.baseURL))
    : await attach(sp.match || sp.baseURL);
  ...
  if (STAGE_MODE && F.compose?.fullDesktop && !NATIVE_DISPLAY) await sizeWindow(cdp, F.win);
  ...
  if(!NATIVE_DISPLAY)await sizeWindow(cdp, F.win);
  ...
  try {
    ...
    if (STAGE_MODE) {
      const modalEvents = [];
      const preparation = await createModalPolice({ onEvent: event => modalEvents.push(event) }).check("preparing");
      const banner = {
        schema: "captutor-chrome-presentation/v1", checkedAt: preparation.checkedAt,
        dismissed: modalEvents.some(event => event.kind === "automation-banner" && event.type === "handled"),
        bannerFound: modalEvents.some(event => event.kind === "automation-banner"),
      };
      await sleep(250);
      const clean = { ...assertPresentationClean(), ...assertBookmarksHidden() };
      writeFileSync(join(workDir, "chrome-presentation.json"), JSON.stringify({...banner,...clean}, null, 2));
    }
    ...
    if(NATIVE_DISPLAY)writeFileSync(join(workDir,"capture-geometry.json"),JSON.stringify({mode:"native",display:NATIVE_DISPLAY,browser:screen,output:F.out,preserveWindow:true},null,2));
    assertHiDPIStage({
      required:REQUIRE_HIDPI,
      stageMode:STAGE_MODE,
      vertical:VERTICAL_MODE,
  ...
  const take = (async () => {
    ...
    for (const beat of beats) {
      ...
      if (STAGE_MODE) assertPresentationClean();
  ...
  const modalPolice = STAGE_MODE ? createModalPolice({
    onEvent: event => {
      appendFileSync(join(workDir, "modal-police.jsonl"), JSON.stringify(event) + "\n");
      process.stderr.write(JSON.stringify({ source: "modal-police", ...event }) + "\n");
    },
  }) : null;
  const modalGuard = (async () => {
    if (!modalPolice) return new Promise(() => {});
    while (recording) {
      assertBookmarksHidden();
      await modalPolice.check("recording");
      await sleep(750);
    }
    return null;
  })();
  let timed;
  try {
    timed = await Promise.race([take, upgradeGuard, browserGuard, modalGuard]);
  } catch (err) {
  ...
  // A bake-time fold remaps offsets independently of the runner's own
  // never-overlap invariant; re-assert it mechanically on whatever the final
  // timeline turns out to be, before it's written anywhere.
  compositionTimed = preventOverlappingCues(compositionTimed);
  ...
  const finalDurationSec = bakeTime ? bakeTime.outputDurationSec : sourceDurationSec;
  const bed = sp.rhythmBed
    ? rhythmBedClip({ durationSec:finalDurationSec, ...(sp.rhythmBed === true ? {} : sp.rhythmBed) })
    : null;

  mux({
    clip:compositionClip, beats:compositionTimed, out:outMp4, vtt:outVtt,
    bed, bedDurationSec:bed ? finalDurationSec : null,
  });
  ...
  writeFileSync(storyboard, JSON.stringify({
    ...
    subtitle:sp.subtitle == null ? "" : say(sp.subtitle, locale),
    ...
    receiptEnglish:{
      ...
      subtitle:sp.subtitle == null ? "" : say(sp.subtitle, "en"),
