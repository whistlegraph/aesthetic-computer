import {narrationRequest} from './narration-voice.mjs';
...
async function speak(line, { voice = "jeffrey", dir }) {
  const request = narrationRequest(voice,line);
  const key = createHash("sha256").update(`provider-v2:${JSON.stringify(request)}`).digest("hex").slice(0, 16);
