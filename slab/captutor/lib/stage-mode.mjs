...
const macpal = macPalStage();
import { chromeBookmarksStage } from "./chrome-bookmarks.mjs";
const bookmarks = chromeBookmarksStage();
import {
...
export function parseStageFlags(rawArgs = []) {
  ...
  const native = rawArgs.includes("--native");
  if(native && vertical)throw new Error("--native preserves display orientation; do not combine with --vertical");
  ...
  const args = rawArgs.filter((arg, index) =>
    arg !== "--vertical" && arg !== "--native" &&
    (brandIndex < 0 || (index !== brandIndex && index !== brandIndex + 1)));
  return { vertical, native, brand, args };
}
...
export function nativeDisplayGeometry() {
  return JSON.parse(swift('import CoreGraphics; import Foundation; let m = CGDisplayCopyDisplayMode(CGMainDisplayID())!; let d: [String:Int] = ["width":m.width,"height":m.height,"pixelWidth":m.pixelWidth,"pixelHeight":m.pixelHeight]; print(String(data:try! JSONSerialization.data(withJSONObject:d),encoding:.utf8)!)'));
}
...
export async function enterStageMode({
  ...
  const state = {
    ...
    displayChanged: vertical || process.env.CAPTUTOR_STAGE_KEEP_DISPLAY !== "1",
    ...
    darkMode: darkMode(),
    macpal: macpal.capture(),
    chromeBookmarksShown: bookmarks.capture(),
  ...
  await macpal.hide(state.macpal);
  bookmarks.hide();
...
export async function exitStageMode() {
  ...
  await restore("Chrome bookmarks", () => bookmarks.restore(state.chromeBookmarksShown));
  await restore("MacPal", () => macpal.restore(state.macpal));
  await restore("Stats", () => {
  ...
  await restore("display", () => {
    if (state.displayChanged === false) return;
