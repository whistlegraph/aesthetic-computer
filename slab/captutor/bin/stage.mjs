...
import { enterStageMode, exitStageMode, parseStageFlags, nativeDisplayGeometry } from "../lib/stage-mode.mjs";
...
const { vertical, native, brand, args } = parsed;
if (!args.length) {
  console.error("usage: node bin/stage.mjs [--native | --vertical] [--brand fuser|classic] render <screenplay> [captutor options]");
  process.exit(2);
}
...
try {
  ...
  const nativeGeometry = native ? nativeDisplayGeometry() : null;
  if(native)process.env.CAPTUTOR_STAGE_KEEP_DISPLAY="1";
  ...
  child = spawn(process.execPath, [resolve(HERE, "../captutor.mjs"), ...args], {
    ...
    env: {
      ...
      CAPTUTOR_NATIVE_DISPLAY: nativeGeometry ? JSON.stringify(nativeGeometry) : "",
