...
final class PromptPreview {
    ...
    /// Holds the web view at viewport size and is what gets scaled: the web
    /// view's own layer belongs to WebKit, and a transform set on it is a
    /// transform WebKit may take back.
    ...
    private func layoutWindow() {
        ...
        if loadedURL.isEmpty {
            stage.frame = NSRect(origin: .zero, size: viewport)
            return
        }
    ...
    private var stageSize: CGSize { stage.frame.size }
    ...
    private func settleResize() {
        ...
        let reframe = { [weak self] in
            ...
            self.stage.frame = NSRect(origin: .zero, size: target)
    ...
    private func layoutCard(animated: Bool) {
        ...
        if animated {
            ...
            if let layer = stage.layer {
                // A view's backing layer takes no implicit animation, so the
                // scale is animated by hand — from wherever it is on screen
                // right now, so a hover reversed mid-flight turns around
                // rather than jumping to one end first.
            ...
            shadow.frame = shadowRect
            border.frame = borderRect
            stage.layer?.removeAnimation(forKey: "scale")
    ...
    /// Scale the stage about its own bottom-left, which is also the card's:
    /// the layer scales about its centre, so the shrunken stage is slid back
    /// into the corner by half of what it lost on each axis. Both states put
    /// the stage's corner on the card's corner, which is what lets the two
    /// animate together without either drifting.
    private func stageTransform() -> CATransform3D {
        let s = scale, size = stageSize
        var t = CATransform3DMakeTranslation(-(size.width * (1 - s)) / 2,
                                             -(size.height * (1 - s)) / 2, 0)
...
/// A borderless window that can take the keyboard. AppKit's borderless
/// windows refuse key status by default, which is right for a card that is
/// only looked at and wrong for one being played.
