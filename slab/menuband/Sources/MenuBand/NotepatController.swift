...
final class NotepatController {
    private let midi = NotepatMIDI()
    private let synth = NotepatSynth()
    ...
    private func enableTypeMode(promptForPermission: Bool, playFeedback: Bool = false) {
        ...
        if !tap.start() {
            NSLog("Notepat: CGEventTap creation failed (likely missing Accessibility permission)")
            UserDefaults.standard.set(false, forKey: typeModeKey)
            onChange?()
            return
        }
    ...
    func startTapNote(_ midiNote: UInt8, velocity: UInt8 = 100, pan: UInt8 = 64) {
        ...
        let ch: UInt8 = isDrum ? 9 : nextMelodicChannel()
        tapNoteChannel[midiNote] = ch
        // Set pan first so the noteOn lands on a panned channel.
        midi.sendCC(10, value: pan, channel: ch)
        if !midiMode { synth.noteOn(midiNote, velocity: velocity, channel: ch) }
        midi.noteOn(midiNote, velocity: velocity, channel: ch)
    ...
    func updateTapPan(_ midiNote: UInt8, pan: UInt8) {
        guard let ch = tapNoteChannel[midiNote] else { return }
        midi.sendCC(10, value: pan, channel: ch)
    }
    ...
    func stopTapNote(_ midiNote: UInt8) {
        ...
        let ch = tapNoteChannel.removeValue(forKey: midiNote) ?? channel(for: midiNote)
        let isDrum = midiNote < UInt8(KeyboardIconRenderer.firstMidi)
        ...
        if !isDrum && !midiMode {
            synth.noteOff(midiNote, channel: ch)
        }
        midi.noteOff(midiNote, channel: ch)
    ...
    private func handleKey(keyCode: UInt16, isDown: Bool, isRepeat: Bool, flags: CGEventFlags) -> Bool {
        ...
        if isDown {
            ...
            guard let note = NotepatLayout.midiNote(forKeyCode: keyCode, octaveShift: shift, keymap: keymap) else {
