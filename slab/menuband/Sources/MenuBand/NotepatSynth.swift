...
// percussion (channel 9 — GM drum kit). NotepatController routes drum notes
// to the drum sampler.
final class NotepatSynth {
    ...
    func start() {
        ...
        do {
            ...
            NSLog("Notepat synth engine start failed: \(error)")
            return
        }
        loadDefaultPatches()
        primeForLowLatency()
    ...
    private func loadDefaultPatches() {
        let url = NotepatSynth.bankURL
        guard FileManager.default.fileExists(atPath: url.path) else {
            NSLog("Notepat: gs_instruments.dls not found — falling back to default sampler tone")
            return
        }
        ...
        do {
            ...
            NSLog("Notepat: melodic patch load failed: \(error)")
        }
        do {
            try drums.loadSoundBankInstrument(at: url, program: 0, bankMSB: percussionMSB, bankLSB: lsb)
        } catch {
            NSLog("Notepat: drum kit load failed: \(error)")
        }
    }
