...
set blueberry_ssh jas@blueberry.local
set chicken_ssh fusermacminichicken@chicken.local
set panda_ssh fusermacminipanda@panda.local

for host in $blueberry_ssh $chicken_ssh $panda_ssh
    ssh $host "rm -rf $stage; mkdir -p $stage"
    or exit 1
    scp -q $here/deskflow-* $here/unipointer.swift $here/install.sh $host:$stage/
    or exit 1
end
...
or exit 1
ssh $blueberry_ssh "bash $stage/install.sh --defer-start --machine blueberry --screen-name blueberry.local --address 100.79.75.53 --controller --clients jas@neo.local,chicken,panda --role client --server-host 100.108.5.81 --trusted-servers $neo_fp --trusted-clients $neo_fp,$chicken_fp,$panda_fp"
or exit 1
ssh $chicken_ssh "bash $stage/install.sh --defer-start --machine chicken --screen-name chicken.local --address 100.98.158.126 --role client --server-host 100.108.5.81 --trusted-servers $neo_fp,$blueberry_fp"
or exit 1
ssh $panda_ssh "bash $stage/install.sh --defer-start --machine panda --screen-name panda.local --address 100.88.155.94 --role client --server-host 100.108.5.81 --trusted-servers $neo_fp,$blueberry_fp"
or exit 1
