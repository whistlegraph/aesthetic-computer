...
import { withMachineLease, withMachineLeaseSync } from "../lib/computer-use-lease.mjs";

// ptyInject uses 50ms; 60 leaves headroom for ssh round-trip jitter.
const SUBMIT_DELAY_MS = 60;
const MAX_BUFFER = 16 * 1024 * 1024;

// Known local automation commands need no login shell or shell startup files.
const LOCAL_COMMANDS = {
  "osascript -": ["/usr/bin/osascript", ["-"]],
  "osascript -l JavaScript -": ["/usr/bin/osascript", ["-l", "JavaScript", "-"]],
  pbcopy: ["/usr/bin/pbcopy", []],
};
function invocation(spec, command) {
  if (!spec) throw new Error("no machine spec");
  if (spec.local) return LOCAL_COMMANDS[command] || ["bash", ["-lc", command]];
  const q = "'" + command.replace(/'/g, "'\\''") + "'";
  if (spec.ssh) return ["ssh", [...spec.ssh.split(/\s+/), `bash -lc ${q}`]];
  if (spec.sshHost) return ["ssh", [spec.sshHost, `bash -lc ${q}`]];
...
function shAsync(spec, command, { stdin } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // A failed spawn or an early child exit can close stdin before the write.
    child.stdin.on("error", () => {});
    child.stdin.end(stdin);
  });
}

// Capture the existing synchronous operation as a command plan, then execute
// it without blocking the MCP event loop. Serialize the WHOLE operation per
// host so a second paste cannot replace the clipboard before the first Cmd+V.
const operations = new Map();
function asyncOperation(fn, spec, args, optionIndex) {
  const commands = [];
  args[optionIndex] = { ...args[optionIndex], run: (...command) => { commands.push(command); return ""; } };
  fn(spec, ...args);
  const key = spec.local ? "local" : spec.ssh || spec.sshHost;
  const previous = operations.get(key) || Promise.resolve();
  const operation = previous.catch(() => {}).then(() => withMachineLease(spec, async () => {
    let result = "";
    for (const command of commands) result = await shAsync(...command);
    return result;
  }));
  operations.set(key, operation);
  const cleanup = () => { if (operations.get(key) === operation) operations.delete(key); };
...
function clickPointCore(spec, x, y, { count = 1, run = sh } = {}) {
...
export function osa(spec, script, { run = sh } = {}) {
  return run(spec, "osascript -", { stdin: script });
}

export function jxa(spec, script, { run = sh } = {}) {
  return run(spec, "osascript -l JavaScript -", { stdin: script });
}
...
function hoverPointCore(spec, x, y, { run = sh } = {}) {
...
export function termList(spec, { run = sh } = {}) {
  return osa(
    spec,
...
function typeTextCore(spec, text, { tty, paste, enter, clear, run = sh } = {}) {
  ...
  if (paste) run(spec, "pbcopy", { stdin: text });
...
function sendKeysCore(spec, key, mods = [], { run = sh } = {}) {
  const using = mods
...
export function clickPoint(spec, ...args) { return withMachineLeaseSync(spec, () => clickPointCore(spec, ...args)); }

export function hoverPoint(spec, ...args) { return withMachineLeaseSync(spec, () => hoverPointCore(spec, ...args)); }

export function sendKeys(spec, ...args) { return withMachineLeaseSync(spec, () => sendKeysCore(spec, ...args)); }

export function typeText(spec, ...args) { return withMachineLeaseSync(spec, () => typeTextCore(spec, ...args)); }
