...
Close the lid while Claude has active work in flight (with sleep
suppressed) and the laptop becomes a "sleeping slab": an ambient
C-major-pentatonic bed loops through the speakers while a mic-listener
picks up claps, snaps, shushes, kisses, hums, and close-up sings —
responding in real time with short pentatonic pluck-arpeggios that mirror
each sound's pitch contour. Close the lid with nothing active and it's
silent. When Claude finishes a prompt, it beeps once per other in-flight
prompt or subagent; when the last piece of work completes, the "all-done"
chime plays — and if the lid is closed, the Mac sleeps right after.

Active work is tracked as marker files in two directories:

- `$SLAB_HOME/state/active-prompts/<session_id>` — created by the
  `UserPromptSubmit` hook, removed by `Stop`.
- `$SLAB_HOME/state/active-subagents/<ts>-<pid>-<n>` — created by the
  `PreToolUse` hook (filtered to the Task tool), removed (oldest-first)
  by `SubagentStop`.
...
| Event | Sound |
|---|---|
| Lid close (`disablesleep=1`) | display sleep. Ambient only if a prompt or subagent is in flight. |
| Prompt or subagent starts while lid closed | ascending C-arp + ambient loop starts |
| Mic transient while ambient is playing (> 2 kHz) | 4-note pentatonic pluck arp, contour mirrors input |
| Lid open while ambient playing | descending ding + smooth descending arp stinger + ambient stops |
...
│   ├── claude-tool-pre.sh               # PreToolUse(Task) hook (touches active-subagents marker)
│   ├── claude-subagent-stop.sh          # SubagentStop hook (pops oldest subagent marker + ping)
