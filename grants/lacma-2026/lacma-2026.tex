...
\begin{center}
  \includegraphics[width=0.85\textwidth]{kidlisp-gallery}\\[4pt]
  {\small\color{acgray} \textbf{Fig.\,3} --- Four KidLisp ``cards'' showing code and output. Each card is a self-contained program: Spiral (math), Tree (recursion), Sunflower (nature), Mondrian (art history). Cards can be printed, shared, and minted on-chain.}
\end{center}

\vspace{1em}

\begin{center}
  \includegraphics[width=0.4\textwidth]{kidlisp-spiral}\\[4pt]
  {\small\color{acgray} \textbf{Fig.\,4} --- A single KidLisp card up close. The source code is 8 lines of Lisp. No imports, no build step, no dependencies---the entire program is visible on the card.}
\end{center}
...
\begin{center}
  ...
  {\small\color{acgray} \textbf{Fig.\,5} --- Target hardware: a Lenovo Yoga convertible laptop. AC Native boots from USB on any x86 machine in under 2 seconds, turning commodity hardware into a dedicated creative instrument.}
\end{center}
