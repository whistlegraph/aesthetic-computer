...
\begin{center}
  ...
  {\large A Planetary Laptop Orchestra}\\[24pt]
...
\textbf{AC Native} is the kernel. A Linux boot that runs a custom C runtime as PID\,1 on x86\_64 UEFI laptops---no desktop, no window manager, no browser. It renders graphics through DRM without a compositor, reads input from raw evdev streams, and synthesizes audio sample-by-sample through ALSA at 192\,kHz with 32-voice polyphony. Per-seat cost lands near \$50, two orders of magnitude below Princeton's PLOrk laptop-orchestra model. A built-in \texttt{code} command drops into a native terminal running Anthropic's Claude Code---the only bare-metal creative OS we know of with an AI coding partner built in. The default piece is \textit{notepat}, an 8,466-line polyphonic instrument with eight waveforms, room reverb, sample recording, and USB\,+\,UDP MIDI; twenty other pieces ship alongside it.
...
\begin{enumerate}
  \item \textbf{Portable Instruments}---USB-bootable AC Native drives preloaded with curated pieces for visitors to take home and boot on their own laptops.
