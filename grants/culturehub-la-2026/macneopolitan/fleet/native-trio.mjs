...
// The room's lyric language (LYRICS-SCREENS.md): the seat's colour edge to
// edge; the HEADLINE is the lead line being sung, syllables lit as sung, the
// singer's name above, the next lead line faint beneath; an ANSWER line is
// bigger, boxed in a tint, flashes to white on its first syllable and lingers
// after it ends; HUMS are texture: small, low, breathing with the beat.
const LINGER_ANSWER=2.5,LINGER=.8,FLASH=.25;
let headCursor=0,humCursor=0;
function layoutRows(words,w,maxSize,minSize,maxRows){
 // biggest size at which the words wrap into ≤ maxRows rows; returns [size, rows]
 for(let size=maxSize;size>=minSize;size--){
  const rows=[];let row='';
  for(const wd of words){const cand=row?row+' '+wd:wd;if(textWidth(cand,size)>w*.9&&row){rows.push(row);row=wd;}else row=cand;}
  if(row)rows.push(row);
  if(rows.length<=maxRows||size===minSize)return [size,rows];
 }
 return [minSize,[words.join(' ')]];
}
function drawKaraoke(write,ink,line,t,x0,y0,size,rowH,w,litRGB,dimRGB){
 // syllables map onto words in order; each word takes the syllables that spell it
 const words=line.text.split(' '),syls=line.syllables||[];
 let sung=-1;for(let i=0;i<syls.length;i++){if(syls[i].t<=t)sung=i;else break;}
 const [sz,rows]=[size,null];
 let sylIdx=0,r=0,row='';const rowsOut=[];
 for(const wd of words){const cand=row?row+' '+wd:wd;if(textWidth(cand,sz)>w*.9&&row){rowsOut.push(row);row=wd;}else row=cand;}
 if(row)rowsOut.push(row);
 for(const rw of rowsOut){
  let x=Math.round((w-textWidth(rw,sz))/2),y=y0+r*rowH;r++;
  for(const wd of rw.split(' ')){
   let take=0,acc='';while(sylIdx+take<syls.length&&acc.length<wd.length){acc+=syls[sylIdx+take].text;take++;}
   if(take===0)take=1;
   let cx=x;
   for(let k=0;k<take;k++){const piece=syls[sylIdx+k]?.text??wd;const lit=(sylIdx+k)<=sung;ink(...(lit?litRGB:dimRGB));write(piece,{x:cx,y,font:FONT,size:sz});cx+=textWidth(piece,sz);}
   sylIdx+=take;x+=textWidth(wd,sz)+6*sz;
  }
 }
