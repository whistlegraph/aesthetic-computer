...
export function layoutBrandChrome(theme, {
  width, height, format = "docs", fps:frameRate = "30",
}) {
  if (!theme || typeof theme !== "object") throw new Error("brand chrome needs a theme object");
  if (!theme.asset && !((theme.markAsset || theme.markVideoAsset) && (theme.label || theme.labelAsset))) {
  ...
  const fpsParts = String(frameRate).split("/").map(Number);
  const fps = fpsParts[0] / (fpsParts[1] || 1);
  ...
  return {
    ...
    markVideoAsset: theme.markVideoAsset ? resolve(String(theme.markVideoAsset)) : null,
    markPreserveColor: Boolean(merged.markPreserveColor),
    markOrientation: merged.markOrientation === "upright" ? "upright" : "side",
    ...
    markPerspective: merged.markPerspective && typeof merged.markPerspective === "object"
      ? {
          amount: clamp(finite(merged.markPerspective.amount, 0.08), 0.02, 0.16),
          periodSec: clamp(finite(merged.markPerspective.periodSec, merged.periodSec), 4, 30),
        }
      : null,
...
function renderSeparateElements(layout, workDir) {
  const animatedMark = Boolean(layout.markVideoAsset);
  const markSource = layout.markVideoAsset || layout.markAsset;
  if (!existsSync(markSource)) throw new Error(`brand mark does not exist: ${markSource}`);
  ...
  if (animatedMark) {
    // The checked-in mark movie contains unique 60 fps ray-marched views of
    // the real 3D surface. Keep it upright when the client contract asks for
    // an emblem beside vertical type; never fake depth by warping a still.
  } else if (extname(layout.markAsset).toLowerCase() === ".svg") {
    execFileSync("rsvg-convert", [
    ...
    execFileSync("magick", [
      layout.markAsset, "-resize", `${layout.markSide * 2}x${layout.markSide * 2}!`,
      markMaster,
    ], { stdio:"pipe" });
  }
  // Rasterize at 2× for clean edges, then return to the contracted size. A
  // dimensional client mark may preserve its authored light and material;
  // flat glyph assets continue to receive the theme's mark color.
  if (!animatedMark) {
    const markArgs = [markMaster, "-resize", `${layout.markSide}x${layout.markSide}!`];
    if (!layout.markPreserveColor) {
      markArgs.push("-fill", layout.markColor, "-colorize", "100");
    }
    markArgs.push(mark);
    execFileSync("magick", markArgs, { stdio:"pipe" });
  }
  if (layout.labelAsset) {
    ...
    const labelArgs = [
      "-background", "none", "-fill", layout.labelColor,
    ];
    if (layout.labelStrokeFraction > 0) labelArgs.push(
      "-stroke", layout.shadow.color || defaults.shadow.color,
    ...
    execFileSync("magick", labelArgs, { stdio:"pipe" });
  }
  if (!animatedMark) sharpShadow(mark, markShadow, layout.shadow);
  if (layout.labelShadow) sharpShadow(label, labelShadow, layout.shadow);
  ...
  const labelSource = layout.labelShadow ? labelShadow : label;
  const result = {
    kind:"separate",
    labelLeft:rotate(labelSource, "label-left", 90),
    labelRight:rotate(labelSource, "label-right", -90),
  };
  if (animatedMark) {
    const animated = (suffix, transpose) => {
      const output = join(workDir, `${stem}-${suffix}.mov`);
      const orient = layout.markOrientation === "upright"
        ? ""
        : `,transpose=${transpose}`;
      execFileSync(FFMPEG, [
        "-y", "-i", layout.markVideoAsset,
        "-vf", `scale=${layout.markSide}:${layout.markSide}:flags=lanczos${orient},format=argb`,
        "-an", "-c:v", "qtrle", output,
      ], { stdio:["ignore", "ignore", "pipe"] });
      return output;
    };
    result.markLeft = animated("mark-left", "clock");
    result.markRight = animated("mark-right", "cclock");
    result.animatedMark = true;
  } else {
    result.markLeft = rotate(markShadow, "mark-left", 90);
    result.markRight = rotate(markShadow, "mark-right", -90);
  }
...
export function separateBrandChromeFilter(layout, characterCount = 0, outputLabel = "outv") {
  const phase = "2*PI*t/" + layout.periodSec.toFixed(3);
  ...
  const markPlate = (input, name, phaseOffset = 0) => {
    if (!layout.markPerspective) {
      return `[${input}:v]format=rgba,colorchannelmixer=aa=${layout.opacity.toFixed(3)}[${name}]`;
    }
    const amount = layout.markPerspective.amount.toFixed(4);
    const frames = Math.max(1, Math.round(layout.fps * layout.markPerspective.periodSec));
    const offsetFrames = Math.round(frames * phaseOffset);
    const turn = `2*PI*(in+${offsetFrames})/${frames}`;
    return (
      `[${input}:v]format=rgba,pad=iw*1.55:ih*1.55:(ow-iw)/2:(oh-ih)/2:color=black@0,` +
      `perspective=` +
      `x0='0.08*W+${amount}*W*sin(${turn})':y0='0.08*H-${amount}*H*cos(${turn})':` +
      `x1='0.92*W-${amount}*W*sin(${turn})':y1='0.08*H+${amount}*H*cos(${turn})':` +
      `x2='0.08*W-${amount}*W*sin(${turn})':y2='0.92*H-${amount}*H*cos(${turn})':` +
      `x3='0.92*W+${amount}*W*sin(${turn})':y3='0.92*H+${amount}*H*cos(${turn})':` +
      `sense=destination:eval=frame:interpolation=cubic,` +
      `colorchannelmixer=aa=${layout.opacity.toFixed(3)}[${name}]`
    );
  };
  if (characterCount > 0 && layout.characterMotion) {
    ...
    for (let i = 0; i < characterCount; i += 1) {
      ...
      const next = i === characterCount - 1 ? outputLabel : `vr${i + 1}`;
  ...
  return [
    ...
    `[c][tr]overlay=x='${rightX}+${layout.driftPx}*sin(${phase}+2.1)':y='${y(layout.rightLabelCenterY, "+", layout.bobPx)}':eval=frame:shortest=1[${outputLabel}]`,
  ].join(";");
}
...
function mixSectionColor(base, tint, amount = 0.46) {
  const parse = (value, fallback) => /^#[0-9a-f]{6}$/i.test(String(value || ""))
    ...
    : fallback;
  const from = parse(base, [79, 61, 89]);
  const to = parse(tint, from);
  return `#${from.map((part, index) => Math.round(part + (to[index] - part) * amount)
    .toString(16).padStart(2, "0")).join("")}`;
}

function renderSectionLabels(layout, { title, chapters, duration }, workDir) {
  if (!layout.sectionLabel || !title || !Array.isArray(chapters) || !chapters.length) return [];
  ...
  return ordered.map((chapter, index) => {
    const stem = `section-${String(index).padStart(2, "0")}`;
    ...
    return {
      ...
      endSec:Number(ordered[index + 1]?.startSec ?? duration),
      color, copy,
    };
  });
}

function sectionLabelFilter(layout, sections, firstInput, inputLabel) {
  if (!sections.length) return "";
  const phase = `2*PI*t/${layout.periodSec.toFixed(3)}`;
  const leftX = `${layout.edgePx + layout.sectionLabelGapPx}-overlay_w/2`;
  const rightX = `main_w-${layout.edgePx + layout.sectionLabelGapPx}-overlay_w/2`;
  const filters = [];
  let previous = inputLabel;
  sections.forEach((section, index) => {
    const leftInput = firstInput + index * 2;
    const rightInput = leftInput + 1;
    const leftPlate = `section-left-${index}`;
    const rightPlate = `section-right-${index}`;
    const leftOut = `section-left-out-${index}`;
    const rightOut = index === sections.length - 1 ? "outv" : `section-right-out-${index}`;
    const enable = `gte(t,${section.startSec.toFixed(3)})*lt(t,${section.endSec.toFixed(3)})`;
    filters.push(
      `[${leftInput}:v]format=rgba,colorchannelmixer=aa=${layout.sectionLabelOpacity.toFixed(3)}[${leftPlate}]`,
      `[${rightInput}:v]format=rgba,colorchannelmixer=aa=${layout.sectionLabelOpacity.toFixed(3)}[${rightPlate}]`,
      `[${previous}][${leftPlate}]overlay=x='${leftX}':` +
      `y='main_h*${layout.leftLabelCenterY.toFixed(5)}-overlay_h/2+${layout.bobPx}*sin(${phase})':` +
      `enable='${enable}':eval=frame:shortest=1[${leftOut}]`,
      `[${leftOut}][${rightPlate}]overlay=x='${rightX}':` +
      `y='main_h*${layout.rightLabelCenterY.toFixed(5)}-overlay_h/2-${layout.bobPx}*sin(${phase})':` +
      `enable='${enable}':eval=frame:shortest=1[${rightOut}]`,
    );
    previous = rightOut;
  });
  return filters.join(";");
...
export function applyBrandChrome({
  input, out, theme, workDir, format = "docs", context = null,
}) {
  ...
  const lockups = (layout.markAsset || layout.markVideoAsset) && (layout.label || layout.labelAsset)
    ? renderSeparateElements(layout, assetsDir)
    : renderLockups(layout, assetsDir);
  const sectionLabels = lockups.kind === "separate"
    ? renderSectionLabels(layout, {
        title:context?.title,
  ...
  return { out, layout, lockups, sectionLabels, source:basename(input) };
}
