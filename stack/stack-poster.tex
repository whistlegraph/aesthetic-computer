...
\chip{cDO}{DO} droplet &
Chat + single shared realtime room (Geckos WS+UDP). Loads chat history on boot ($\sim$45s) &
fish session-server/deploy.fish \newline systemd, node 20.17 (fnm) \\
\addlinespace
\textbf{oven} \newline oven.aesthetic.computer &
\chip{cDO}{DO} 2GB/2vCPU &
...
\chip{cDO}{DO} Droplet --- session-server & subscription & \tbd \\
\chip{cDO}{DO} Droplet --- oven & subscription & \tbd \\
\chip{cDO}{DO} Spaces --- asset CDN & storage + egress & \tbd \\
\chip{cMongo}{MongoDB} Atlas --- primary DB & subscription/usage & \tbd \\
\chip{cRedis}{Redis} --- managed, sfo3 & subscription/usage & \tbd \\
