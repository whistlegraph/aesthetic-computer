...
\twocolumn[{%
\begin{center}
{\acbold\fontsize{27pt}{31pt}\selectfont\color{acdark} Gaps in {\color{acgreen}Granularity}}\par
\vspace{0.5em}
\includegraphics[width=0.7\textwidth]{figures/cover-video-v4}\par
\vspace{0.4em}
{\itshape\fontsize{14.5pt}{17pt}\selectfont\color{acpink} Expressive Control and the Impoverishment of Consumer Playback}\par
\vspace{0.5em}
...
Consumer playback interfaces reduce continuous media to a small set of discrete commands. A record under a hand offers something richer: position, speed, direction, and pitch remain coupled to one gesture, with the sound itself as immediate feedback. I call the difference between these systems a \emph{gap in granularity}. Drawing on direct manipulation, input bandwidth, turntablism, and elastic audio, I argue that expressive control of recorded media is already a solved technical problem that consumer platforms have chosen not to expose. \ac{} demonstrates the alternative through tape-like players in which scrubbing, speed, and pitch remain continuous properties of touch. Playback can be an instrument, not merely a settings panel.
\end{quote}
\vspace{0.5em}
}]
