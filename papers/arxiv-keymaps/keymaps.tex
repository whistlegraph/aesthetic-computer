...
\centering
\begin{tikzpicture}[
  valve/.style={draw=keystroke, fill=keyfill, circle,
                 minimum size=0.55cm, inner sep=0pt,
                 font=\tiny\sffamily\bfseries},
  vlabel/.style={font=\tiny\sffamily, text=keymapped!90},
  string/.style={draw=keystroke, line width=0.7pt},
  slabel/.style={font=\tiny\sffamily\bfseries, text=keymapped!90},
  hole/.style={draw=keystroke, fill=acdark!85, circle,
                minimum size=0.16cm, inner sep=0pt}
]
% --- Trumpet valves (top-left): 3 valves with semitone offsets ---
\node[font=\footnotesize\sffamily\bfseries, anchor=south west, text=acdark]
  at (0, 1.7) {Trumpet};
\foreach \x/\n/\semi in {0/1/-2, 0.7/2/-1, 1.4/3/-3} {
  \node[valve] at (\x, 1.05) {\n};
  \node[vlabel, anchor=north] at (\x, 0.75) {\semi};
}
\node[font=\tiny\sffamily, text=keystroke, anchor=south]
  at (0.7, 0.45) {(semitones from open)};
% --- Ocarina (top-right): oval with asymmetric hole arrangement ---
\node[font=\footnotesize\sffamily\bfseries, anchor=south west, text=acdark]
  at (3.4, 1.7) {Ocarina};
\draw[draw=keystroke, fill=keyfill, rounded corners=8pt]
  (3.3, 0.55) rectangle (5.4, 1.4);
% Asymmetric hole positions (4-hole "English" pattern simplified)
\foreach \x/\y in {3.55/1.05, 3.85/0.75, 4.20/1.05, 4.50/0.75, 4.85/1.05, 5.15/0.75}
  \node[hole] at (\x, \y) {};
\node[font=\tiny\sffamily, text=keystroke, anchor=south]
  at (4.35, 0.30) {(holes asymmetric across body)};
% --- Guitar tuning (bottom-left): 6 strings, irregular B interval ---
\node[font=\footnotesize\sffamily\bfseries, anchor=south west, text=acdark]
  at (0, -0.2) {Guitar};
\foreach \i/\note/\hl in {1/E/0, 2/A/0, 3/D/0, 4/G/0, 5/B/1, 6/E/0} {
  \draw[string] (0, {-0.5 - (\i-1)*0.18}) -- (1.85, {-0.5 - (\i-1)*0.18});
  \ifnum\hl=1
    \node[slabel, anchor=east] at (-0.05, {-0.5 - (\i-1)*0.18}) {\note};
  \else
    \node[font=\tiny\sffamily, text=acdark, anchor=east]
      at (-0.05, {-0.5 - (\i-1)*0.18}) {\note};
  \fi
}
\node[font=\tiny\sffamily, text=keymapped!90, anchor=west]
  at (1.95, -0.86) {$\leftarrow$ M3 (others P4)};
% --- Violin tuning (bottom-right): 4 strings, all P5 ---
\node[font=\footnotesize\sffamily\bfseries, anchor=south west, text=acdark]
  at (3.4, -0.2) {Violin};
\foreach \i/\note in {1/G, 2/D, 3/A, 4/E} {
  \draw[string] (3.4, {-0.5 - (\i-1)*0.21}) -- (5.4, {-0.5 - (\i-1)*0.21});
  \node[font=\tiny\sffamily, text=acdark, anchor=east]
    at (3.35, {-0.5 - (\i-1)*0.21}) {\note};
}
\node[font=\tiny\sffamily, text=keystroke, anchor=west]
  at (5.45, -0.92) {(uniform P5)};
\end{tikzpicture}
\caption{Asymmetric pitch interfaces across instrument families. Trumpet valves produce non-uniform semitone offsets ($-2, -1, -3$); ocarina holes are placed asymmetrically across the body to fit the player's grasp; guitar's 6-string tuning is uniform in perfect fourths \emph{except} for the B-string irregularity (a major-third interval); violin keeps strict P5s but each string is a different absolute pitch. The standard piano keyboard is one member of a larger family in which the physical-to-pitch interface is irregular by design — the diatonic naming keymap is embodied in many surfaces, each with its own asymmetry.}
