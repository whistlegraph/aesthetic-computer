...
\hypersetup{colorlinks=true,linkcolor=acblue,urlcolor=acblue,citecolor=acblue,
  pdfauthor={@jeffrey},
  pdftitle={OSKIEWAR: A Native Xbox Fighting-Game Laboratory for Aesthetic Computer}}
...
{\aclight\fontsize{12pt}{14pt}\selectfont\color{acpink}
  A Native Xbox Fighting-Game Laboratory for Aesthetic Computer}\par
\vspace{0.55em}
...
This paper records the first playable system build of \game{}, a two-player fighting-game laboratory running as a native Universal Windows Platform application on Xbox. The game places Aesthetic Computer's URL-addressable piece model inside a fixed-timestep 3D world rendered through an orthographic gameplay camera, with perspective cinematics for introductions and deaths. It combines quantized controller input, spatial sound, wind and ball physics, weapons, compact deterministic replays, phone spectation, and a low-latency OSC bridge for sound design in Ableton Live. The native host preserves a narrow boundary: downloaded JavaScript can draw, synthesize, read indexed controllers, publish bounded match snapshots, save bounded demos, and emit a fixed game-signal vocabulary, but it cannot open arbitrary sockets or URLs. The current package is version 1.0.0.35, self-signed for Xbox Developer Mode. Thirty-two gameplay/input tests, six spectator/relay tests, native preflight checks, and live telemetry support the implementation. A frame-accurate pose and hitbox pipeline has also been specified; its principal remaining work is replacing the current shared 220\,ms melee pulse with deterministic startup, active, recovery, hit-stop, and block-stun timelines.
\end{quote}
\vspace{0.4em}
}]
...
\begin{tikzpicture}[
  ...
  \node[net,right=of relay] (phone) {Phone viewer\\\code{/oskiewar:match}};
  ...
  \draw[arrow] (osc)--(live);
\end{tikzpicture}
\caption{The native shell owns privileged I/O; the game script receives bounded, purpose-specific capabilities. Public spectators never connect to the console directly.}
...
A ball begins above the platform, falls under gravity, bounces, and is carried by wind while airborne. Grounded wind does not move it. Running into a grounded ball boots it. A punch or kick wacks it; coincident punch and kick produces a stronger cross-wack, scaled by contact geometry. Shielding returns the ball. Only a head contact is lethal and is announced as \textsc{BALLED}; torso or limb contact bounces the ball away. The ball need not remain visible in the camera frustum, preserving it as a world object rather than a HUD mechanic.
...
\section{Endpoint and Capability Inventory}
...
Identity/HUD & @jeffrey, @oskie, @fifi, @sat, DUMMY; handle colors; MOTD/latest-chat enrichment; compact top HUD and lower-third plates. & Remote profile data is opportunistic and may be stale or unavailable.\\
...
The committed verification suite contains 32 gameplay/input tests and six spectator/relay tests. It exercises controller indexing, axis quantization and hysteresis, tap/hold separation, disconnect reset, player-two movement, shield and back-block ordering, trades, head-only ball death, body bounce, pickups, bullet cancellation, round reset, replay shape, match naming, publisher limits, viewer fan-out, malformed payload rejection, and latest-only behavior. Native preflight additionally checks JavaScript syntax, XML/package structure, portable source constraints, and the host diff.
...
The remote session server deploy wrapper presently contains an invalid Fish heredoc and required a guarded direct deployment path. That tooling defect is outside gameplay but is operationally relevant because shared client/server movement code must remain version-aligned.
...
The next technical sequence is narrow: replace the shared melee pulse with the proposed fixed-frame action data; measure end-to-end controller, present, local audio, and OSC-receiver latency over distributions rather than snapshots; add sequence-gap display in the Max device; preserve per-physics replay runners; and complete Microsoft Partner Center association only with the account owner's authenticated review. The existing Store work is packaging readiness, not Store registration.

\section{Conclusion}

\game{} turns a controller diagnostic into a networked fighting-game instrument without abandoning the small-program character of Aesthetic Computer. Its strongest result is the boundary between a highly mutable game script and a deliberately narrow native console host. That boundary supports two controllers, 3D rendering, live sound design, replay, and phone spectation while keeping networking and storage bounded and inspectable. The game is playable now; its remaining work is not feature count but precision---deterministic combat frames, controlled latency measurements, replay compatibility, balance, and certified distribution.
...
\bibitem{repo}
Aesthetic Computer contributors, ``OSKIEWAR source, tests, native host, replay endpoint, spectator relay, and Ableton devices,'' commit \code{8d374b3ec5}, 4 August 2026. \url{https://github.com/whistlegraph/aesthetic-computer}
\end{thebibliography}
