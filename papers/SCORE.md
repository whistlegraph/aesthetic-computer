...
- [`rhythm-platter/`](rhythm-platter/) — reading platter for the geometry of musical rhythm: Toussaint's corpus plus the wider field (maximal evenness, rhythmic oddity, syncopation measures, distance geometry, tiling canons, entrainment, spatial hearing). 31 sources across 8 shelves, 8 digest entries, and [`timelines.json`](rhythm-platter/timelines.json) — 48 Euclidean rhythms extracted from Toussaint (2005) §4 and cross-checked against a local Bjorklund implementation, plus 9 named timelines. Index and citation only; the CRC book is never mirrored, and `sources/` is gitignored. Every digest entry ends in a `tools:` block, because this platter exists to specify [`pop/lib`](../pop/lib/) rhythm tooling and the [`pop/bracelet/`](../pop/bracelet/) spatial-necklace lane. Built via `fetch-sources.mjs` + `build-timelines.mjs`.
