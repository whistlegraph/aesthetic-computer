...
- [`lines-platter/`](lines-platter/) — reading platter for lines, making, and grown marks: Ingold ("Thinking through Making" lecture with local transcript, *The Life of Lines*, *Making*), Hockney/van Eyck, Joselit on painted time, plus the private "Accretion Summer" correspondence with Sage Jenson (2026-08-14). Digest entries end in `tools:` blocks pointing at [`neofarm/`](../neofarm/) and the farm lineage; `sources/` is gitignored and the Sage materials are permission-gated.
- [`people-platter/`](people-platter/) — TODO; will hold AC-adjacent people biographies
- [`prose-platter/`](prose-platter/) — reading platter for the plain register: Hemingway (declarative sentence, the iceberg), Tao Lin (flat affect, exact quantities), Ann Beattie (understatement, objects, the sentence that stops early), with an adjacent Carver/Didion/Davis shelf. Opened 2026-09-20 for *The Ballad of neo*; carries the register as a seven-rule digest plus a before/after stanza. Short study excerpts only, sources named; entries marked **verify** are descriptions awaiting a checked quotation. Feeds `VOICE.md` and the sung lyrics of the MacNeoPolitan lane.
- [`rhythm-platter/`](rhythm-platter/) — reading platter for the geometry of musical rhythm: Toussaint's corpus plus the wider field (maximal evenness, rhythmic oddity, syncopation measures, distance geometry, tiling canons, entrainment, spatial hearing, spoken solkattu, timbre space). 33 sources across 10 shelves — including one OCR'd video source (Levin's konnakol lesson) and the Wessel 1979 timbre-space paper (hand-placed scan from Sage Jenson, 2026-08-15) — 10 digest entries, and [`timelines.json`](rhythm-platter/timelines.json) — 48 Euclidean rhythms extracted from Toussaint (2005) §4 and cross-checked against a local Bjorklund implementation, plus 9 named timelines. Index and citation only; the CRC book is never mirrored, and `sources/` is gitignored. Every digest entry ends in a `tools:` block, because this platter exists to specify [`pop/lib`](../pop/lib/) rhythm tooling and the [`pop/bracelet/`](../pop/bracelet/) spatial-necklace lane. Built via `fetch-sources.mjs` + `build-timelines.mjs`.
- [`chamber-platter/`](chamber-platter/) — reading platter for arranging tonal chamber music in a room of separate speakers: tonal forms at 10–16 minutes, spatial antiphony (cori spezzati, Brant, Stockhausen, Xenakis, PLOrk), process form and sine-stack envelopes, what `pop/` already does, and (05) the psychoacoustics of an audience *inside* a ring of laptop speakers — precedence, onsets, motion limits, masking release, bass below the speaker. Five digests with numbered rules, `sources-NN.json` per digest. Its consumer is *Note(s)pat(ial) Native* (`fedac/native/tools/compose-notespatial-native.mjs`, CultureHub LA, 2026-09-24); the long-form studies sit in `grants/culturehub-la-2026/chamber-studies/`.
...
- [`architecture-platter/`](architecture-platter/) — reading platter for foundational architectural treatises. Opens with a matched pair: Palladio's *Four Books of Architecture* (Ware 1738 folio, Getty/Internet Archive scan) and Durand's *Précis of the Lectures on Architecture* (Getty Texts & Documents translation, 2000) — proportion-as-authority vs. composition-as-algorithm. Index and citation only; PDFs fetched into gitignored `sources/` via `fetch-sources.mjs` (the Getty Durand is in copyright, distributed free by Getty — never redistributed).
- [`sosoft-cycle2`](https://papers.aesthetic.computer/platter/sosoft-cycle2/) — published index of every asset from UCLA Social Software Cycle 2 ("Scores for Social Software," DESMA, spring 2026): the *Keymaps as Social Software* paper and 64-copy edition score, the live sosoft.arts.ucla.edu doc with Casey Reas's edit log, the header-illustration study, the interactive notepat keymap, the March 2026 proposal and KidLisp score cards, and the unboxing reel (review cuts on the assets CDN). Sources live in [`../sosoft/`](../sosoft/), [`../social-software/`](../social-software/), [`../marketing/sosoft-reel/`](../marketing/sosoft-reel/), and `sosoft-keymaps-*` here; page at `system/public/papers.aesthetic.computer/platter/sosoft-cycle2/`.
...
| [`aesthetic-eye.mjs`](aesthetic-eye.mjs) | Render final-PDF page contacts and diagram crops; enforce the colored `Aesthetic.Computer` period and current visual-inference verdicts. See [`AESTHETIC-EYE.md`](AESTHETIC-EYE.md). |
...
| iOS So Far: Ten Years of Aesthetic.Computer on Apple's Store (six app records, one demand event, the last mile; eight ranked openings) | arXiv (LaTeX, 6pp first-pass) | `arxiv-ios-so-far/ios-so-far.pdf` | `arxiv-ios-so-far/ios-so-far.tex` |
