...
% `float` provides [H] — force figure exactly here, no floating. Used to keep
% per-section card pairs snug against their paragraphs so the document doesn't
% leave huge blank gaps between text and floats in a two-column layout.
\usepackage{float}
\usepackage{ragged2e}
...
{\kidlispbold\fontsize{24pt}{28pt}\selectfont\color{kldark} Kid{\color{klbrand}Lisp} Cards}\par
\includegraphics[width=0.5\textwidth]{figures/cover}\par
\vspace{0.35em}
\vspace{0.2em}
{\kidlispfont\fontsize{11pt}{13pt}\selectfont\color{klbrand} Programs That Fit on a Card}\par
\vspace{0.6em}
...
\kidlisp{} programs are short. A typical program is 2--4 lines of S-expressions that produce a complete visual composition---a starburst, a spiral, a quilt, a fractal. This brevity is not incidental but designed: the language omits user-defined functions, recursion, and abstraction mechanisms, so programs stay flat and legible. We observe that this flatness creates a natural correspondence between programs and physical cards. A \kidlisp{} program, its source code, and its rendered output can coexist on a single printed card. This paper describes the \emph{KidLisp Cards} format---a deck of programs organized by visual category---and a Python rendering pipeline (PIL-based interpreter) that produces card images outside the browser. We discuss what cards are now (a curated notebook of visual koans), what they could become (trading cards, pedagogical sequences, printed scores, postal art), and how the card constraint feeds back into language design.
\end{quote}
\vspace{0.5em}
}]
