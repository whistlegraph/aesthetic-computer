...
test('Jev self-play uses only the external controller and becomes neutral when it waits', () => {
  const saved = globalThis.__oskiewarJevPad;
  try {
    let pressed = ['ArrowRight', 'B', 'INVALID'];
    globalThis.__oskiewarJevPad = () => pressed;
    const { fight, tick } = createFight(false);
    fight.startSelfPlay();
    tick(3000001); tick();
    assert.deepEqual(fight.inputPadDown(0), ['ArrowRight', 'B']);
    assert.deepEqual(fight.inputPadDown(1), ['ArrowRight', 'B']);
    pressed = [];
    for (let n = 0; n < 20; n++) tick();
    assert.deepEqual(fight.inputPadDown(0), []);
    assert.deepEqual(fight.inputPadDown(1), []);
  } finally {
...
test("Open Graph uses a landscape fallback and silent vertical title loop", () => {
  ...
  assert.match(source,
    /\(viewWidth\(\) - 56 - socialLockupGap\) \/\s*\(handleWidth\(title, 1\) \+ handleWidth\(version, \.3\)\)/);
...
test("fighters start three tiles apart astride the station's center", () => {
  ...
  assert.match(source, /const spawnColLeft = 18;/);
  assert.match(source, /const spawnColRight = 21;/);
  // And the bell still rings on flat ground. A fight that opens with both
  // fighters sliding down a transition is a fight nobody chose the start of.
  const { fight: settled } = createFight(false, false, "web");
  for (const player of settled.players)
    assert.equal(settled.terrainFloorAt(player.spawnX), 1800,
      "a spawn mark sits on a ramp");
...
test("the lens holds still while fighters idle at close range", () => {
  ...
  for (let frame = 0; frame < 240; frame++) {
    // Both on the park's flat middle: two fighters idling at different
    // heights is not an idle, it is a camera being asked to frame a slope.
    fight.players[0].x = 1730;
    fight.players[1].x = 1870;
...
test("the park shares one deterministic surface with physics and foot IK", () => {
  // The surface is authored rather than noised now -- a run of named skatepark
  // features instead of two sine terms -- but the property that mattered is
  // unchanged and is the reason this test exists: ONE function answers where
  // the ground is, and physics, rendering, the ball and foot IK all ask it.
  assert.match(source, /function terrainFloorAt\(x\)/);
  assert.match(source, /const parkFeatures = \[/);
  assert.match(source, /kind: "transition"/);
  assert.match(source, /kind: "bank"/);
  ...
  // And it is a real profile: continuous everywhere, with the halfpipe dug in
  // below the floor and the quarterpipes standing above it. A discontinuity
  // here is a fighter teleporting, so it is worth walking the whole park.
  const { fight } = createFight(false, false, "web");
  const stage = fight.stageGeometry();
  let previous = null;
  let widest = 0;
  ...
  for (let x = stage.worldLeft; x <= stage.worldRight; x += 5) {
    const y = fight.terrainFloorAt(x);
    if (previous !== null) widest = Math.max(widest, Math.abs(y - previous));
    previous = y;
    lowest = Math.min(lowest, y);
    deepest = Math.max(deepest, y);
  }
  assert.ok(widest < 200, `the park jumps ${widest.toFixed(0)} units somewhere`);
  assert.ok(lowest < stage.floorY, "the quarterpipes rise above the floor");
  assert.ok(deepest > stage.floorY, "the halfpipe is dug in below it");
});
...
test("waiting-room death cancels actions and respawns without a frozen body", () => {
  const capable = globalThis.__oskiewarVersusCapable;
  globalThis.__oskiewarVersusCapable = true;
  try {
    const { fight, pads, tick, signals } = createFight(false, false);
    fight.enterLobby();
    tick(100000);
    const player = fight.players[0];
    player.attackKind = "PUNCH";
    player.attackUntil = 1000000;
    player.blocking = true;
    player.grabHeld = true;
    player.pounding = true;
    player.itemAction = "FIRE";
    player.itemActionUntil = 1000000;
    fight.killLocal();
    tick();
    assert.equal(player.alive, false);
    ...
    assert.equal(fight.deathCinematicState(), null);
    const spawnX = player.x;
    pads[0].leftX = 1;
    for (let frame = 0; frame < 12; frame++) tick();
    assert.ok(player.x > spawnX, "the respawned body moves with the player");
    fight.paint();
  } finally {
    globalThis.__oskiewarVersusCapable = capable;
  }
...
test("the large Start button keeps its hit area while hovered", () => {
  const { fight, tick } = createFight(false, false, "web", null,
    { width: 1920, height: 1080 });
  const pointer = { x: 0, y: 0, active: false };
  globalThis.__oskiewarTouch = { taps: [], pointer };
  ...
  try {
    ...
    assert.ok(button.width >= 320);
    assert.ok(button.height >= 100);
    assert.deepEqual(globalThis.__oskiewarTouch.titleButton, button);
    pointer.active = false;
    run(60);
...
test("bot uses the player input and physics path to pursue and strike", () => {
  ...
  fight.players[0].x = 1550;
  bot.x = 2050;
  tick();
  assert.ok(bot.vx < 0);
  fight.players[0].x = 1880;
  bot.x = 2000;
  bot.botAttackAt = 0;
  tick();
...
test("a bot spends a held item at range and still swings up close", () => {
  ...
  fight.players[0].x = 1500;
  bot.x = 2100;
  bot.botItemAt = 0;
  tick();
  assert.equal(bot.gunAmmo, 3);
  assert.equal(fight.bullets.length, 1);
  fight.players[0].x = 1750;
  bot.x = 1900;
  bot.botAttackAt = 0;
  tick();
...
test("title +/- steps the game speed without starting the fight", () => {
  const { fight, pads, tick } = createFight(false, false);
  assert.equal(fight.shellState().mode, "MENU");
  assert.equal(fight.gameSpeedState(), 1);
  pads[0].down = ["SpeedUp"]; tick(); pads[0].down = []; tick();
  assert.equal(fight.gameSpeedState(), 1.25);
  assert.equal(fight.shellState().mode, "MENU",
    "a pace key must never read as start");
  // Held keys step once; the edge is against the unfiltered pad.
  pads[0].down = ["SpeedDown"];
  for (let frame = 0; frame < 10; frame++) tick();
  pads[0].down = []; tick();
  assert.equal(fight.gameSpeedState(), 1);
  // The clock floors at a quarter and ceils at double.
  for (let step = 0; step < 6; step++) {
    pads[0].down = ["SpeedDown"]; tick(); pads[0].down = []; tick();
  }
  assert.equal(fight.gameSpeedState(), .25);
  for (let step = 0; step < 10; step++) {
    pads[0].down = ["SpeedUp"]; tick(); pads[0].down = []; tick();
  }
  assert.equal(fight.gameSpeedState(), 2);
  assert.equal(fight.shellState().mode, "MENU");
  // Speed is the monotonic stream itself, scaled at the source.
  assert.match(source, /scaledClockUs \+= \(raw - lastRawClockUs\) \* gameSpeed/);
  // The web shell maps the keyboard's +/- (and their numpad twins).
  assert.match(webShell, /\["Equal", \[0, "SpeedUp"\]\]/);
  assert.match(webShell, /\["Minus", \[0, "SpeedDown"\]\]/);
});

test("the cube deals no sword and serves no ball", () => {
  // Both came and went: the blade lasted one build, the ball left with it.
  // The machinery sleeps — swordHeld still renders and buffs if training
  // ever deals steel again, and enableBall() is the ball's whole
  // re-installation — but a live round starts bare-handed with one pistol.
  const { fight, tick } = createFight(true, true);
  assert.doesNotMatch(source, /swordPickups/);
  assert.match(source, /let ballEnabled = false/);
  for (let frame = 0; frame < 240; frame++) tick();
  assert.equal(fight.ball.active, false);
  for (const player of fight.players) {
    assert.equal(player.swordHeld, false);
    assert.equal(player.grenadeAmmo, 0);
  }
});

test("the result card speaks and the winner laughs with an open mouth", () => {
  const { fight, tick } = createFight();
  fight.players[0].score = 1;
  for (let frame = 0; frame < 750; frame++) tick(40000);
  assert.match(fight.roundState().roundResult, /WINS ROUND/);
  // Ride past the 1.1s result-card sting.
  for (let frame = 0; frame < 40; frame++) tick(40000);
  // The winner's mouth opens on the sting — the same open-ring LAUGH face
  // the A+B chord earns — and the staccato lands 420ms into it.
  assert.equal(fight.players[0].resultReaction, "LAUGH");
  assert.equal(fight.players[1].resultReaction, "");
  // The card's sentence is published for whichever shell can speak it.
  // The harness seat is anonymous — a blank nameplate speaks as "you".
  assert.equal(globalThis.__oskiewarResultLine, "you win the round");
  assert.match(source, /emitSignal\("victory-laugh"/);
  // The web shell reads the line aloud and voices the laugh through the
  // signal bank; the console synthesizes its own five sine steps.
  assert.match(webShell, /SpeechSynthesisUtterance/);
  assert.match(webShell, /__oskiewarResultLine/);
});
...
test("the front door is versus, and the climb keeps its named entrance", () => {
  ...
  try {
    ...
    assert.equal(web.fight.survivalState().active, false);
    // There is no coin here: the same shell returns to the same door every
    // time the title is rebuilt.
    web.fight.backToTitle();
    assert.equal(web.fight.survivalState().active, false);
    // A door asked for by name still wins over the capability flag.
    globalThis.__oskiewarOpponent = "survival";
    const named = createFight(false, false);
    assert.equal(named.fight.survivalState().active, true);
  } finally {
    if (previousOpponent === undefined) delete globalThis.__oskiewarOpponent;
    else globalThis.__oskiewarOpponent = previousOpponent;
    if (previousCapable === undefined) delete globalThis.__oskiewarVersusCapable;
    else globalThis.__oskiewarVersusCapable = previousCapable;
  }
  assert.match(webShell, /oskiewar stick fight and lava climb/);
  // The oven never rolls: it deals its door from the slot's seed and names
  // it in the URL, so a re-render of a day reproduces the same doors.
  assert.match(reelRenderer, /doors = \["survival", "fight"\]/);
  assert.match(reelRenderer, /opponent=\$\{door\}/);
  assert.match(socialRenderer.toString(), /opponent=fight/);
});

test("the lava climb is a solo run with rising lava", () => {
  const previousOpponent = globalThis.__oskiewarOpponent;
  globalThis.__oskiewarOpponent = "survival";
  let fresh;
  try {
  ...
  const { fight, replays, liveFrames, analyticsEvents, signals, tick } = fresh;
  assert.equal(fight.survivalState().active, true);
  assert.equal(fight.shellState().mode, "MENU");
  assert.equal(fight.players[1].npc, true);
  assert.equal(fight.players[1].bot, false);
  assert.equal(fight.players[1].alive, false, "survival seats no adversary");

  const platforms = fight.platformTable();
  assert.equal(platforms.length, 32);
  for (let index = 1; index < platforms.length; index++) {
    assert.equal(platforms[index - 1].y - platforms[index].y, 235);
    const gap = Math.max(0,
      Math.max(platforms[index - 1].left, platforms[index].left) -
      Math.min(platforms[index - 1].right, platforms[index].right));
    assert.ok(gap < 180, `platform ${index + 1} remains jump-reachable`);
  }

  fight.enterGame();
  const lavaStart = fight.survivalState().lavaY;
  for (let frame = 0; frame < 70; frame++) tick();
  assert.ok(signals.some(([event]) => event === "climb"));
  assert.ok(fight.survivalState().lavaY < lavaStart, "lava rises after climb!");
  assert.match(source, /const callout = "climb!"/);

  for (let frame = 0; frame < 520 && !fight.roundState().roundResult; frame++)
    tick();
  assert.equal(fight.roundState().roundResult, "LAVA");
  assert.equal(fight.seriesState(), "", "survival carries no fight series");
  assert.deepEqual(replays, []);
  assert.deepEqual(liveFrames, []);
  assert.deepEqual(analyticsEvents, []);
});

test("the offline oven gets one local survival completion envelope", () => {
  const previousOpponent = globalThis.__oskiewarOpponent;
  globalThis.__oskiewarOpponent = "survival";
  globalThis.__oskiewarCaptureSurvival = true;
  let capture;
  try {
    capture = createFight(false, false);
    capture.fight.startSurvival(true);
    for (let frame = 0; frame < 2400 && !capture.replays.length; frame++)
      capture.tick();
  } finally {
    delete globalThis.__oskiewarCaptureSurvival;
    if (previousOpponent === undefined) delete globalThis.__oskiewarOpponent;
    else globalThis.__oskiewarOpponent = previousOpponent;
  }
...
test("the climb bot plays off the frame: a scene graph every tick, options inside it", () => {
  const previousOpponent = globalThis.__oskiewarOpponent;
  globalThis.__oskiewarOpponent = "survival";
  let capture;
  try {
  ...
  const { fight, tick } = capture;
  fight.startSurvival(true);
  const runner = fight.players[0];
  const ladder = fight.platformTable();
  const apex = 1760 * 1760 / (2 * 4800);
  let lastAt = -1;
  let narrowest = Infinity;
  let waited = 0;
  let jumpsChosen = 0;
  let ticks = 0;
  for (; ticks < 2400 && !fight.roundState().roundResult; ticks++) {
    tick();
    const scene = fight.botSceneState(0);
    if (!scene) continue;
    // Rebuilt this tick, not cached from an earlier one.
    assert.ok(scene.at > lastAt, "the scene is rebuilt every tick");
    lastAt = scene.at;
    // The frame is the screen a person sees: every rung the scene holds
    // projects onto it with the lens the bot read, and no rung off it does.
    const fresh = fight.botSceneNow(0);
    const camera = fight.cameraState();
    const onScreen = (rung) => {
      const a = fight.projectPoint(rung.left, rung.y, 0);
      const b = fight.projectPoint(rung.right, rung.y, 0);
      const y = (a.y + b.y) / 2;
      return y >= 0 && y <= camera.viewHeight &&
        Math.max(a.x, b.x) >= 0 && Math.min(a.x, b.x) <= camera.stageRight;
    };
    assert.deepEqual(fresh.rungs.map((rung) => rung.level),
      ladder.filter(onScreen).map((rung) => rung.level));
    narrowest = Math.min(narrowest, scene.rungs.length);
    // Every option ends on a rung in the frame, and every jump is one the
    // legs can make: above the feet, under the apex.
    for (const option of scene.options) {
      if (option.kind === "walk") continue;
      assert.ok(scene.rungs.some((rung) => rung.level === option.level) ||
        (option.kind === "sink" && option.level === 0),
        `${option.kind} to ${option.level} is in the frame`);
      if (option.kind === "jump") {
        assert.ok(option.dy > 0 && option.dy <= apex);
        assert.ok(option.takeoffLeft <= option.takeoffRight);
      }
    }
    const goal = fight.botSubgoalState(0);
    if (scene.self.footing && !goal) waited++;
    if (goal) {
      assert.equal(goal.kind, "jump");
      jumpsChosen++;
      assert.ok(scene.self.footing === null ||
        scene.options.includes(goal), "the subgoal is one of this frame's options");
    }
  }
  assert.equal(fight.roundState().roundResult, "SUMMIT",
    "the bot summits off the frame alone: " + JSON.stringify(fight.survivalState()));
  assert.ok(narrowest < ladder.length,
    "the frame never shows the whole ladder on a television");
  assert.ok(jumpsChosen > 0);
  // Reported, not asserted: how long the climb took and how many ticks the
  // bot stood waiting for a deck to enter the frame.
  console.log(`  climb: ${ticks} ticks, waited ${waited}, narrowest frame ${narrowest} rungs`);
...
test("the fight bot reads the opponent and the station's decks off the frame", () => {
  ...
  tick();
  tick();
  const scene = fight.botSceneState(1);
...
test("the frame meter records what a fighter was doing, frame by frame", () => {
  ...
  const attacker = fight.players[0];
  const target = fight.players[1];
  target.x = attacker.x + 110;

  assert.equal(fight.frameMeterState(0), "neutral");
  // A swing that has not connected is ACTIVE: `resolveMelee` gates on nothing
  ...
  assert.equal(fight.frameMeterState(0), "recovery");
  // Stun outranks whatever you were trying to do.
  attacker.hitStunUntil = fight.players[0].attackUntil + 500000;
  assert.equal(fight.frameMeterState(0), "stun");

  // And the ring buffer actually fills as the round runs.
  for (let frame = 0; frame < 12; frame++) tick();
  const [first] = fight.frameMeters();
  assert.ok(first.length > 0, "the meter recorded nothing");
  assert.ok(first.every((state) => typeof state === "string"));
});

test("the frame meter is bounded, per-round, and costs nothing when off", () => {
  assert.match(source, /const frameMeterLength = 120/);
  assert.match(source, /if \(meter\.length > frameMeterLength\) meter\.shift\(\)/);
  // Debug-only: nothing is recorded while the overlay is off.
  assert.match(source, /function recordFrameMeter\(now\) \{\n  if \(!debugHitboxes\) return;/);
  // Recorded AFTER the exchange resolves, or a frame that landed a hit would
  // read as the active frame it was instead of the recovery it became.
  const tick = source.match(/resolveMelee\(now\);[\s\S]{0,400}/)[0];
  assert.ok(tick.indexOf("recordFrameMeter") > tick.indexOf("resolvePogoAttacks"),
    "the meter is recorded after the exchange resolves");
  // A bell clears it: carrying frames across a round draws a history that
  // never happened.
  assert.match(source, /for \(const meter of frameMeters\) meter\.length = 0;/);

  const { fight } = createFight();
  fight.setDebugHitboxes(false);
  assert.deepEqual(fight.frameMeters(), [[], []]);
});
...
test("the station parks one skateboard, and it is furniture rather than a ball", () => {
  const { fight } = createFight();
  const boards = fight.balls.filter((item) => item.type === "skateboard");
  ...
  assert.equal(boards[0].y, stage.floorY - 18, "resting on its wheels");
  // And the ride still exists: the mount path and the flag it sets.
  assert.match(source, /ball\.type === "skateboard" && headDistance > ball\.radius/);
...
test("one ball kind survives every round of the same match", () => {
  ...
  fight.nextRound();
  // One ball, still — the skateboard shares the array because it is the same
  ...
  assert.equal(fight.balls.filter((item) => item.type !== "skateboard").length, 1);
  assert.notEqual(fight.seriesBallType(series), "skateboard");
  assert.equal(fight.ball.type, fight.seriesBallType(series));
});
...
test("a console whose clock reads negative can still fight", () => {
  ...
  // The rebase is the accumulator: scaled time starts at zero and counts
  // deltas forward whatever number the host first reports.
  assert.match(source, /let scaledClockUs = 0;/);
  assert.match(source, /if \(lastRawClockUs === null\) lastRawClockUs = raw;/);
});
...
test("self-death replays recent demo frames and returns a playable body", () => {
  ...
  // The death burst is part of the replay: as the playhead crosses the
  // frames after the kill the impact comes back with its debris, then ages
  // out the way it did live instead of being frozen or missing.
  let burstSeen = false, motesMoved = false;
  for (let frame = 0; frame < 600 && fight.instantReplayState().active; frame++) {
    const before = fight.impacts.find((impact) => impact.death)?.debris?.[0];
    const snapshot = before ? { x: before.x, y: before.y } : null;
    tick(33334);
    const burst = fight.impacts.find((impact) => impact.death);
    if (burst && fight.instantReplayState().active) {
      burstSeen = true;
      if (snapshot && burst.debris?.[0] &&
          (burst.debris[0].x !== snapshot.x || burst.debris[0].y !== snapshot.y))
        motesMoved = true;
    }
  }
  assert.equal(burstSeen, true);
  assert.equal(motesMoved, true);
  assert.equal(fight.instantReplayState().active, false);
  assert.equal(fight.players[0].alive, true);
  assert.equal(fight.clientErrorState(), "");
});

test("replay poses blend between neighbouring frames", () => {
  const { fight } = createFight();
  const from = { head: { x: 0, y: 0, z: 0, radius: 10 },
    segments: [{ x1: 0, y1: 0, z1: 0, x2: 10, y2: 0, z2: 0, width: 4, part: "torso", hitboxOnly: false }] };
  const to = { head: { x: 20, y: 10, z: 0, radius: 10 },
    segments: [{ x1: 20, y1: 10, z1: 0, x2: 30, y2: 10, z2: 0, width: 4, part: "torso", hitboxOnly: false }] };
  const mid = fight.blendReplayGeometry(from, to, .5);
  assert.deepEqual(mid.head, { x: 10, y: 5, z: 0, radius: 10 });
  assert.equal(mid.segments[0].x1, 10);
  assert.equal(mid.segments[0].y2, 5);
  assert.equal(mid.segments[0].part, "torso");
  // A body that changed shape (a lost limb) is read off the nearer frame.
  const other = { head: to.head, segments: [] };
  assert.equal(fight.blendReplayGeometry(from, other, .5), from);
  assert.equal(fight.blendReplayGeometry(from, to, 0), from);
});

test("replay collections keep surviving objects for render interpolation", () => {
  const { fight } = createFight();
  const live = [{ id: 1, x: 0, life: 1, debris: [{ x: 0, y: 0 }] },
    { id: 2, x: 5, life: .5, debris: [{ x: 5, y: 5 }] }];
  const first = live[0];
  const mote = first.debris[0];
  const frame = [{ id: 1, x: 3, life: .8, debris: [{ x: 9, y: 9 }] },
    { id: 3, x: 7, life: .3, debris: [{ x: 1, y: 1 }] }];
  fight.syncReplayCollection(live, frame, "id");
  assert.equal(live.length, 2);
  assert.equal(live[0], first);
  assert.equal(live[0].debris[0], mote);
  assert.deepEqual(live[0].debris[0], { x: 9, y: 9 });
  assert.equal(live[0].life, .8);
  assert.equal(live[1].id, 3);
  assert.notEqual(live[1], frame[1]);
  fight.syncReplayCollection(live, [], "id");
  assert.equal(live.length, 0);
});

test("a move caption names its keys and wears its own color", () => {
  const { fight, tap } = createFight();
  tap(0, "ArrowRight");
  tap(0, "ArrowRight");
  assert.equal(fight.players[0].lastButton, "DASH RIGHT");
  assert.equal(fight.players[0].lastButtonKeys, "> >");
  assert.equal(fight.moveKeys("DASH LEFT"), "< <");
  assert.equal(fight.moveKeys("ULTRA AIR"), "^ ^");
  assert.equal(fight.moveKeys("GROUND POUND 2"), "v v");
  assert.equal(fight.moveKeys("KICK", ["A", "ArrowRight"]), "> /");
  assert.equal(fight.moveKeys("KICK", []), "");
  const seen = new Set(["DASH RIGHT", "DASH LEFT", "PUNCH", "KICK", "SHIELD",
    "ULTRA AIR", "GROUND POUND", "GRENADE 2", "ROCKET 1"].map((label) =>
    fight.moveColor(label).join(",")));
  assert.equal(seen.size, 9);
  assert.deepEqual(fight.moveColor("punch"), fight.moveColor("PUNCH"));
  for (const channel of fight.moveColor("PUNCH"))
    assert.ok(channel >= 0 && channel <= 255);
  assert.match(source, /moveColor\(player\.lastButton\)/);
  assert.match(source, /const keys = player\.lastButtonKeys/);
});

test("the near terrain skirt wears the wall material in bands", () => {
  const { fight } = createFight();
  assert.deepEqual(fight.photoRegions.skirt, fight.photoRegions.wall);
  // The TV skirt is tiled four deep; the reel's deep skirt keeps its shade.
  assert.match(source, /terrainPass\(left, right, wallZ, wallZ, wallBottom, \(\) => wall, reel \? 1 : 4\)/);
  assert.match(source, /if \(wall && tileRows > 1 && photoThemeActive &&\s*terrainSkirtTiles\(a1, a, b, b1, tileRows\)\)/);
  // Without the photo atlas the bands decline and the shaded quad still draws.
  const corner = (x, y) => ({ x, y, z: 0 });
  assert.equal(fight.terrainSkirtTiles(corner(0, 0), corner(90, 0),
    corner(90, 720), corner(0, 720), 4), false);
  // Tall structural walls still refuse the stretched strip.
  assert.match(source, /if \(region === "wall"\) return false;/);
});

test("a sideways dash streaks motion lines and boosts a lone head", () => {
  const { fight, tap, tick, now } = createFight();
  tap(0, "ArrowRight");
  tap(0, "ArrowRight");
  assert.equal(fight.players[0].lastButton, "DASH RIGHT");
  assert.ok(fight.players[0].dashLinesUntil > now());
  assert.equal(fight.players[0].dashLinesFacing, 1);
  tick(300000);
  assert.ok(fight.players[0].dashLinesUntil <= now());
  assert.match(source, /drawDashMotion\(renderable\.item, t\)/);
  // Everything but the head gone, the double-tap still moves it: a shove
  // along the floor with a hop, rather than nothing.
  const player = fight.players[0];
  player.removedParts = ["torso", "left-arm", "right-arm", "left-leg", "right-leg"];
  assert.equal(fight.isHeadOnly(player), true);
  tick(40000);
  player.vx = 0;
  tap(0, "ArrowLeft");
  tap(0, "ArrowLeft");
  assert.equal(player.lastButton, "DASH LEFT");
  assert.ok(player.vx < -2000);
  assert.ok(player.dashLinesUntil > now());
  assert.equal(player.dashLinesFacing, -1);
});
...
test("the park keeps its tile size, its depth, and thin walls", () => {
  assert.match(source, /const tileSize = 90/);
  assert.match(source, /let gridCols = 40/);
  assert.match(source, /const gridRows = 16/);
  ...
  // The ramps came back, and they are the map now. @jeffrey asked for them
  // gone in the tower, and what he was refusing was scenery that got in the
  // way of a fight; a skatepark is the opposite proposition, which is why the
  // request this time was for MORE of them.
  assert.match(source, /const parkFeatures = \[/);
  assert.match(source, /let terrainSamples = gridCols \* 6/);
  assert.doesNotMatch(source, /const terrainAmplitude/);
});
...
test("the map is a 40x16 grid of 90-unit tiles and is authored on it", () => {
  const { fight } = createFight(false, false, "web");
  const stage = fight.stageGeometry();
  // Playable faces sit exactly one grid apart on both axes.
  assert.equal(stage.worldRight - stage.worldLeft, 40 * 90 + 2 * 40);
  assert.equal(stage.floorY - stage.ceilingY, 16 * 90 + 40);
  ...
  assert.equal((40 * 90) * (16 * 90) / ((10 * 90) * (10 * 90)), 6.4);
  ...
  assert.ok(rows[0] - rows[rows.length - 1] < apex,
    "the low decks reach the high one");
  assert.ok(stage.floorY - rows[rows.length - 1] > apex,
    `the floor must NOT reach the high deck straight up ` +
    `(${stage.floorY - rows[rows.length - 1]} vs ${apex.toFixed(0)})`);
  ...
  assert.match(source, /const cornerColLeft = 5;/);
  assert.match(source, /const cornerColRight = 34;/);
  // The decks are authored on the lattice too — column, row, width in tiles.
  assert.match(source, /const parkDeckPlan = \[/);
  ...
  assert.match(source, /let gridField = new Float32Array\(gridCols \* gridRows\)/);
...
test("skateboard feet stay on a rigid deck in depth", () => {
  const { fight, tick } = createFight();
  const player = fight.players[0];
  player.x = 1800; player.y = fight.terrainFloorAt(player.x);
  player.skateboard = true; player.skatePitch = .35; player.grounded = true;
  const frame = fight.skateFrame(player);
  const pose = fight.runnerWorldGeometry(player, 0);
  for (const role of ["lead-shin", "rear-shin"]) {
    const bone = pose.segments.find((item) => item.role === role);
    const direction = (role === "lead-shin" ? 1 : -1) * player.facing;
    const side = bone.part === "left-leg" ? -1 : 1;
    const foot = frame(direction * 32, -4, side * 9);
    assert.deepEqual([bone.x2, bone.y2, bone.z2], [foot.x, foot.y, foot.z]);
  }
  assert.ok(new Set(pose.segments.flatMap((b) => [b.z1, b.z2])).size >= 3);
  fight.paint();
});

test("skateboard wheels roll by contact distance and land audibly", () => {
  const { fight, pads, tick, signals, drums } = createFight();
  const player = fight.players[0];
  player.x = 1800; player.y = fight.terrainFloorAt(player.x);
  player.skateboard = true; player.skateVx = 800; player.grounded = true;
  pads[0].down = [];
  for (let i = 0; i < 12; i++) tick();
  assert.ok(player.skateSpin > 0);
  assert.equal(player.skateContacts, 2);
  assert.ok(player.skateVx < 800 && player.skateVx > 0, "rolling friction slows coasting");
  assert.ok(signals.some(([name]) => name === "skate-roll"));
  assert.ok(drums.some(([name]) => name === "hat"));
  player.y -= 30; player.vy = 600; player.grounded = false;
  for (let i = 0; i < 10; i++) tick();
  assert.ok(signals.some(([name]) => name === "skate-land"));
});

test("dismounting restores walking after a downhill ride", () => {
  const { fight, pads, tick, now } = createFight();
  const player = fight.players[0];
  player.x = 1800; player.y = fight.terrainFloorAt(player.x);
  player.skateboard = true; player.skateVx = 900; player.grounded = true;
  for (let i = 0; i < 5; i++) tick();
  assert.equal(fight.dismountSkateboard(player, now()), true);
  fight.disableBall();
  assert.equal(player.skateboard, false);
  assert.equal(player.skateContacts, 0);
  // The descending bank exposed repeated false landings and static crouch legs.
  player.x = 2750; player.y = fight.terrainFloorAt(player.x); player.grounded = true;
  pads[0].down = ["ArrowRight"];
  const feet = [];
  for (let i = 0; i < 25; i++) {
    tick();
    const shin = fight.runnerWorldGeometry(player, 0).segments.find((b) => b.role === "lead-shin");
    if (shin) feet.push(shin.x2 - player.x);
    assert.equal(player.grounded, true, "walking follows the bank");
  }
  assert.ok(feet.length > 20);
  assert.ok(Math.max(...feet) - Math.min(...feet) > 20, "feet keep taking steps");
});

test("terrain profile keeps every original sampled height with fewer faces", () => {
  const { fight } = createFight();
  const { worldLeft, worldRight } = fight.stageGeometry();
  assert.ok(fight.terrainProfile.length < fight.terrainSamples / 2);
  for (let i = 0; i <= fight.terrainSamples; i++) {
    const x = worldLeft + i * (worldRight - worldLeft) / fight.terrainSamples;
    const index = fight.terrainProfile.findIndex((p) => p.x >= x - 1e-8);
    const b = fight.terrainProfile[index], a = fight.terrainProfile[Math.max(0, index - 1)];
...
test("skateboard momentum follows half-pipe tangents into wallrides", () => {
  ...
  // The slope still drives the roll, but it is resolved rather than tuned now.
  // `terrainTangentAt` is a TANGENT, and the old `clamp(slope, -2.5, 2.5) *
  // 1900` needed that clamp because a tangent runs away to infinity as a
  // transition goes vertical. The horizontal share of gravity on a slope is
  // g·sin·cos, which in terms of tan is g·t/(1+t²): it peaks at 45° where it
  // should, falls off toward vertical where it should, and cannot exceed half
  // of g — so there is nothing left to clamp.
  assert.match(source, /const terrainSlope = Math\.tan\(contact\.pitch\);/);
  assert.match(source,
    /fallGravity\(\) \* terrainSlope \/\n      \(1 \+ terrainSlope \* terrainSlope\) \* dt/);
  // And the park it rides: a real profile, with the halfpipe dug in.
  assert.match(source, /const parkFeatures = \[/);
...
test("the game module and its module graph leave with the first byte of the head", () => {
  ...
  assert.doesNotMatch(webShell, /oskiewar-open-room/);
...
test("a ground pound craters on landing and leaves its fighter head-only", () => {
  ...
  // Flat ground with nothing overhead. Both halves of that matter, and the
  // park broke both: this used to say `platformLeft - 1500`, meaning "far
  // left" on a map whose far left was floor, and the park's far left is the
  // face of a quarterpipe -- a pound measured against a ramp measures the
  // ramp. The middle is flat but has a deck over it, which catches the drop
  // early, which is what the original comment was already warning about.
  // The western apron is the one flat span with clear air above it.
  pounder.x = 450;
...
test("render pose reuse ends before simulation and the next paint", () => {
  const { fight, tick } = createFight();
  fight.disableBall();
  const player = fight.players[0];
  player.x = 1800; player.y = fight.terrainFloorAt(player.x);
  player.skateboard = true; player.skatePitch = 0;
  fight.setDebugHitboxes(true);
  assert.ok(fight.countPoseBuilds(() => fight.paint()) <= 2,
    "camera, body and debug overlay share each fighter's pose");
  const before = fight.runnerWorldGeometry(player, 0).head.x;
  player.x += 50;
  assert.equal(fight.runnerWorldGeometry(player, 0).head.x, before + 50,
    "a state change after paint is visible immediately");
  assert.ok(fight.countPoseBuilds(() => fight.paint()) > 0,
    "the next paint rebuilds its poses");
  assert.equal(fight.countPoseBuilds(() => {
    fight.runnerWorldGeometry(player, 0);
    fight.runnerWorldGeometry(player, 0);
  }), 2, "simulation queries never share stale geometry");
  tick();
  assert.equal(fight.clientErrorState(), "");
});

test("undrawn ambient particles cost no simulation work", () => {
  const { fight, tick } = createFight();
...
test("the title omits the coach promotion", () => {
  assert.doesNotMatch(source, /drawCoachLine|add your coach/);
});
...
function createSkatePark() {
  const session = createFight();
  session.fight.configureWorldMap("skatepark");
  session.fight.nextRound();
  session.tick(3000001);
  return session;
}

test("the long skatepark keeps a rider-scale scrolling camera and finite terrain", () => {
  const { fight, tick } = createSkatePark();
  const stage = fight.stageGeometry();
  assert.ok(stage.worldRight - stage.worldLeft > 28000);
  for (let x = stage.worldLeft; x <= stage.worldRight + 100; x += 30)
    assert.ok(Number.isFinite(fight.terrainFloorAt(x)), `terrain at ${x}`);
  const player = fight.players[0];
  assert.equal(player.skateboard, true);
  const width = fight.cameraState().cameraWidth;
  player.x = 19000; player.y = fight.terrainFloorAt(player.x);
  for (let i = 0; i < 90; i++) tick();
  assert.ok(fight.cameraState().cameraCenter > 18000);
  assert.ok(fight.cameraState().cameraWidth < 4000);
  assert.equal(fight.cameraState().cameraWidth, width);
});

test("booster pads catch a fast crossing but leave airborne riders alone", () => {
  const { fight, now } = createSkatePark();
  const player = fight.players[0], pad = fight.skateBoosts[0];
  player.x = pad.right + 20; player.y = fight.terrainFloorAt(player.x);
  player.skateVx = 800; player.grounded = true;
  fight.skateBoostStep(player, pad.left - 20, now());
  assert.equal(player.skateVx, pad.speed);
  player.boostUntil = 0; player.grounded = false; player.skateVx = 800;
  fight.skateBoostStep(player, pad.left - 20, now());
  assert.equal(player.skateVx, 800);
});

test("a fast board rides a complete loop and a slow board drops from the top", () => {
  const { fight, now } = createSkatePark();
  const player = fight.players[0], loop = fight.skateLoops[0];
  const floor = fight.stageGeometry().floorY;
  Object.assign(player, { x: loop.x - 10, y: floor, skateVx: 3900,
    grounded: true, skateLoop: -1 });
  let highest = floor;
  for (let frame = 0; frame < 160; frame++) {
    assert.equal(fight.skateLoopStep(player, { horizontal: 0 }, 1 / 60,
      now() + frame * 16667), true);
    highest = Math.min(highest, player.y);
    if (player.skateLoop < 0) break;
  }
  assert.equal(player.skateLoop, -1);
  assert.ok(highest < floor - loop.radius * 1.9);
  assert.equal(player.grounded, true);
  assert.ok(player.skateVx > 2000);
  Object.assign(player, { skateLoop: 0, loopAngle: Math.PI, loopSpeed: 50 });
  fight.skateLoopStep(player, { horizontal: 0 }, 1 / 60, now());
  assert.equal(player.skateLoop, -1);
  assert.equal(player.grounded, false);
});

test("rope grabs latch, survive button release, climb, swing and detach on a new grab", () => {
  const { fight, pads, tick } = createSkatePark();
  const player = fight.players[0], rope = fight.skateRopes[0];
  const node = rope.nodes[14];
  Object.assign(player, { x: node.x, y: node.y + 125, vx: 0, vy: 0,
    grounded: false });
  pads[0].down = ["A", "B"]; tick();
  assert.equal(player.ropeIndex, 0);
  pads[0].down = []; tick();
  assert.equal(player.ropeIndex, 0, "release keeps the latch");
  const original = player.ropeLink;
  pads[0].down = ["ArrowUp"];
  for (let i = 0; i < 30; i++) tick();
  assert.ok(player.ropeLink < original - 1);
  const climbed = player.ropeLink;
  pads[0].down = ["ArrowDown", "ArrowRight"];
  for (let i = 0; i < 60; i++) tick();
  assert.ok(player.ropeLink > climbed + 1);
  assert.ok(player.x > rope.x + 20);
  for (const chain of fight.skateRopes) for (let i = 1; i < chain.nodes.length; i++) {
    const a = chain.nodes[i - 1], b = chain.nodes[i];
    assert.ok(Math.abs(Math.hypot(b.x - a.x, b.y - a.y) - 44) < 8);
  }
  pads[0].down = ["A", "B"]; tick();
  assert.equal(player.ropeIndex, -1);
  assert.ok(Math.abs(player.vx) > 1, "release keeps swing momentum");
});

test("crossing a rope without grab does not attach; an off-depth grab also misses", () => {
  const { fight, pads, tick } = createSkatePark();
  const player = fight.players[0], node = fight.skateRopes[0].nodes[14];
  Object.assign(player, { x: node.x, y: node.y + 125, grounded: false });
  tick(); assert.equal(player.ropeIndex, -1);
  player.z = 200; pads[0].down = ["A", "B"]; tick();
  assert.equal(player.ropeIndex, -1);
});

test("rollback restores every rope node, velocity, map and rider attachment", () => {
  const { fight, pads, tick } = createSkatePark();
  const player = fight.players[0], node = fight.skateRopes[0].nodes[14];
  Object.assign(player, { x: node.x, y: node.y + 125, grounded: false });
  pads[0].down = ["A", "B"]; tick();
  const saved = fight.netSnapshot(), hash = fight.netStateHash();
  pads[0].down = ["ArrowRight", "ArrowUp"];
  for (let i = 0; i < 30; i++) tick();
  assert.notEqual(fight.netStateHash(), hash);
  fight.netRestore(saved);
  assert.equal(fight.netStateHash(), hash);
  assert.deepEqual(fight.skateRopes, saved.arrays.skateRopes);
  assert.equal(player.ropeIndex, 0);
});

test("holding grab catches a rope crossed later, without reattaching on a release press", () => {
  const { fight } = createSkatePark();
  const player = fight.players[0], node = fight.skateRopes[0].nodes[14];
  Object.assign(player, { x: node.x - 200, y: node.y + 125, vx: 0, vy: 0 });
  assert.equal(fight.skateRopeInput(player, { horizontal: 0, vertical: 0 }, true, 1 / 60, true), false);
  player.x = node.x;
  assert.equal(fight.skateRopeInput(player, { horizontal: 0, vertical: 0 }, true, 1 / 60, false), true);
  fight.skateRopeInput(player, { horizontal: 0, vertical: 0 }, false, 1 / 60, false);
  fight.skateRopeInput(player, { horizontal: 0, vertical: 0 }, true, 1 / 60, true);
  assert.equal(player.ropeIndex, -1);
  fight.skateRopeInput(player, { horizontal: 0, vertical: 0 }, true, 1 / 60, false);
  assert.equal(player.ropeIndex, -1);
});

test("live rope frames fit the relay's existing message budget", () => {
  const { fight, now } = createSkatePark();
  const state = fight.spectatorState(now());
  assert.equal(state.course, "skatepark");
  assert.equal(state.ropes.length, 3);
  assert.equal(state.fighters[0].skateboard, true);
  assert.ok(Buffer.byteLength(JSON.stringify({ type: "oskiewar:state", content: state })) < 8192);
});

test("rollback restores station dimensions after leaving the long course", () => {
  const { fight } = createFight();
  const saved = fight.netSnapshot();
  assert.equal(fight.gridCols, 40);
  fight.configureWorldMap("skatepark");
  fight.resetSkateRopes();
  assert.equal(fight.gridCols, 320);
  fight.netRestore(saved);
  assert.equal(fight.gridCols, 40);
