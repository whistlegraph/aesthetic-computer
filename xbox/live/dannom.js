...
/* #region 🏁 TODO
  - [x] Add a leaderboard / score persistence.
...
let troggles = [];
let level = 1; // current board number, displayed and included in ranking
...
function reset() {
  ...
  score = 0; correctMunches = 0; playerHandle = null; playerColors = null;
  authorizeFn = null; leaderboard = []; leaderboardRank = null;
  leaderboardStatus = ""; lastSubmittedKey = ""; scoreGeneration += 1;
...
function nomBoot({ params, hud, clock, artnom, net, handle, authorize, num: { randInt } }) {
  ...
  loadComicFont();
  loadLeaderboard(scoreGeneration);
  loadPlayerColors(scoreGeneration);
}
...
function gameKey() {
  ...
  return { en: "engnom", es: "mexinom", da: "dannom", ru: "rusnom", cat: "catnom" }[lang] || "engnom";
}

async function readScoreResponse(response) {
  if (!response?.ok) throw new Error(`score api ${response?.status || "failed"}`);
  return response.json();
}

async function loadLeaderboard(generation = scoreGeneration) {
  if (typeof fetch !== "function") return;
  try {
    const data = await readScoreResponse(await fetch(`/api/nom-scores?game=${gameKey()}`));
    if (generation === scoreGeneration)
      leaderboard = Array.isArray(data.scores) ? data.scores : [];
  } catch {
    if (generation === scoreGeneration) leaderboardStatus = "scores offline";
  }
}

async function loadPlayerColors(generation = scoreGeneration) {
  if (!playerHandle || typeof fetch !== "function") return;
  try {
    const bare = playerHandle.replace(/^@/, "");
    const response = await fetch(`/api/handle-colors?handle=${encodeURIComponent(bare)}`);
    if (!response.ok) return;
    const data = await response.json();
    if (generation === scoreGeneration && Array.isArray(data.colors)) playerColors = data.colors;
  } catch {
    // Custom colors are optional; the renderer supplies a lively fallback.
  }
}
...
async function submitRun() {
  ...
  const key = `${score}:${level}:${correctMunches}`;
  ...
  try {
    ...
    const data = await readScoreResponse(await fetch("/api/nom-scores", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      body: JSON.stringify({ game: gameKey(), score, level, correct: correctMunches }),
    }));
    if (generation !== scoreGeneration) return;
    leaderboardRank = data.rank || null;
    leaderboardStatus = data.improved ? `new best · #${data.rank}` : `best · #${data.rank}`;
    await loadLeaderboard(generation);
  } catch {
    if (generation === scoreGeneration) leaderboardStatus = "score not saved";
  }
}
...
function munch() {
  ...
  if (cell.correct) {
    ...
    if (remaining <= 0) {
      score += clearPoints(level, beatsLeft);
      state = "clear";
      clearPhase = 96;
...
function handleColor(colors, index, ch) {
  const color = colors?.[index];
  if (color && [color.r, color.g, color.b].every(Number.isFinite))
    return [color.r, color.g, color.b];
  return charColor(String(ch).toLowerCase());
}

function paintHandle({ ink }, handle, colors, x, y, size = 1) {
  const value = String(handle || "");
  const advance = 6 * size;
  for (let index = 0; index < value.length; index += 1)
    ink(...handleColor(colors, index, value[index])).write(
      value[index], { x: x + index * advance, y, size },
    );
  return value.length * advance;
}
...
let comicFontPromise = null;

function loadComicFont() {
  if (comicFontPromise || typeof FontFace === "undefined" || typeof document === "undefined")
    return comicFontPromise;
  comicFontPromise = new FontFace(
    "Comic Relief",
    "url(https://papers.aesthetic.computer/foundry/fonts/ComicRelief-Regular.ttf)",
  ).load().then((font) => {
...
function hdFont(font, px) {
  return font === "unifont"
    ...
    : `400 ${px * 0.92}px "Comic Relief", "Comic Sans MS", ui-rounded, sans-serif`;
}
...
function paintGame({ wipe, ink, screen, write, box, line, text, paste }) {
  ...
  // Rule title, top-right. Word editions render it in
...
function paintLeaderboard({ ink, screen }, top) {
  ...
  for (const row of rows) {
    ...
    const points = String(row.score || 0);
    const gap = 6 * size;
    ...
    ink(...(mine ? T.introName : T.overlaySub)).write(points, { x, y, size });
    y += lineH;
  }
  const status = leaderboardStatus || (rows.length ? "" : "no ranked runs yet");
  if (status) {
    let statusSize = size;
    while (statusSize > 1 && status.length * 6 * statusSize > screen.width - 8)
      statusSize -= 1;
    const statusW = status.length * 6 * statusSize;
    ink(...T.bg).box(round((screen.width - statusW) / 2) - 5, y - 3, statusW + 10, 8 * statusSize + 6);
    ink(...T.overlaySub).write(status, { center: "x", y, size: statusSize });
  } else if (leaderboardRank && leaderboardRank > rows.length) {
    ink(...T.overlaySub).write(`#${leaderboardRank}`, { center: "x", y, size });
  }
}
