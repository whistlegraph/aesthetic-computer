#!/usr/bin/env python3
"""Measure the Explody Head primary take (7275499036398865706).

What comes out (all into analysis/):
  tempo.json    — onset-derived tempo: global, windowed drift, verdict
  melody.json   — per-word pyin: median f0, note name, cents off
...
SRC = os.path.join(HERE, "..", "source", "xpld-7275499036398865706.wav")
ANA = os.path.join(HERE, "..", "analysis")
RAW = os.path.join(ANA, "whisper-raw", "xpld-7275499036398865706.json")

# The sung piece inside the take: a spoken intro ("Hey come here I want to
# show you a whistlegraph called Explodey Head") runs 0-4.5 s; the song
# starts on "Hit a hump" ~6.0 s and sings through to the end of the take.
SONG = (5.5, 99.5)

# ---------------------------------------------------------------- transcript
def load_words():
    """Merge whisper's -ml 1 sub-word tokens back into words."""
    segs = json.loads(open(RAW, "rb").read().decode(
...
# frame-level tonic probe: voiced pyin frames in the sung region, folded to
# fractional pitch class — for reading where the take's own grid sits.
sm = (times >= SONG[0]) & (times <= SONG[1]) & vflag & (vprob > 0.6)
v = f0[sm]
v = v[np.isfinite(v)]
st = librosa.hz_to_midi(v)
pc_hist, pc_edges = np.histogram(np.mod(st, 12), bins=120, range=(0, 12))
...
top = np.argsort(pc_hist)[::-1][:8]
print("frame pc peaks (midi mod 12, 0.1-st bins):",
      [(round(float(pc_edges[i]), 1), int(pc_hist[i])) for i in top])
for m in melody:
