...
All five tagged Explody Head performances, transcribed word-level
...
KEY = json.load(open(os.path.join(ANA, "key.json")))
SCALE = np.array(KEY["scale_semitones"])
TONIC0 = KEY["tonic_hz"]
SONG = json.load(open(os.path.join(ANA, "transcript.json")))["song_region_sec"]
...
takes = {}
for path in sorted(glob.glob(os.path.join(RAW, "xpld-*.json"))):
    tid = re.search(r"xpld-(\d+)\.json", path).group(1)
    src = os.path.join(LANE, "source", f"xpld-{tid}.wav")
    ...
    # per-take tonic: histogram peak nearest 188.3 within +/-3 st
    v = f0[vf & (vp > 0.6)]
    v = v[np.isfinite(v)]
    tonic = TONIC0
    if len(v) > 200:
...
canon = [p for p in canon if SONG[0] <= p["t0"] <= SONG[1]]
