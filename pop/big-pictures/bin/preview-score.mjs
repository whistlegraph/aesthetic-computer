#!/usr/bin/env node
// big-pictures/bin/preview-score.mjs — storyline visualizer for amaythingra.
//
// Renders the 8 metaverse-session section panels (gen-sections.mjs) into
// a 9:16 Instagram-Story mp4. Forked from marimba/bin/preview-score.mjs
// and built on the shared engine (pop/lib/preview-shared.mjs +
// cover-engine.mjs). The story: jeffrey boots a metaverse game, the grey
// beach renders, he meets the oblivious selfie girls, gets a wakeboard,
// carves the maze, plows them over (the gag), then rides off. Features:
//
//   • THE STRING + SOUND ELEMENTS — a verlet-physics playhead string
//     down the centre; amaythingra's note events (struct.json, the
//     C-engine deep-house bed) ride past it as pitch-coloured blocks
//     across 4 voice lanes (glock / pad / sub / kick). Each note PLUCKS
//     the string as it crosses; the four-on-floor kick drives the punch.
...
//   • FACE ZOOM in + out — the camera eases between a wide framing and a
//     tight crop twice per section (centre fallback, no forms.json yet).
//   • TRANSITIONS — 6 transition types (iris / blinds / push /
...
//
// Usage:
//   node pop/big-pictures/bin/gen-sections.mjs    # → the 8 panels + cover
//   node pop/big-pictures/bin/preview-score.mjs   # → the mp4
...
// ── voice lanes — amaythingra's sound elements ───────────────────────
// 4 voices from struct.events (kick / sub / glock / pad — the C-engine
// deep-house bed). Each lane gets a base colour; individual notes are
// tinted toward their notepat pitch-colour.
const LANES = [
...
// kick onsets drive the "sharpen" punch — the four-on-floor heartbeat.
...
// amaythingra identity — desaturated grey metaverse tints, energy
// shifting with the deep-house arc (boot → load → discover → lull →
// carve → the gag → ride off → lonely calm). All kept cool and muted to
// match the washed-out grey virtual world; jeffrey's burgundy is the
// only warmth in the panels themselves.
const SECTION_TINTS = {
  intro:   "rgba(150,158,176,1)",   // cool slate — boot-in void
  build:   "rgba(166,176,190,1)",   // lighter grey — the world renders
  drop1:   "rgba(176,170,168,1)",   // faint warm grey — discovery
  break:   "rgba(150,166,176,1)",   // cool steel — the lull
  vortex:  "rgba(136,156,178,1)",   // deep steel-blue — the carve
  drop2:   "rgba(186,168,156,1)",   // warmer taupe — the gag peak
  outro:   "rgba(160,168,180,1)",   // soft grey — riding off
  resolve: "rgba(170,180,196,1)",   // pale grey-blue — lonely calm
};
...
// PANELS — TWO visual beats per musical section (an `a` first-half and a
// `b` second-half panel), so the illustration changes ~every 16 s while
// the note-train + tints stay keyed to the 8 musical SECTIONS. Each
// section's [startSec,endSec] is split at its midpoint. Falls back to the
// old single-panel naming (sec-<i>-<name>.png), then the cover.
// Discover however many beat panels exist per section (a, b, c, d, …)
// and split the section's [startSec,endSec] evenly across them, so the
// illustration cadence is set purely by how many panels gen-sections
// produced — no hardcoded count. Falls back to the old single-panel
...
for (let i = 0; i < SECTIONS.length; i++) {
  ...
  const found = PANEL_SUFFIXES.filter((h) =>
    existsSync(`${LANE}/out/${SLUG}-p-sec-${i}-${nm}-${h}.png`));
  const slots = found.length ? found : [null];   // null → single-panel fallback
...
for (let j = 0; j < PANELS.length; j++) {
  ...
  const p    = `${LANE}/out/${SLUG}-p-sec-${P.si}-${safeName(P.name)}${suffix}.png`;
  const pOld = `${LANE}/out/${SLUG}-p-sec-${P.si}-${safeName(P.name)}.png`;
  if (existsSync(p))         { PANEL_IMGS[j] = await loadImage(p);    haveImgs++; }
    ...
    console.error(`  generate panels first: node pop/big-pictures/bin/gen-sections.mjs`);
    process.exit(1);
  }
}
...
// ── PER-PANEL chrome tint (each slide its own colour) ────────────────
...
function varyTint(rgbaStr, beatIdx) {
  ...
  h = (h + beatIdx * 26 + 360) % 360;
  s = Math.min(1, s + 0.12 + beatIdx * 0.04);
...
function drawPalsTitleChars(audioT) {
  ...
  for (const sp of spots) {
    ...
    for (let i = 0; i < titleChars.length; i++) {
      ...
      const y = -dh / 2 + lift;
      // SHIMMER — a glint travels along the letters so the side labels feel alive.
...
function drawTransmittedBacklight(c, sectionImg, xform, faces, intensity, audioT = 0) {
  ...
  // STAINED-GLASS colour = warm base blended with THIS SLIDE's tint.
  ...
  // No forms.json → illuminate the WHOLE panel (glow gated by luminance).
...
// the frame edge (reality warping through a lens). No black bars: centre
// stays the sharp panel; the periphery is a blurred, bulged,
...
function _ewChannel(src, scale, tint) {
...
function renderPanel(c, idx, audioT, env, punch) {
  ...
  const xform = drawCoverKenBurns(c, PANEL_IMGS[idx], audioT, {
    ...KB, env: 0, punch, zoomBox, fillBackground: true,   // env off (no shake), kick punch ON (beat-bump)
  });
...
// Which of the 16 visual PANELS is showing at time t (drives the
// illustration + its transitions; independent of the musical section).
  ...
  for (const L of LANES) {
    ...
    for (const ev of laneEvents[L.key]) {
      ...
      const yC = yCBase - (LANE_BOTTOM - LANE_TOP) / LANES.length / 2
               + subH / 2 + rowIdx * subH;
      // No event-level flash: HIGHLIGHT is per-column, firing only as
      // each column CROSSES the string; incoming bars stay opaque.
      const baseAlpha = 1.0;
...
for (let f = startFrame; f < endFrame; f++) {
  ...
  // Panel changes ~every 16 s (2 per musical section); transition fires
  // at each panel boundary, not just section boundaries.
