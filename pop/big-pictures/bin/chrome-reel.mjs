#!/usr/bin/env node
// chrome-reel.mjs — the amaythingra reel's side-stamp chrome. This is a
// HOMECOMING: momboba/bin/chrome-reel.mjs is itself a port of this lane's
// own drawWatermark + drawPalsTitleChars (preview-score.mjs), by way of
// comodiddies/twofa/story/chrome.mjs. It comes back as the motion-reel
// pass. Two pals stamps hugging the edges (rotated ±90°, looping wiggle +
// swivel, 4-pass multiply/burn/overlay SEEP so they stain INTO the frame,
// LED pulse on the four-on-the-floor kick), and "Amaythingra" as
// per-character climbing columns — each char its own glyph with a
// phase-offset bounce, shimmer glint, and the same seep stack — all tinted
// per-beat. Segmented progress bar flush to the bottom edge + YWFT
// timecode above it.
//
// The per-beat tint IS the story, told in colour: drab office grey →
// Meta blue as the metaverse takes hold → deep blue through the conflict →
// WARM AMBER at the pixsies table (the human antidote) → white-out as the
// world shatters → a warmed grey for the office he returns to, changed.
//
...
//
// Two variants over the same base (--variant, default story): the pals
// stamps + title chars are the identity and stay in BOTH; the reel strips
// the story-flavored chrome (progress bar + timecode) because the Reels UI
// brings its own furniture and tighter tolerances.
//   story → out/amaythingra-story.mp4   (full chrome)
//   reel  → out/amaythingra-reel.mp4    (pals only)
//
// Usage: node pop/big-pictures/bin/chrome-reel.mjs [--variant story|reel]
...
const OUT = `${LANE}/out/${_af("--slug", "amaythingra")}-${VARIANT}.mp4`;
...
const TITLE = "Amaythingra";

// per-beat tints — the story in colour (see the header note). Meta blue is
// #0668E1 (6,104,225); the metaverse beats sit around it, lifted enough to
// read as chrome against the frame.
const BEAT_TINTS = {
  office:  [150, 152, 158],  // drab cubicle grey
  longing: [172, 158, 152],  // the one warm thing in the grey — her
  offer:   [116, 138, 186],  // Meta blue creeping into the office
  portal:  [ 92, 142, 232],  // pale portal light
  spawn:   [ 46, 124, 235],  // META BLUE — the avatar assembling
  lagoon:  [ 30, 112, 230],  // ▶ DROP 2 — Meta blue at full strength
  carve:   [110, 150, 220],  // blue + the grey spray off the board
  grab:    [ 66, 106, 198],  // the conflict, darker
  gag:     [142, 176, 236],  // bright comic spray
  pixsies: [235, 175, 105],  // WARM AMBER — candlelight, the human antidote
  shatter: [196, 210, 246],  // the white-out
  smile:   [196, 172, 158],  // office grey, warmed — he came back changed
};

// ── audio envelope (drives LED pulse + char bounce — this is 120 BPM deep
// house, so the pals stamps light up on the four-on-the-floor kick) ────
...
const CHAR_SCALE = 0.44; // 11 chars — a touch bolder than momboba's 13 and still inside its half
...
function drawWatermark(audioT) {
  ...
  // the LOGO floats on its own rhythm
...
progress.begin({ type: "video", label: `amaythingra ${VARIANT} chrome · ${FRAMES} frames` });
