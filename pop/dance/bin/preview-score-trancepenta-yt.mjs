...
// YT cut: uses the trancepenta-yt-* landscape panel set + 16 slides.
...
const OUT    = flags.out
  || `${LANE}/out/${SLUG}-preview-score-yt.mp4`;
const FPS    = Number(flags.fps ?? 30);
const SIZE   = flags.size || "1920x1080";    // landscape YouTube default
...
// 16-SLIDE override — each struct section split in HALF into two viz
// slides named `<section>-a` / `<section>-b`. Doubles the visual pace
// (the 4th struct section was way too long as a single image). The
// audio-lane events still come from struct.events directly; only the
// IMAGE cycling uses these 16 slides.
const SECTIONS = (() => {
  const out = [];
  const src = struct.sections.slice().sort((a, b) => a.startSec - b.startSec);
  for (const s of src) {
    const mid = (s.startSec + s.endSec) / 2;
    out.push({ name: `${s.name}-a`, startSec: s.startSec, endSec: mid });
    out.push({ name: `${s.name}-b`, startSec: mid,        endSec: s.endSec });
  }
  return out;
})();
...
const LANES = [
  { key: "kick",   color: "#c84a3c" },   // deep terracotta — gabber kick punch
  { key: "sub",    color: "#2a4a72" },   // oceanic deep blue — sub bass
  { key: "lead",   color: "#e8a13c" },   // warm amber — lead (harbour lamp colour)
  { key: "bells",  color: "#a8dde8" },   // pale ice — bells (foam / mist sparkle)
  { key: "piano",  color: "#9bb09a" },   // soft sage — piano (kelp / drift)
];
const laneEvents = {};
...
// KICK onsets drive the "sharpen" punch (the gabber kick is the beat).
...
// 16 tints — PROGRESSIVE DARKENING across the arc (late afternoon to
// dismal night). Each struct section's -a / -b sub-slides each get
// their own colour so the bottom progress bar reads the descent.
const SECTION_TINTS = {
  "intro-a":   "rgba(230,170,110,1)",  // late-afternoon warm amber
  "intro-b":   "rgba(210,150,100,1)",  // dipping toward dusk
  "break1-a":  "rgba(180,150,130,1)",  // dusty pink-grey
  "break1-b":  "rgba(160,140,130,1)",  // cooling further
  "build1-a":  "rgba(140,140,140,1)",  // neutral dusk
  "build1-b":  "rgba(120,130,140,1)",  // dusky teal-grey
  "drop1-a":   "rgba(100,120,140,1)",  // slate
  "drop1-b":   "rgba(90,110,140,1)",   // deepening twilight
  "break2-a":  "rgba(80,90,120,1)",    // bruised violet-blue
  "break2-b":  "rgba(70,80,110,1)",    // night creeping in
  "build2-a":  "rgba(60,70,100,1)",    // deep storm-blue
  "build2-b":  "rgba(50,60,90,1)",     // dark cool
  "drop2-a":   "rgba(40,55,90,1)",     // near-night blue
  "drop2-b":   "rgba(35,50,85,1)",     // night blue
  "outro-a":   "rgba(28,38,70,1)",     // dismal night
  "outro-b":   "rgba(20,28,55,1)",     // fully dim blue-black
};
const TITLE_PALETTE = ["#a6bcd6", "#7fb3c8", "#b0c4a8", "#eec48c", "#d6b4b4"];
const BACKLIGHT_RGB = "255,196,116";       // warm harbour-lamp / cottage-window light
...
const PALS_EDGE_X  = 100;                      // cx of pals (left side) — nudged further from the edge
...
for (let i = 0; i < SECTIONS.length; i++) {
  const p = `${LANE}/out/${SLUG}-yt-sec-${i}-${safeName(SECTIONS[i].name)}.png`;
...
// Iterate over ANY figure keys in the section object (jeffrey + zuck
// for trancepenta, jeffrey + gates for marimba, etc) — not hardcoded.
function figureBoxes(name) {
  const f = FORMS[name];
  if (!f) return [];
  return Object.values(f).map(_toBox).filter(Boolean);
}
function faceBoxes(name) {
  const f = FORMS[name];
  if (!f) return [];
  return Object.values(f).map(_toFace).filter(Boolean);
}
...
for (let f = startFrame; f < endFrame; f++) {
  ...
  // trancepenta reel keeps the segmented progress bar (different from
  // the marimba reel which strips it). Timecode still off in reel.
