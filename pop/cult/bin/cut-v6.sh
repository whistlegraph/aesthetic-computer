#!/usr/bin/env bash
# cut-v6.sh — master the full v6 render once, then cut windows out of it.
#
...
#
# v6's ten acts, for choosing START:
...
#
#   ./pop/cult/bin/cut-v6.sh                       # full + 0:50 + 2:40 cuts
#   START=80 DUR=40 SUFFIX=-secret ./pop/cult/bin/cut-v6.sh
set -euo pipefail
...
FULL="$OUT/cult-remix-v6-full.wav"
SRC="$OUT/cult-remix-v6-full-master.wav"

if [ ! -f "$SRC" ] || [ "$FULL" -nt "$SRC" ]; then
  echo "→ measure full v6"
...
# ── the full record ────────────────────────────────────────────────────
mp3 "$SRC" "$OUT/cult-remix-v6.mp3" \
  "whistlegraph cult --- remix (v6, the descent)" \
...
cut () { # <start> <dur> <suffix> <title> <comment>
  ...
  local WAV="$OUT/cult-remix-v6${SUFFIX}-master.wav"
  local MP3="$OUT/cult-remix-v6${SUFFIX}.mp3"
  local FADE_AT
...
cut "${START:-48.0}" "${DUR:-50.0}" "${SUFFIX:--50s}" \
  "whistlegraph cult --- remix (v6, 50s cut)" \
  "0:48–1:38 — act III, the message, into the turn"

if [ -z "${SUFFIX:-}" ]; then
  cut 48.0 160.0 "-extended" \
    "whistlegraph cult --- remix (v6, 2:40 cut)" \
