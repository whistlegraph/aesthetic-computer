#!/usr/bin/env node
// render2.mjs — "whistlegraph cult --- remix (v2, chill)"
//
...
function shot(name, t, { gain = 1, pan = 0, semis = 0, bus = "drums", side = 0.35, dark = 0, dur = null, dly = 0 } = {}) {
  const s = BANK[name];
...
function hook(bar, { full = true } = {}) {
  ...
  sung("runitfast-sung", t + 4.00 + jit(4), { gain: 0.96 * G, pan: 0.00, side: 0.45, dly: 0.26 });
...
for (let bar = 0; bar < BARS; bar++) {
  ...
  if (bar === S.hollow[0]) sung("threeofus-rise", at(bar), { gain: 0.62, pan: -0.10, side: 0.7, dly: 0.24 });
  if (bar === S.hollow[0] + 4) sung("threeofus-fall", at(bar), { gain: 0.62, pan: 0.10, side: 0.7, dly: 0.24 });
  if (inS(bar, "hollow") && bar % 2 === 0) choir(bar, 0.34);
  if (bar === S.hollow[0] + 2 || bar === S.hollow[0] + 6)
    sung("bassdash-b2", at(bar, 2), { gain: 0.26, pan: 0, side: 0.3, dark: 0.55 });
...
const outWav = resolve(OUT, "cult-remix-v2.wav");
{
...
writeFileSync(resolve(OUT, "cult-remix-v2.events.json"), JSON.stringify({
