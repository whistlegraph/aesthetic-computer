#!/usr/bin/env bash
# render-v2.sh — score the chill cult remix, then master it.
#
# Same mastering law v1 landed on, and for the same reason: at a loud
# target ffmpeg's `loudnorm` silently abandons `linear=true` and starts
# riding gain, which lifts the quiet intro ~20 dB and manufactures
...
#
# v1's outputs are left alone — this writes `-v2` files beside them.
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LANE="$(dirname "$HERE")"
OUT="$LANE/out"
RAW="$OUT/cult-remix-v2.wav"
WAV="$OUT/cult-remix-v2-master.wav"
MP3="$OUT/cult-remix-v2.mp3"

node "$HERE/sing.mjs"          # speech-to-singing bank (cached; free on reruns)
node "$HERE/render2.mjs"

# Chill wants a touch more room than the club cut had, so the target sits
# at the quiet end of @jeffrey's -13/-14 LUFS window.
TARGET_I=-13.9
...
ffmpeg -y -v error -i "$WAV" -c:a libmp3lame -b:a 320k \
  -metadata title="whistlegraph cult --- remix (v2, chill)" \
  -metadata artist="Whistlegraph" \
  -metadata album="pop / cult" \
  -metadata comment="chill techno remix of 'The Three of Us Are in a Cult' (2022) — words sung by WORLD" \
  "$MP3"
