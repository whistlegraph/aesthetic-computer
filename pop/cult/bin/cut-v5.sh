#!/usr/bin/env bash
# cut-v5.sh — master the full v5 render once, then cut windows out of it.
#
...
#
# v5's ten acts, for choosing START:
...
#
#   ./pop/cult/bin/cut-v5.sh                       # full + 0:50 + 2:40 cuts
#   START=80 DUR=40 SUFFIX=-secret ./pop/cult/bin/cut-v5.sh
set -euo pipefail
...
FULL="$OUT/cult-remix-v5-full.wav"
SRC="$OUT/cult-remix-v5-full-master.wav"

if [ ! -f "$SRC" ] || [ "$FULL" -nt "$SRC" ]; then
  echo "→ measure full v5"
...
# ── the full record ────────────────────────────────────────────────────
mp3 "$SRC" "$OUT/cult-remix-v5.mp3" \
  "whistlegraph cult --- remix (v5, the signal)" \
  "Ten acts: a signal goes out, and it is answered, and the joke turns out to be true."

# ── the windows ────────────────────────────────────────────────────────
cut () { # <start> <dur> <suffix> <title> <comment>
  local START=$1 DUR=$2 SUFFIX=$3 FADE=1.2
  local WAV="$OUT/cult-remix-v5${SUFFIX}-master.wav"
  local MP3="$OUT/cult-remix-v5${SUFFIX}.mp3"
  local FADE_AT
...
cut "${START:-48.0}" "${DUR:-50.0}" "${SUFFIX:--50s}" \
  "whistlegraph cult --- remix (v5, 50s cut)" \
  "0:48–1:38 — act III, the message, into the turn"

if [ -z "${SUFFIX:-}" ]; then
  cut 48.0 160.0 "-extended" \
    "whistlegraph cult --- remix (v5, 2:40 cut)" \
    "0:48–3:28 — acts III through VII: the message, the secret, the reply, the spread, the whole message"
fi
