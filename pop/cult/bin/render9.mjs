#!/usr/bin/env node
// render9.mjs — "whistlegraph cult --- remix (v9, the loop and the signal)"
//
...
//
//   node pop/cult/bin/render9.mjs            # → out/cult-remix-v9-full.wav
...
// audible. Written to out/cult-remix-v9.events.json as `events[]`.
...
function chorus(bar, {
  ...
  if (!gone.has(1)) {
    if (fast) {
      ...
      sung("runrealfast-long-lo", t + jit(4), { gain: 0.94 * G, pan: 0.00, side: 0.45, dly: 0.22 });
    } else {
      sungSub("runrealfast-long-hi", t + jit(4), { gain: 0.96 * G, pan: both ? -0.10 : 0.00, side: 0.5, dly: 0.24 }, { amount: 0.32 });
      if (both) sung("runrealfast-long-lo", t + 0.022 + jit(4), { gain: 0.58 * G, pan: 0.16, side: 0.45, dly: 0.20 });
    }
  }

  // ── line 2 · "i wanna hide a — waaaay" ────────────────────────────────
  if (!gone.has(2)) {
    sung(lo ? "iwannahide-lo" : "iwannahide-hi", t + 2.00 + jit(4), { gain: 0.86 * G, pan: -0.14, side: 0.5, dly: 0.20 });
    if (both) sung("iwannahide-lo", t + 2.022 + jit(4), { gain: 0.52 * G, pan: 0.18, side: 0.45, dly: 0.20 });
    // the held vowel the line was written for: tube bus, deep pump
    held(lo ? "away-lo" : "away-hi", t + 3.00 + jit(3), {
      gain: 0.86 * G, pan: -0.06, side: 0.62, dly: 0.26,
      wig: wigDepth(bar) * 0.8, wigHz: 4.9, wigPhase: 1.1, wigDrift: 0.3 * wigDepth(bar), wigIn: 0.55,
    });
    if (both) held("away-lo", t + 3.026 + jit(3), {
  ...
  if (answer === "dots") {
    for (const [k, nm, pan] of [[1.30, "dot-c-b3", 0.38], [1.65, "dot-j-g3", -0.38]])
      sung(nm, t + k + jit(4), { gain: 0.30 * G, pan, side: 0.8, dly: 0.5 });
  } else if (answer === "sos") {
  ...
  // a tag in the last bar: the double-time line as punctuation
  if (tag === "fast")
  ...
  if (choirUnder) choir(bar, 0.30 * G);
}
...
for (let bar = 0; bar < BARS; bar++) {
  ...
  // down. v9: the record COLD-OPENS on the chorus (v3's signature move) —
  // the message is already looping before the channel finishes dialing.
  // One voice, no drums, over the drone: the loop precedes everything.
  if (inS(bar, "carrier")) {
    if (bar === 0) chorus(0, { lead: "hi", g: 0.82 });
    ...
    // the raw chant waits for the statement to finish, then answers it
    if (bar === 5) shot("chant-full", at(5, 1), { gain: 0.44, pan: 0, bus: "vox", side: 0.9, dark: 0.55, dly: 0.30 });
    if (bar === 6) shot("tagline", at(6, 3), { gain: 0.46, pan: 0, bus: "vox", side: 0.7, dark: 0.40, dly: 0.28 });
  }
  ...
  if (sosBar(bar)) {
    // v9: the loop re-enters here, stripped to its bones — lines 1 and 4
    // only, the long dash where "i wanna dash" was (v3's chorus4 take) —
    // and SOS answers it in the back half instead of stating twice.
    if (bar === 16) chorus(bar, { lead: "hi", g: 0.92, drop: [2, 3] });
    if (bar === 20) sos(bar, 1.0);
...
if (process.argv.includes("--stems")) {
  ...
  writeStereo(resolve(dir, "v9-vox.wav"), ...mk(voxL, voxR, voxDuck, VOXG));
  writeStereo(resolve(dir, "v9-tube.wav"), ...mk(tubeL, tubeR, pumpEnv, TUBEG));
  writeStereo(resolve(dir, "v9-music.wav"), ...mk(musicL, musicR, bedEnv, 1));
  writeStereo(resolve(dir, "v9-drums.wav"), ...mk(drumsL, drumsR, null, 1));
  writeStereo(resolve(dir, "v9-signal.wav"), ...mk(sigL, sigR, sigDuck, SIGG));
...
const outWav = resolve(OUT, "cult-remix-v9-full.wav");
...
writeFileSync(resolve(OUT, "cult-remix-v9.events.json"), JSON.stringify({
  schema: "aesthetic.computer/pop-events/v1",
  track: "whistlegraph cult --- remix (v9, the loop and the signal)",
