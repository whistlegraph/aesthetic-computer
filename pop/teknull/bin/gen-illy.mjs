#!/usr/bin/env node
// teknull/bin/gen-illy.mjs — generate the teknull album cover via
// gpt-image-2, then embed it into the rendered mp3 as ID3 art.
//
// Teknull's cover is the technique made visible: jeffrey alone at a
// rack of vintage 32-band graphic equalizers in a dark warehouse,
// pushing one slider up — carving the track out of noise. Red diegetic
// work-lamp light, amber LED freckles, closed Citrus Neo with the
// butterfly scrap on a road case. Same identity-ref path as
// pop/momboba/bin/gen-illy.mjs.
//
// Prompt: pop/teknull/teknull.illy.txt (lowercase fragments — papers voice)
// Output: pop/teknull/out/teknull-cover.png
// Embed:  pop/teknull/out/teknull.mp3  (ID3v2 attached picture)
//
// Usage:
//   node pop/teknull/bin/gen-illy.mjs            # cached if cover exists
//   node pop/teknull/bin/gen-illy.mjs --force    # regenerate
//   node pop/teknull/bin/gen-illy.mjs --embed-only   # skip gen, just embed
...
const PROMPT_PATH = resolve(process.cwd(), _af("--prompt", `${LANE}/teknull.illy.txt`));
const OUT_PATH    = resolve(process.cwd(), _af("--cover",  `${LANE}/out/teknull-cover.png`));
const MP3_PATH    = resolve(process.cwd(), _af("--mp3",    `${LANE}/out/teknull.mp3`));
mkdirSync(`${LANE}/out`, { recursive: true });

// ── identity refs (mirrors momboba/bin/gen-illy.mjs) ─────────────────────
...
console.log(`▸ teknull cover · ${SIZE} · ${REFS.length} refs`);
const t0 = Date.now();
