#!/usr/bin/env node
// gen-illy.mjs — generate the maytrax album cover via gpt-image-2, then
...
// SHOOT + IG-archive refs + the whistlegraph-butterfly scrap), so maytrax
// sits in the same jeffrey singles series. The prompt
// (pop/maytrax/maytrax.illy.txt) drives a summer-jam stoop scene —
...
//
// Output: pop/maytrax/out/maytrax-cover.png (1024x1024)
// Embed:  pop/maytrax/out/maytrax.mp3       (ID3v2 attached picture)
//
//   node pop/maytrax/bin/gen-illy.mjs            # cached if cover exists
//   node pop/maytrax/bin/gen-illy.mjs --force    # regenerate
//   node pop/maytrax/bin/gen-illy.mjs --embed-only   # skip gen, just embed
...
const PROMPT_PATH = resolve(process.cwd(), _af("--prompt", `${LANE}/maytrax.illy.txt`));
const OUT_PATH    = resolve(process.cwd(), _af("--cover",  `${LANE}/out/maytrax-cover.png`));
const MP3_PATH    = resolve(process.cwd(), _af("--mp3",    `${LANE}/out/maytrax.mp3`));
...
console.log(`▸ maytrax cover · ${SIZE} · ${REFS.length} jeffrey refs`);
const t0 = Date.now();
