#!/usr/bin/env python3
"""aesthetivox.py — every flwe vocal goes through the WORLD chain.

The house rule (cult -> loner -> here): no lead vocal ships raw — every
line becomes a SUNG NOTE, not a chopped speech hit. This is the flwe
adaptation of loner's halo3.py v4pid regulation engine, driven by
analysis/melody-chart.json (bin/notes.py's score — the derived 8-degree
scale in the take's own 133.9 Hz / C3+40c frame).
...
Renders:
  ...
  out/flwe-aesthetivox-halo.wav/.mp3  + octave halo (vowels-only, dark,
                                      +/-6c pair) and low self-backup at
                                      -2 and -4 scale degrees, low gain
  vox/NN-slug.wav + vox/.manifest.json   the phrase bank (dry renders,
                                      lead-in noted so beat 0 is placeable)
  analysis/aesthetivox.json           per-word receipts: runway ms,
                                      stretch ratio, holds, cents-to-target
...
TONIC = CHART["tonic_hz"]            # 133.9 — C3 +40c, his own frame
SCALE = np.array(CHART["scale_semitones"])   # [0,1,2,3,7,9,10,11]
BPM = CHART["bpm"]
SPB = 60.0 / BPM
COUNT_IN = 8                          # beats, matches flwe-clickvox
...
def analyze(x, fs):
    ...
    voiced = f0 > 0
    # rate of pitch change — where she is genuinely sliding the snap lets go
    if voiced.sum() >= 2:
...
def build_warp(a, unit_src, beats, dursb, tail_cap_s=0.40, tail_end=None):
    ...
    return np.array(idx, dtype=int), holds, fade, Z, ants, ratios, synth_rel
...
# the click grid (same voice as flwe-clickvox)
def kick():
    n = int(0.065 * FS)
    t = np.arange(n) / FS
...
for pi, ph in enumerate(CHART["phrases"]):
    ...
    tail_end = None
    if pi + 1 < len(CHART["phrases"]):
        next_t0 = CHART["phrases"][pi + 1]["words"][0]["src"][0]
        tail_end = int((next_t0 - t0 - 0.05) / FRAME_S)
    idx, holds, fade, Z, ants, ratios, synth_rel = \
    ...
    manifest[name] = dict(
        phrase=ph["phrase"], beat=ph["beat"], beats_total=ph["beats_total"],
    ...
    receipt["phrases"].append(dict(
        ...
        boundary_moves=blog, trims=tlog, words=wr,
        release_synthesized=manifest[name]["release_synthesized"],
    ))
...
mix *= 0.89 / peak
sf.write(os.path.join(OUT, "flwe-aesthetivox.wav"), mix.astype(np.float32), FS)
hmix = mix + halo_extra * (0.89 / peak)
hp = np.max(np.abs(hmix)) or 1.0
hmix *= 0.89 / hp
sf.write(os.path.join(OUT, "flwe-aesthetivox-halo.wav"),
         hmix.astype(np.float32), FS)
for stem in ("flwe-aesthetivox", "flwe-aesthetivox-halo"):
...
print("wrote out/flwe-aesthetivox.wav+.mp3, out/flwe-aesthetivox-halo.wav+.mp3,"
      f" vox/ ({len(manifest)} phrases), analysis/aesthetivox.json")
