#!/usr/bin/env node
// render2.mjs — "whistlegraph factory --- remix (v2, the pressed voice)"
//
// v2 = v1's arrangement with the voice itself machine-pressed. The rule
// (established on the cult lane): vocals never skip the aesthetivox —
// an exposed unprocessed take is too raw to ship. v1 stamped raw slices
// on principle; v2 keeps the principle and applies it to the voice:
// every vocal in the score is a WORLD resynthesis from bin/aesthetivox.py
// (vox/), pitch-corrected at strength 0.95 with 18 ms smoothing — tight
// enough to staircase, because quantized pitch IS the cookie-cutter
// concept. Full correction strengthens this record instead of sanding it.
//
//   · stamps and lines — snapped to D natural minor on the measured
//     146.83 Hz tonic, +2.4 dB singer's formant. A pressed voice.
//   · the wrong-die words (takes b/c) — same press against the CHROMATIC
//     grid: their pitches stay theirs (cutter-b B2, factory-c C#4); the
//     die is wrong but the stamping is exact.
//   · OUT OF SPEC — the drift is no longer varispeed on raw takes: copy
//     k plays a render whose CORRECTION GRID is detuned ±(k·9) cents
//     (vox/<word>.d<k>.wav) — the aesthetivox targets going out of
//     calibration. Timing slip, die bounces and wobble stay mechanical.
//   · the count-in — loner's treated-speech move: range-compressed
//     (^0.55), down a semitone, darkened. The operator through the
//     intercom, no longer raw speech.
//   · the final bird — WORLD-resynthesized, range-compressed and
//     darkened with its centre LOCKED to G#2. Treated, still outside
//     the factory's key.
//
// Everything below the bank loader is v1's score, unchanged where it
// could be. v1's header follows — the arrangement it describes is still
// the arrangement.
//
// ─────────────────────────────────────────────────────────────────────
// (v1) "whistlegraph factory --- remix (the stamp)"
//
...
//
//   node pop/factory/bin/render2.mjs          # → out/factory-remix-v2-full.wav
//   node pop/factory/bin/render2.mjs --stems  # + per-bus stems
//   bash pop/factory/bin/cut-v2.sh            # master + mp3
...
const SR = 48_000;
// @jeffrey: "should be slower". The chant was measured at ~100; the
// record now runs at 90 and the stamps BREATHE — each 2-bar cell is
// 5.33 s against line spans of 4.2–5.0 s, so every pressing gets real
// air before the next cycle. The words are NOT stretched to the new
// grid: each line is stamped as one die-block at the take's own internal
// word offsets (seconds, measured), because a pressing is one gesture.
const BPM = 90;
const BEAT = 60 / BPM;               // 0.667 s
const BAR = 4 * BEAT;                // 2.667 s
const BARS = 88;                     // 234.7 s = 3:55
const TAU = Math.PI * 2;
...
// The aesthetivox bank OVERRIDES the raw slices name-for-name, and the
// drifted variants (<word>.d<k>) arrive alongside. v2 refuses to run
// without it — falling back to raw takes silently would defeat the rule.
const VOX = resolve(LANE, "vox");
if (!existsSync(VOX)) {
  console.error("vox/ missing — run ../.venv/bin/python bin/aesthetivox.py first");
  process.exit(1);
}
let pressed = 0;
for (const f of readdirSync(VOX))
  if (f.endsWith(".wav")) { load(f.replace(/\.wav$/, ""), join(VOX, f)); pressed++; }
console.log(`  aesthetivox bank: ${pressed} pressed renders (raw slices overridden)`);
...
// ── THE STAMP — each line one die-block at its own internal timing ────
// Word offsets are SECONDS measured from each take's own pressing (the
// whisper word timestamps in harvest.json/survey.json), not beats: a
// pressing is one gesture and stays one. At 90 the cell gives it air.
// In THE STAMP every copy of a given die is identical by construction:
// same slices, same gains, no jitter. The drift parameters exist so OUT
// OF SPEC can spend them; there the die also changes per batch — the
// factory has more than one now.
const LINE1_WORDS = ["factory", "cookie", "cutter", "personalities"];
const LINE1_OFFS = {                       // word starts inside each pressing
  "":   [0, 0.78, 1.45, 2.15],             // a — the original
  "-b": [0, 0.78, 1.72, 2.52],
  "-c": [0, 0.78, 1.45, 2.12],
  "-d": [0, 0.56, 1.31, 2.05],             // the 6.7M bright pressing
  "-e": [0, 1.52, 2.24, 2.96],             // the low slow 2024 pressing
  "-f": [0, 0.61, 2.26, 3.77],             // cookay cubber — runs long, in character
};
const LINE1_G = [1.0, 0.95, 0.97, 1.0];
// v2: `driftK` replaces v1's varispeed `cents` — copy k plays the render
// whose correction grid was itself detuned ±(k·9) cents by aesthetivox.py.
// The detune is baked into the audio; the read head stays honest.
function stampLine1(bar, g = 1, {
  slipMs = 0, driftK = 0, wig = 0, dblAt = -1, dark = 0, take = "",
} = {}) {
  const offs = LINE1_OFFS[take] ?? LINE1_OFFS[""];
  for (let w = 0; w < LINE1_WORDS.length; w++) {
    const name = LINE1_WORDS[w];
    let nm = take && BANK[name + take] ? name + take : name;
    if (driftK && BANK[`${nm}.d${driftK}`]) nm = `${nm}.d${driftK}`;
    const slip = slipMs ? ((w % 2 ? -1 : 1) * slipMs) / 1000 : 0;
    const t = at(bar) + offs[w] + slip;
    word(nm, t, {
      gain: 0.92 * g * LINE1_G[w], dark,
      wig, wigHz: 4.3 + w * 0.7, wigPhase: w * 1.9,
      pan: 0, side: 0.4, evVoice: "stamp",
    });
    if (dblAt === w)                       // a die bounce — the copy struck twice
      word(nm, t + 0.048, { gain: 0.34 * g, semis: -0.18, dark: 0.3,
...
// the PA finds its voice before the operator does — "here's the factory"
// (the 2022 follow-along take, intercom-treated)
word("heres-the-factory", at(1, 2), { gain: 0.72, dark: 0.1, pan: -0.2,
  side: 0.5, dly: 0.3, evVoice: "intercom" });
...
// The die ROTATES between batches now — a, then the bright 6.7M pressing
// (d), then the low 2024 one (e) — but within a batch every copy is
// identical. More dies, same discipline.
const STAMP_DIES = ["", "", "", "-d", "", "-e", "-d", ""];
for (let c = 0; c < 8; c++) {
  const b = 8 + c * 2;
  if (c === 0) continue;                    // one empty cycle: press first, words second
  stampLine1(b, 1.0, { take: STAMP_DIES[c] });
}
...
for (let b = 24; b < 40; b++) {
  ...
  // @jeffrey: "the snares / claps are too much" — one clap, backbeat
  // only, and quieter; the conveyor ticks are the timekeeper.
  if (BANK.clap)
    shot("clap", at(b, 3), { bus: "drums", gain: 0.22, pan: -0.1, side: 0.3, evVoice: "clap" });
...
// Cell one is the original pressing; cell two swaps in the other dies'
// lines — the bright d pressing of line 2, the low e pressing of line 3.
for (const [cell, l2, l3] of [[24, "line2", "line3"], [32, "line2-d", "line3-e"]]) {
  stampLine1(cell, 1.0, { take: cell === 32 ? "-d" : "" });
  word(l2, at(cell + 2), { gain: 1.00, pan: -0.12, side: 0.5, evVoice: "line" });
  word(l3, at(cell + 4), { gain: 1.00, pan: 0.12, side: 0.5, evVoice: "line" });
...
for (let k = 0; k < 8; k++) {
  ...
  stampLine1(b, 1.0 - k * 0.025, {
    slipMs: k * 9, driftK: k, wig: k * 3.2,      // grid detuned ±(k·9)c, baked
    dblAt: k >= 3 ? (k % LINE1_WORDS.length) : -1,
    dark: k * 0.03,
    // wrong dies in the press — b, then the 2026 "cookay cubber" (f),
    // then the bright c
    take: k === 5 ? "-b" : k === 6 ? "-f" : k === 7 ? "-c" : "",
  });
  if (k >= 2)
...
// and the PA justifies itself, right at the turn: "it has a score and it
// can be performed the same way every time" (the 58 s talk, treated)
word("intercom-sameway", at(61, 3), { gain: 0.55, dark: 0.1, pan: 0.25,
  side: 0.6, dly: 0.4, evVoice: "intercom" });
...
for (let b = 64; b < 80; b++) {
  ...
  if (BANK.clap)                            // backbeat only, thinned
    shot("clap", at(b, 3), { bus: "drums", gain: 0.24, pan: -0.1, side: 0.3, evVoice: "clap" });
  if (BANK.hatO && b % 2 === 1)
...
// the last line of the section is the d pressing — the one whose "bird"
// holds for two seconds, ringing over the shutdown's first press
word("line3-d", at(78), { gain: 0.95, pan: -0.1, side: 0.5, evVoice: "line" });
// the 2022 takes keep saying bird — a chain of them, drifting rightward
word("bird-chain-h", at(74, 2), { gain: 0.6, pan: 0.5, dark: 0.1,
  side: 0.7, dly: 0.45, evVoice: "birdword" });
// and the stretched 3.5 s "spinning" from the follow-along take, laid
// under the circling like a second hum
word("spinning-g", at(71), { gain: 0.55, dark: 0.2, pan: -0.55,
  side: 0.8, dly: 0.4, evVoice: "spin" });
...
stampLine1(82, 0.85, { slipMs: 6, driftK: 1 });            // already sagging
...
if (process.argv.includes("--stems")) {
  ...
  writeStereo(resolve(dir, "v2-mach.wav"), ...mk(machL, machR, machDuck, MACHG));
...
const outWav = resolve(OUT, "factory-remix-v2-full.wav");
...
writeFileSync(resolve(OUT, "factory-remix-v2.events.json"), JSON.stringify({
  schema: "aesthetic.computer/pop-events/v1",
  track: "whistlegraph factory --- remix (v2, the pressed voice)",
  renderer: "pop/factory/bin/render2.mjs",
  bpm: BPM,
  key: "D minor",
  narrative: {
