// heartshard.c — the "What's Inside Your Heart" club remix, carved from
// goatshard.c (the blubclub engine). HEART = the Whistlegraph vocal
// (Alex / Camille / Jeffrey singing the round — post-mixed on top from
// samples/whats-inside-your-heart/acapella-150bpm.wav, 136->150 stretch);
// SHARD = the goatshard grammar answering it: twomass goat throat,
// chaosfm bifurcation stabs, frictus glass. Battery is the batch-2
// percussion winners (memkick / implokick / cavikick / wiresnare /
// gransnare / stretched cracklesnare washes).
//
// THE LAW OF THIS TRACK: the bed is arranged around the vocal, not the
// other way — every section door is a vocal event (see ../VOCAL-MAP.md).
// The screams ending vocal bars 11 and 22 ARE the drop triggers; the held
// "heart" melisma (vocal bars 27-31) gets the +5 climax; the only true
// vocal gap (bars 31-32) opens the bridge; the doubled "hey what's up"
// (bars 37-44) is the chant.
//
...
//
// Form: E minor (score written in the Gm goatshard grammar, sounded -3),
// 150 BPM, 240 beats (~1:36) + the vocal rides beats 32-208.
//   intro -> VERSE (vocal round 1) -> roll+scream -> DROP 1 (round 2) ->
//   riser+scream -> DROP 2 (+5 Am, climax + melisma) -> bridge (the gap)
//   -> CHANT ("hey what's up" x2, crunch late) -> outro
//
// Build:  ./build.sh      Render: ./build/heartshard [--out path.wav]
// Full mix: ffmpeg-overlay the 150 BPM acapella at +12.611 s (see ../mix.sh)
...
static const double TRACK_BEATS = 240.0; /* 208 composed + ring-out */

/* section doors, in beats (4-beat bars) — every door is a vocal event.
   The vocal enters at beat 32; vocal bar N starts at beat 32 + (N-1)*4.
   Screams end vocal bars 11 and 22; the melisma holds bars 27-31; the
   one real gap is bars 31-32; the chant doubles bars 37-44. */
#define S_VERSE 32.0   /* vocal bars 1-10: round one, the floor clears  */
#define S_ROLL 72.0    /* vocal bar 11: "...use a hug" -> SCREAM        */
#define S_DROP1 76.0   /* vocal bars 12-22: round two over the wall     */
#define S_RISER 120.0  /* vocal bar 23: empty bar, everything leans in  */
#define S_DROP2 124.0  /* vocal bars 24-31: +5 Am climax + melisma      */
#define S_BRIDGE 156.0 /* vocal bars 32-36: the gap, then C#/F# turn    */
#define S_CHANT 176.0  /* vocal bars 37-44: "hey what's up" x2          */
#define S_OUTRO 208.0
...
static Part ghostP, snrP, clapP, clickP; /* fwd: the unpitched battery */

/* the whole score is WRITTEN in the goatshard G-minor grammar and
   SOUNDED a minor third down — E minor, the key of the WIYH vocal.
   Drums keep their factory tuning. */
static const double KEY_TR = -3.0;

static void emit(Part *p, double beat, double beats, double midi, double vel, double pan) {
  if (p->n >= MAXEV) { fprintf(stderr, "heartshard: part overflow\n"); exit(1); }
  Ev *e = &p->ev[p->n];
  int drum = (p == &kickP || p == &impP || p == &ghostP || p == &snrP ||
              p == &clapP || p == &clickP || p == &washP);
  if (!drum) midi += KEY_TR;
...
static Bus bus_new(int n) {
  ...
  if (!b.L || !b.R) { fprintf(stderr, "heartshard: alloc failed\n"); exit(1); }
  return b;
}
...
static void substrate_score(void) {
  ...
  sub_add(0, 0.0, 2.5, S_VERSE * B - 4.0, 1.5, 700.0, 0, 12.0, 5.0, 0);
  /* verse: a low quiet presence under the singing, mostly felt */
  sub_add(0, S_VERSE * B, 1.5, (S_ROLL - S_VERSE) * B - 2.5, 1.0,
          420.0, 0, 8.0, 4.0, 0);
  /* risers: the whistle glides arrive WITH the screams */
  sub_add(0, (S_DROP1 - 8.0) * B, 0.2, 8.0 * B - 0.7, 0.5,
          500.0, 5200.0, 17.0, 5.0, 0);
  sub_add(0, (S_DROP2 - 8.0) * B, 0.2, 8.0 * B - 0.7, 0.5,
          420.0, 6000.0, 18.0, 5.0, 0);
  sub_add(0, (S_CHANT - 6.0) * B, 0.2, 6.0 * B - 0.7, 0.5,
          460.0, 5600.0, 17.0, 5.0, 0);
  /* drops: low pump bell (B2, the fifth of Em) + velvet air, breathing */
  sub_add(0, S_DROP1 * B, 0.05, (S_RISER - S_DROP1) * B - 0.4, 0.4,
          123.5, 0, 23.0, 1.1, 1);
  sub_add(1, S_DROP1 * B, 0.05, (S_RISER - S_DROP1) * B - 0.4, 0.4,
          8500.0, 0, 6.0, 0.9, 1);
  sub_add(0, S_DROP2 * B, 0.05, (S_BRIDGE - S_DROP2) * B - 0.4, 0.4,
          123.5, 0, 24.0, 1.1, 1);
  sub_add(1, S_DROP2 * B, 0.05, (S_BRIDGE - S_DROP2) * B - 0.4, 0.4,
          9000.0, 0, 7.0, 0.9, 1);
  sub_add(0, S_CHANT * B, 0.05, (S_OUTRO - S_CHANT) * B - 0.4, 0.4,
          123.5, 0, 24.0, 1.1, 1);
  sub_add(1, S_CHANT * B, 0.05, (S_OUTRO - S_CHANT) * B - 0.4, 0.4,
          9000.0, 0, 7.0, 0.9, 1);
  /* bridge breath under the naked voices */
  sub_add(0, S_BRIDGE * B, 1.5, (S_CHANT - S_BRIDGE) * B - 2.5, 1.0,
          480.0, 0, 12.0, 4.0, 0);
...
static Part goatP, stabP, shardP, droneP, robotP, gongP; /* ghostP/snrP/clapP/clickP live up top */
...
/* (goatshard's goat_verse lived here — in the remix the actual
   Whistlegraph vocal sings the verse, so the goat only answers) */
...
static void compose(void) {
  ...
  /* ── VERSE (32-72, vocal bars 1-10): the song walks in and the floor
     clears — light four-on-floor, offbeat bass, and the goat/girl only
     ANSWER in the vocal's rests (positions from ../VOCAL-MAP.md) ────── */
  static const int vroots[4] = { 31, 29, 27, 29 };    /* written Gm F Eb F -> sounds Em D C D */
  for (int b = 0; b < 40; b++) {
    emit(&kickP, S_VERSE + b, 0.25, 36, 0.78, PAN_KICK);
    emit(&clickP, S_VERSE + b + 0.5, 0.1, 60, 0.26, (b % 2 ? 0.35 : -0.35));
    if (b % 2) emit(&ghostP, S_VERSE + b + 0.5, 0.25, 36, 0.25,
                    (b % 4 == 1 ? 0.25 : -0.25));
  }
  for (int k = 0; k < 10; k++) {                      /* light offbeat bass */
    if (k == 4 || k == 5) continue; /* bass truce: "pumping blood" sits at D3-E2 */
    double bb = S_VERSE + 4.0 * k;
    emit(&bassP, bb + 0.5, 0.38, vroots[k % 4], 0.55, PAN_BASS);
    emit(&bassP, bb + 2.5, 0.38, vroots[k % 4], 0.52, PAN_BASS);
  }
  emit(&goatP, S_VERSE + 13.8, 1.6, 55, 0.55, PAN_GOAT); /* after "...left behind" */
  bleat(S_VERSE + 24.3, 55, 2, 0.56);                    /* after "...worms inside" */
  emit(&robotP, S_VERSE + 30.0, 0.8, 70, 0.50, PAN_ROBOT); /* echoes "what's up" */
  emit(&robotP, S_VERSE + 34.9, 0.6, 72, 0.52, PAN_ROBOT); /* echoes "the love" */
  emit(&droneP, S_VERSE, 7.5, 43, 0.22, -0.5);        /* quiet mid pads */
  emit(&droneP, S_VERSE + 8.0, 7.5, 41, 0.22, 0.5);
  emit(&droneP, S_VERSE + 16.0, 7.5, 43, 0.22, -0.5);
  emit(&droneP, S_VERSE + 24.0, 7.5, 41, 0.22, 0.5);
  emit(&droneP, S_VERSE + 32.0, 7.5, 43, 0.22, -0.5);

  /* ── ROLL (72-76, vocal bar 11): "looks like you could use a hug" —
     one gathering bar; the SCREAM at the bar line is the door ───────── */
  emit(&kickP, S_ROLL + 0.0, 0.25, 36, 0.84, PAN_KICK);
  emit(&kickP, S_ROLL + 1.0, 0.25, 36, 0.86, PAN_KICK);
  for (int s = 0; s < 16; s++) {                      /* 16th roll into the scream */
    double t = (double)s / 15.0;
    emit(&snrP, S_ROLL + 0.25 * s, 0.18, 50, 0.30 + 0.60 * t, -0.4 + 0.8 * t);
  }
  bleat(S_ROLL + 3.4, 55, 2, 0.93);                   /* the growl joins the scream */
  emit(&washP, S_DROP1 - 2.0, 4.0, 50, 0.85, -0.5);

  /* ── DROP 1 (76-120, 11 bars, vocal bars 12-22): the wall arrives but
     the SUNG round is the hook — no robot hook here, the goat only
     answers where the vocal converses ──────────────────────────────── */
  emit(&impP, S_DROP1, 0.25, 36, 1.0, PAN_KICK);      /* door slam */
  for (int k = 0; k < 11; k++) {
    double b0 = S_DROP1 + 4.0 * k;
    drop_bar(b0, k, 0, 0);
    if (k % 4 == 0) emit(&gongP, b0, 4.0, 43, 0.85, (k ? 0.3 : -0.3));
    if (k == 2 || k == 6) bell_run(b0, 0);
    if (k == 8) goat_riff2(b0, 0);   /* the climb under "where's the love" */
    if (k == 10) bleat(b0 + 1.4, 58, 2, 0.90); /* growl under "use a hug" */
  }
  emit(&washP, S_RISER - 2.0, 4.0, 50, 0.70, 0.5);

  /* ── RISER (120-124, vocal bar 23): scream #2 rang at the bar line —
     one near-empty bar, everything leans into the lift ─────────────── */
  emit(&kickP, S_RISER + 0.0, 0.25, 36, 0.86, PAN_KICK);
  emit(&kickP, S_RISER + 1.0, 0.25, 36, 0.90, PAN_KICK);
  for (int s = 0; s < 16; s++) {
    double t = (double)s / 15.0;
    emit(&snrP, S_RISER + 0.25 * s, 0.16, 50, 0.36 + 0.58 * t, -0.5 + t);
  }
  bleat(S_RISER + 3.4, 60, 3, 0.96);
  emit(&washP, S_DROP2 - 2.0, 4.0, 50, 0.90, 0.5);

  /* ── DROP 2 (124-156, 8 bars, +5 Am, vocal bars 24-31): the climax
     pass — G#5s over implokick chaos; the harmony choir slides UNDER
     the held "heart" melisma ───────────────────────────────────────── */
  emit(&impP, S_DROP2, 0.25, 38, 1.0, PAN_KICK);
  for (int k = 0; k < 8; k++) {
    double b0 = S_DROP2 + 4.0 * k;
    drop_bar(b0, k, 5, 1);
    if (k % 4 == 0) emit(&gongP, b0, 4.0, 48, 0.88, (k ? 0.3 : -0.3));
    if (k == 2) bell_run(b0, 5);
    if (k == 4) robot_hook(b0, 1, 2); /* the choir enters as "heart" holds */
    if (k == 6) goat_riff2(b0, 5);    /* countermelody under the long note */
  }
  emit(&washP, S_BRIDGE - 2.0, 4.0, 55, 0.75, 0.5);

  /* ── BRIDGE (156-176, vocal bars 32-36): the drums leave. Bar 32 is
     the vocal's only true silence; then "pumping blood" returns shifted
     onto C#/F# and the drones walk E -> F# underneath it ────────────── */
  emit(&droneP, S_BRIDGE, 11.0, 43, 0.36, -0.5);      /* E pedal (written G) */
  emit(&droneP, S_BRIDGE + 0.4, 10.0, 50, 0.28, 0.5); /* + the fifth */
  emit(&droneP, S_BRIDGE + 12.0, 7.5, 45, 0.34, -0.4);/* the F# turn (written A) */
  emit(&gongP, S_BRIDGE + 0.0, 4.0, 55, 0.55, -0.2);
  emit(&gongP, S_BRIDGE + 8.0, 4.0, 59, 0.50, 0.2);
  emit(&gongP, S_BRIDGE + 14.0, 4.0, 57, 0.58, 0.0);
  emit(&goatP, S_BRIDGE + 1.5, 1.8, 55, 0.55, PAN_GOAT);  /* he sighs in the gap */
  emit(&goatP, S_BRIDGE + 10.5, 1.4, 57, 0.54, PAN_GOAT); /* leans on the F# */
  emit(&robotP, S_BRIDGE + 18.6, 1.8, 74, 0.62, PAN_ROBOT); /* she calls the chant in */
  for (int s = 0; s < 12; s++) {                      /* soft 8th roll into the chant */
    double t = (double)s / 11.0;
    emit(&snrP, S_BRIDGE + 14.0 + 0.5 * s, 0.18, 50, 0.20 + 0.40 * t, -0.3 + 0.6 * t);
  }
  emit(&washP, S_CHANT - 2.0, 4.0, 50, 0.85, -0.5);

  /* ── CHANT (176-208, 8 bars, vocal bars 37-44): "hey what's up" twice
     through — the crowd section: claps on, the choir joins the second
     pass, crunch zone late ─────────────────────────────────────────── */
  emit(&impP, S_CHANT, 0.25, 36, 1.0, PAN_KICK);
  for (int k = 0; k < 8; k++) {
    double b0 = S_CHANT + 4.0 * k;
    drop_bar(b0, k, 0, 1);
    if (k % 4 == 0) emit(&gongP, b0, 4.0, 43, 0.85, (k ? 0.3 : -0.3));
    if (k == 2) goat_riff2(b0, 0);
    if (k == 4) robot_hook(b0, 0, 2); /* the choir on the second "hey" */
    if (k == 6) bell_run(b0, 0);
  }
  emit(&washP, S_OUTRO - 2.0, 4.0, 55, 0.75, 0.5);

  /* ── OUTRO (208-240): the room empties, the gong has the last word ─ */
  ...
  emit(&droneP, S_OUTRO + 0.4, 14.0, 31, 0.32, 0.3);  /* E1 floor */
...
static double width_of(double beats) {
  if (beats < S_VERSE)  return 0.35;
  if (beats < S_ROLL)   return 0.55;
  if (beats < S_DROP1)  return 0.55 + 0.40 * (beats - S_ROLL) / (S_DROP1 - S_ROLL);
  if (beats < S_RISER)  return 1.0;
  if (beats < S_DROP2)  return 0.80;
  if (beats < S_BRIDGE) return 1.0;
  if (beats < S_CHANT)  return 0.48;                 /* the corridor */
...
/* tape print amount: full print (lo-fi) in the intro, easing through
   the verse, LIFTED at the drops (the room turns real) */
static double tape_of(double beats) {
  if (beats < S_VERSE)  return 1.0;
  if (beats < S_ROLL)   return 0.55;
  if (beats < S_DROP1)  return 0.65;
  if (beats < S_RISER)  return 0.28;
  if (beats < S_DROP2)  return 0.75;
  if (beats < S_BRIDGE) return 0.22;
  if (beats < S_CHANT)  return 0.55;
  if (beats < S_OUTRO)  return 0.30;
  return 0.85;
}
...
static int write_wav(const char *path, const float *L, const float *R, int n) {
  ...
  if (!f) { fprintf(stderr, "heartshard: cannot open %s\n", path); return 1; }
...
int main(int argc, char **argv) {
  const char *outPath = "../out/heartshard.wav";
  ...
  fprintf(stderr, "heartshard: kick %d / implo %d / ghost %d / snare %d / clap %d / click %d / wash %d\n",
          kickP.n, impP.n, ghostP.n, snrP.n, clapP.n, clickP.n, washP.n);
  fprintf(stderr, "heartshard: goat %d / robot %d / bass %d / stabs %d / gongs %d / shards %d / drones %d\n",
  ...
  /* ── channel strips: each family renders alone onto the strip, gets
     ...
     not hoped for ──────────────────────────────────────────────────── */
  fprintf(stderr, "heartshard: strips\n");
  ...
  fprintf(stderr, "heartshard: substrate (%d carved bands)\n", nSub);
  substrate(&master, duck);

  /* ── story pass: width -> crunch zone + self-scratch -> tape print ─ */
  fprintf(stderr, "heartshard: width / crunch / tape print\n");
  ...
  /* crunch zone: late in the chant, bars 6-7 */
  const double ZC = 202.0 * BEAT, ZW = 2.6;
  #define ZMASK 16383
  ...
  static const struct { double t, dur, freq, depth; } SCR[3] = {
    { 204.5 * BEAT, 0.35, 3.0, 0.15 },
    { 206.75 * BEAT, 0.30, 4.2, 0.11 },
    { 228.0 * BEAT, 0.50, 2.0, 0.22 },
  };
  ...
  /* ── MASTER, first pass: rumble filter -> smile EQ -> bus compressor
     -> tanh glue -> -0.5 dBFS ──────────────────────────────────────── */
  fprintf(stderr, "heartshard: master\n");
  ...
  fprintf(stderr, "heartshard: wrote %s (%.1fs, pre-norm peak %.3f)\n",
