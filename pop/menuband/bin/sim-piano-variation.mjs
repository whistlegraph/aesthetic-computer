...
function drawNoteHighway(t, activeRig, stripRect) {
  ...
  for (const arrival of arrivals) {
    ...
    // Approaching notes are rounded blocks. Once a note reaches the keyboard,
    // square it off so the remaining band reads as a column being consumed by
    // the key, not a separate rounded cap sitting on top of it.
    const radius = until <= 0
      ? 0
      : Math.min(width / 2, positive(highway.radiusPx, 9));
...
function drawPaperLoopScore(t, activeRig, stripRect) {
  ...
  for (let index = 0; index < slices; index++) {
    ...
    entries.push({
      ...
      draw() {
        ...
        ctx.fillStyle = back > 0.52
          ? `rgba(45,37,55,${0.16 + (1 - back) * 0.18})`
          : `rgba(255,251,241,${0.82 + (1 - back) * 0.16})`;
  ...
  for (const note of displayNotes) {
    ...
    for (let s0 = from; s0 < to - 1e-6; s0 += step) {
      ...
      entries.push({
        depth: average(corners.map((point) => point.depth)), layer: 1,
        draw() {
          ctx.save();
          ctx.filter = `blur(${(1 - front) * positive(highway.loopBackBlurPx, 4.5)}px)`;
          ctx.fillStyle = rgb(printed, 0.32 + front * 0.66);
          projectedQuad(corners);
          ctx.fill();
    ...
    if (head >= 0 && head <= loopLength) {
      ...
      if (s1 > s0 + 0.04) {
        ...
        if (corners.every((point) => point.ok)) {
          ...
          const label = KEY_LABELS.get(note.visualMidi);
          entries.push({
            depth: average(corners.map((point) => point.depth)) - 0.002, layer: 2,
