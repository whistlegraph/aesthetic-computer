#!/usr/bin/env node
// cornerfive/bin/gen-illy.mjs — generate the cornerfive album cover via
// gpt-image-2, then embed it into the rendered mp3 as ID3 art.
//
// cornerfive is the whistlegraph "five in the corner x2" remix — kick +
// sine-bell sister line + the whistle. The cover draws that idea: a
// whistle made visible as a luminous sine-line sinking the orange 5-ball
// in the corner pocket, twice. Same identity-ref path as the other /pop
// gen-illy scripts (momboba, marimba).
//
// Prompt: pop/cornerfive/cornerfive.illy.txt (lowercase fragments — papers voice)
// Output: pop/cornerfive/out/cornerfive-cover.png
// Embed:  pop/cornerfive/out/cornerfive.mp3  (ID3v2 attached picture)
//
// Usage:
//   node pop/cornerfive/bin/gen-illy.mjs            # cached if cover exists
//   node pop/cornerfive/bin/gen-illy.mjs --force    # regenerate
//   node pop/cornerfive/bin/gen-illy.mjs --embed-only   # skip gen, just embed
...
const PROMPT_PATH = resolve(process.cwd(), _af("--prompt", `${LANE}/cornerfive.illy.txt`));
const OUT_PATH    = resolve(process.cwd(), _af("--cover",  `${LANE}/out/cornerfive-cover.png`));
const MP3_PATH    = resolve(process.cwd(), _af("--mp3",    `${LANE}/out/cornerfive.mp3`));
...
console.log(`▸ cornerfive cover · ${SIZE} · ${REFS.length} refs`);
const t0 = Date.now();
