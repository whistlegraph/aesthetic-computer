#!/usr/bin/env bash
# cut-v2.sh — master the loner v2 render and cut the mp3.
#
...
FULL="$OUT/loner-remix-v2-full.wav"
SRC="$OUT/loner-remix-v2-master.wav"
...
ffmpeg -y -v error -i "$SRC" -c:a libmp3lame -b:a 320k \
  -metadata title="whistlegraph loner --- remix (v2, aesthetivox)" \
  -metadata artist="Whistlegraph" -metadata album="pop / loner" \
  -metadata comment="v1 through the aesthetivox: every vocal a WORLD render, rubato kept, pitch gently pulled to the lane's 237 Hz grid; the spoken asides survive only as treated material." \
  "$OUT/loner-remix-v2.mp3"
echo "✓ $OUT/loner-remix-v2.mp3"
