#!/usr/bin/env node
// render.mjs — "whistlegraph loner --- remix (v1, bedroom ballad)"
//
...
// with a band-limited antisymmetric side return. Mastering (bin/cut-v1.sh)
// targets −16 LUFS, not −14: a ballad keeps its dynamics.
//
//   node pop/loner/bin/render.mjs            # → out/loner-remix-v1-full.wav
//   node pop/loner/bin/render.mjs --stems    # + per-bus stems
...
const outWav = resolve(OUT, "loner-remix-v1-full.wav");
...
writeFileSync(resolve(OUT, "loner-remix-v1.events.json"), JSON.stringify({
  schema: "aesthetic.computer/pop-events/v1",
  track: "whistlegraph loner --- remix (v1, bedroom ballad)",
  renderer: "pop/loner/bin/render.mjs",
  source: {
    ...
    tuning: "A# minor at the take's own pitch — tonic 237 Hz, ~+30 cents over A440",
  },
...
console.log(`✓ ${outWav}`);
console.log(`  ${(N / SR).toFixed(1)} s scored · master with: bash pop/loner/bin/cut-v1.sh`);
