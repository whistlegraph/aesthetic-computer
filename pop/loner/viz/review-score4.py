...
OUT = "/Users/jas/aesthetic-computer/pop/loner/out/lonerclub-v4pid-review-score.mp4"
...
for li in range(NLANE):
    ...
    lane_y.append(y0)
    sd.rectangle([x0m, y0, x1m, y0 + LANE_H - 1], fill=(17, 16, 20))
...
for li, (name, sig, col) in enumerate(LANES):
    ...
    if e.max() > 0:
        e = (e / e.max()) ** 0.8
    fill = tuple(int(v * 0.22) for v in col)
    line = tuple(int(v * 0.78) for v in col)
...
cd.text((40, 18), "lonerclub (v4pid)", font=f_title, fill=(240, 238, 232))
...
for t0, t1, name, col in SECTIONS:
    ...
    cd.rectangle([xa, MM_Y0, xb, MM_Y1], fill=tuple(int(v * 0.40) for v in col))
    cd.text((xa + 6, MM_Y0 + 5), name, font=f_tiny, fill=col)
cd.rectangle([MM_X0, MM_Y0, MM_X1, MM_Y1], outline=(58, 58, 64))

# gutter: lane row stubs + labels
for li, (name, sig, col) in enumerate(LANES):
    y0 = LANES_TOP + li * (LANE_H + LANE_GAP)
    cd.rectangle([10, y0, GUT - 6, y0 + LANE_H - 1], fill=(17, 16, 20))
    cd.text((18, y0 + LANE_H // 2), name, font=f_lbl, fill=col, anchor="lm")
cd.line([GUT - 2, STRIP_TOP, GUT - 2, STRIP_BOT], fill=(44, 44, 50))
...
for f in range(NF):
    ...
    # ACTIVE CLIPS light up while they play
    active_rects = []
    for li in range(NLANE):
        for (t0, t1) in lane_clips[li]:
            if t0 <= t < t1:
                ...
                if xb > xa:
                    ...
                    frame[y0f:y0f + LANE_H, xa:xb] = np.clip(reg.astype(np.uint16) * 16 // 10, 0, 255).astype(np.uint8)
                    active_rects.append((xa, y0f, xb, li))
                break
    # playhead + minimap position
    frame[STRIP_TOP:STRIP_BOT, PLAY_X - 1:PLAY_X + 1] = (255, 245, 230)
    mx = MM_X0 + int(t / DUR * (MM_X1 - MM_X0))
    frame[MM_Y0:MM_Y1 + 1, mx:mx + 2] = (245, 243, 238)
    ...
    for (xa, y0f, xb, li) in active_rects:
        dact.rectangle([xa, y0f + 1, xb - 1, y0f + LANE_H - 2],
                       outline=LANES[li][2], width=3)
    wn = word_now(t)
    if wn is not None:
        dd2 = ImageDraw.Draw(img)
        dd2.text((60, CY + 66), wn[0], font=f_kara, fill=(240, 238, 232), anchor="lm")
    if wn is not None:
        dd2.text((64, CY + 170), wn[1], font=f_title, fill=(150, 148, 145), anchor="lm")
    ImageDraw.Draw(img).text((W - 40, 14),
        f"{int(t) // 60}:{int(t) % 60:02d}.{int((t * 10) % 10)}",
        font=f_tc, fill=(240, 238, 232), anchor="ra")
