...
// rosewood: C5 → A4 → F4 → (held) → A4 → G4 → F4 (held)
events.push(v("rosewood", 0, 0, N.C5, 1.0, 0.55));
events.push(v("rosewood", 0, 1, N.A4, 1.0, 0.55));
events.push(v("rosewood", 0, 2, N.F4, 1.0, 0.55));
events.push(v("rosewood", 1, 0, N.F4, 3.0, 0.45));
events.push(v("rosewood", 2, 0, N.A4, 1.5, 0.55));
events.push(v("rosewood", 2, 1.5, N.G4, 1.5, 0.55));
events.push(v("rosewood", 3, 0, N.F4, 3.0, 0.55));

// bass: gentle low rocker on beat 1 every bar
for (let b = 0; b < 4; b++) {
  events.push(v("bass", b, 0, N.F2, 3.0, 0.55));
}

// ── [twinkle] — bars 4..9 — climbing falling waves ───────────────────
events.push(v("rosewood", 4, 0, N.F5, 1.0, 0.60));
events.push(v("rosewood", 4, 1, N.A5, 1.0, 0.60));
events.push(v("rosewood", 4, 2.0, N.C6, 0.5, 0.65));
events.push(v("rosewood", 4, 2.5, N.A5, 0.5, 0.55));
events.push(v("rosewood", 5, 0, N.G5, 3.0, 0.55));
events.push(v("rosewood", 6, 0, N.F5, 1.0, 0.55));
events.push(v("rosewood", 6, 1, N.G5, 1.0, 0.55));
events.push(v("rosewood", 6, 2.0, N.A5, 0.5, 0.60));
events.push(v("rosewood", 6, 2.5, N.G5, 0.5, 0.50));
events.push(v("rosewood", 7, 0, N.F5, 3.0, 0.55));
events.push(v("rosewood", 8, 0, N.Bb5, 1.0, 0.60));
events.push(v("rosewood", 8, 1, N.D6, 1.0, 0.65));
events.push(v("rosewood", 8, 2, N.Bb5, 1.0, 0.55));
events.push(v("rosewood", 9, 0, N.C6, 1.5, 0.60));
events.push(v("rosewood", 9, 1.5, N.A5, 1.5, 0.55));

// bass pulse — F-C alternating roots, gives the rocking
const twinkleBass = [N.F2, N.F3, N.F2, N.F3, N.F2, N.F3];
for (let i = 0; i < 6; i++) {
  events.push(v("bass", 4 + i, 0, twinkleBass[i], 3.0, 0.45));
}

// vibraphone_off pad — F major sustained chord over bars 4-6, Bb over 7-9
events.push({ preset: "vibraphone_off", startSec: 4 * BAR, midi: N.F4, durSec: 3 * BAR, gain: 0.18 });
events.push({ preset: "vibraphone_off", startSec: 4 * BAR, midi: N.A4, durSec: 3 * BAR, gain: 0.18 });
events.push({ preset: "vibraphone_off", startSec: 4 * BAR, midi: N.C5, durSec: 3 * BAR, gain: 0.18 });
events.push({ preset: "vibraphone_off", startSec: 7 * BAR, midi: N.Bb4, durSec: 3 * BAR, gain: 0.18 });
events.push({ preset: "vibraphone_off", startSec: 7 * BAR, midi: N.D5, durSec: 3 * BAR, gain: 0.18 });
events.push({ preset: "vibraphone_off", startSec: 7 * BAR, midi: N.F5, durSec: 3 * BAR, gain: 0.18 });

// ── [wow wow wow] — bars 10..13 — held wobble (vibraphone takes over) ─
// vibraphone WITH motor for the "wow" wobble feel
events.push({ preset: "vibraphone", startSec: 10 * BAR, midi: N.G5, durSec: 2 * BAR, gain: 0.45 });
events.push({ preset: "vibraphone", startSec: 10 * BAR, midi: N.Bb5, durSec: 2 * BAR, gain: 0.40 });
events.push(v("rosewood", 12, 0, N.A5, 1.0, 0.55));
events.push(v("rosewood", 12, 1, N.G5, 1.0, 0.55));
events.push(v("rosewood", 12, 2, N.A5, 1.0, 0.55));
events.push(v("rosewood", 13, 0, N.F5, 3.0, 0.55));

// kalimba sparkles overlaid on the wow — soft, off-beat ornaments
events.push(v("kalimba", 10, 2.5, N.D6, 0.5, 0.30));
events.push(v("kalimba", 11, 1.0, N.F6, 0.5, 0.25));
events.push(v("kalimba", 12, 2.5, N.C6, 0.5, 0.30));
events.push(v("kalimba", 13, 1.5, N.A5, 0.5, 0.25));

// quiet bass under the wow
events.push(v("bass", 10, 0, N.F2, 3.0, 0.35));
events.push(v("bass", 12, 0, N.F2, 3.0, 0.35));

// ratatatata — a fast staccato 16th-note tumble across bar 11, a
// playful machine-gun burst against the slow held vibraphone.
const wowRatata = [N.A5, N.G5, N.A5, N.G5, N.F5, N.G5, N.A5, N.C6];
...
events.push(v("rosewood", 14, 0, N.A5, 0.5, 0.60));
events.push(v("rosewood", 14, 0.5, N.G5, 0.5, 0.55));
events.push(v("rosewood", 14, 1.0, N.A5, 1.0, 0.60));
events.push(v("rosewood", 14, 2.0, N.F5, 1.0, 0.65));
events.push(v("rosewood", 15, 0, N.C6, 0.5, 0.60));
events.push(v("rosewood", 15, 0.5, N.Bb5, 0.5, 0.55));
events.push(v("rosewood", 15, 1.0, N.C6, 1.0, 0.60));
events.push(v("rosewood", 15, 2.0, N.A5, 1.0, 0.65));
events.push(v("rosewood", 16, 0, N.G5, 1.0, 0.55));
events.push(v("rosewood", 16, 1, N.F5, 1.0, 0.55));
events.push(v("rosewood", 16, 2, N.E5, 1.0, 0.50));
events.push(v("rosewood", 17, 0, N.F5, 3.0, 0.55));

// kalimba doubles the "bap" on bars 14, 15 (Octave up sparkle)
events.push(v("kalimba", 14, 2, N.F6, 1.0, 0.28));
events.push(v("kalimba", 15, 2, N.A5, 1.0, 0.28));

// bass: walk down — F, C, Bb, F
const babaBass = [N.F2, N.F2 + 7, N.F2 - 2, N.F2];
for (let i = 0; i < 4; i++) {
...
// bar 9 — descending pentatonic 16ths closing the twinkle section
run16(9, 0, [N.C6, N.A5, N.G5, N.E5, N.D5, N.C5,
             N.A4, N.C5, N.D5, N.E5, N.G5, N.A5], 0.30);
// bar 13 — ascending 16ths spilling out of the wow
run16(13, 0, [N.F5, N.G5, N.A5, N.C6, N.D6, N.C6,
              N.A5, N.G5, N.F5, N.G5, N.A5, N.C6], 0.28);
// bar 21 — fast 16ths winding down into the ending cadence
run16(21, 0, [N.A5, N.G5, N.F5, N.E5, N.D5, N.C5,
              N.A4, N.G4, N.F4, N.G4, N.A4, N.C5], 0.26);

// ── [sleep now] — bars 18..23 — settling descent ─────────────────────
events.push(v("rosewood", 18, 0, N.C5, 1.5, 0.45));
events.push(v("rosewood", 18, 1.5, N.A4, 1.5, 0.40));
events.push(v("rosewood", 19, 0, N.G4, 1.0, 0.40));
events.push(v("rosewood", 19, 1, N.F4, 1.0, 0.40));
events.push(v("rosewood", 19, 2, N.F4, 1.0, 0.35));
events.push(v("rosewood", 20, 0, N.A4, 1.0, 0.40));
events.push(v("rosewood", 20, 1, N.G4, 1.0, 0.35));
events.push(v("rosewood", 20, 2, N.F4, 1.0, 0.35));
events.push(v("rosewood", 21, 0, N.F4, 3.0, 0.45));

// kalimba — one last twinkle, then silence
events.push(v("kalimba", 19, 2.5, N.C6, 0.5, 0.18));
events.push(v("kalimba", 21, 1.0, N.F5, 0.5, 0.15));

// soft bass alternating through the descent
events.push(v("bass", 18, 0, N.F2, 3.0, 0.35));
events.push(v("bass", 20, 0, N.F2, 3.0, 0.35));
...
events.push(v("rosewood", 22, 0,   N.C5, 1.0, 0.45));
events.push(v("rosewood", 22, 1,   N.A4, 1.0, 0.45));
events.push(v("rosewood", 22, 2,   N.F4, 1.0, 0.45));
events.push(v("rosewood", 23, 0,   N.G4, 1.5, 0.42));
// ratatatata — fast staccato 16th run scooping up to the cadence
const flourish = [N.F4, N.G4, N.A4, N.C5, N.D5, N.F5, N.G5, N.A5];
for (let i = 0; i < flourish.length; i++) {
  events.push(v("staccato", 23, 1.5 + i * 0.1875, flourish[i], 0.22, 0.40));
}
// bar 24-25: final rolled F major chord — staggered entries 30ms apart
const finalChord = [N.F2, N.F3, N.A3, N.C4, N.F4, N.A4, N.C5, N.F5];
...
// vibraphone_off halo under the final chord
for (const m of [N.F3, N.A3, N.C4, N.F4]) {
  events.push({ preset: "vibraphone_off", startSec: 24 * BAR, midi: m,
                durSec: 2 * BAR, gain: 0.16, decayMul: 1.8 });
}
events.push(v("bass", 22, 0, N.F2, 3.0, 0.34));
...
pushPad(0, 4,    [N.F4, N.A4, N.C5]);              // bars 0..3   F
pushPad(10, 4,   [N.G4, N.Bb4, N.D5]);             // bars 10..13 Gm
pushPad(14, 1,   [N.F4, N.A4, N.C5]);              // bar 14      F
pushPad(15, 1,   [N.E4, N.G4, N.C5]);              // bar 15      C
pushPad(16, 2,   [N.F4, N.A4, N.C5]);              // bars 16..17 F
pushPad(18, 4,   [N.F4, N.A4, N.C5]);              // bars 18..21 F
...
pushOsti(2,  [N.F4, N.A4, N.C5]);    // hush B
pushOsti(3,  [N.F4, N.A4, N.C5]);
pushOsti(4,  [N.F4, N.A4, N.C5]);
pushOsti(5,  [N.F4, N.A4, N.C5]);
pushOsti(6,  [N.F4, N.A4, N.C5]);
pushOsti(7,  [N.Bb4, N.D5, N.F5]);   // Bb
pushOsti(8,  [N.Bb4, N.D5, N.F5]);
pushOsti(9,  [N.Bb4, N.D5, N.F5]);
// bars 10–13 — leave the wow section spacious, no ostinato
pushOsti(14, [N.F4, N.A4, N.C5]);
pushOsti(15, [N.E4, N.G4, N.C5]);
pushOsti(16, [N.F4, N.A4, N.C5]);
pushOsti(17, [N.F4, N.A4, N.C5]);
pushOsti(18, [N.F4, N.A4, N.C5]);
pushOsti(19, [N.F4, N.A4, N.C5]);
// bars 20–21 fade out the ostinato gradually
events.push(v("kalimba", 20, 0, N.F4, 0.9, OSTI_GAIN * 0.6));
events.push(v("kalimba", 20, 2, N.A4, 0.9, OSTI_GAIN * 0.45));
events.push(v("kalimba", 21, 1, N.F4, 0.9, OSTI_GAIN * 0.3));
...
const connectors = [
  // bar 1 hush tail: hum F4 → A4 → F4 (gentle breath shape)
  { startBar: 1, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN,
    points: [[0.0, N.F4], [1.2, N.A4], [2.5, N.F4]] },
  // bar 3 hush tail: F4 sustained, drifting up a tiny bit then back
  { startBar: 3, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN,
    points: [[0.0, N.F4], [1.5, N.G4], [2.5, N.F4]] },
  // bar 5 twinkle tail: G5 → A5 → G5 (matches the wave shape above)
  { startBar: 5, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN * 0.85,
    points: [[0.0, N.G5], [1.2, N.A5], [2.5, N.G5]] },
  // bar 7 twinkle tail: F5 lifting briefly to G5
  { startBar: 7, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN * 0.85,
    points: [[0.0, N.F5], [1.3, N.G5], [2.5, N.F5]] },
  // section seam — between [twinkle] and [wow], glide F5 → G5 (where
  // the vibraphone takes over) across the bar 9→10 transition
  { startBar: 9, startBeat: 1.5, durBeats: 2.5, gain: SINE_GAIN * 0.9,
    points: [[0.0, N.F5], [2.5, N.G5]] },
  // bar 13 wow tail: F5 hum settling
  { startBar: 13, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN * 0.9,
    points: [[0.0, N.F5], [1.5, N.E5], [2.5, N.F5]] },
  // bar 17 baba tail: F5 → E5 → F5 (matches the bap resolution)
  { startBar: 17, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN,
    points: [[0.0, N.F5], [1.5, N.E5], [2.5, N.F5]] },
  // bar 21 sleep tail: F4 humming low
  { startBar: 21, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN,
    points: [[0.0, N.F4], [1.0, N.G4], [2.5, N.F4]] },
  // bars 22–25 — the goodnight, a drifting low F drone under the
  // composed cadence, pulling the room tone to the final chord.
  { startBar: 22, startBeat: 0, durBeats: 12.0, gain: SINE_GAIN * 1.1,
    points: [[0.0, N.F3], [4.0, N.F4], [8.0, N.F3], [12.0, N.F3]] },
];
...
setChord(0, 6,    29, 36, 41, 45);   // F1 C2 F2 A2 — F major
setChord(7, 9,    34, 41, 46, 50);   // Bb1 F2 Bb2 D3 — Bb major
setChord(10, 13,  31, 38, 43, 46);   // G1 D2 G2 Bb2 — G minor
setChord(14, 14,  29, 36, 41, 45);   // F
setChord(15, 15,  36, 43, 48, 52);   // C2 G2 C3 E3 — C major
setChord(16, 17,  29, 36, 41, 45);   // F
setChord(18, 21,  29, 36, 41, 45);   // F (sleep)
setChord(22, TOTAL_BARS - 1, 29, 36, 41, 45);   // F (goodnight + tail)
...
const voiceMelody = [
  ...
  // twinkle bars 4-9
  { bar: 4,  beat: 0,   midi: N.F5,  durBeats: 1.0, v: "a" },
  { bar: 4,  beat: 1,   midi: N.A5,  durBeats: 1.0, v: "a" },
  { bar: 4,  beat: 2.0, midi: N.C6,  durBeats: 0.5, v: "a" },
  { bar: 4,  beat: 2.5, midi: N.A5,  durBeats: 0.5, v: "a" },
  { bar: 5,  beat: 0,   midi: N.G5,  durBeats: 3.0, v: "a" },
  { bar: 6,  beat: 0,   midi: N.F5,  durBeats: 1.0, v: "a" },
  { bar: 6,  beat: 1,   midi: N.G5,  durBeats: 1.0, v: "a" },
  { bar: 6,  beat: 2.0, midi: N.A5,  durBeats: 0.5, v: "a" },
  { bar: 6,  beat: 2.5, midi: N.G5,  durBeats: 0.5, v: "a" },
  { bar: 7,  beat: 0,   midi: N.F5,  durBeats: 3.0, v: "a" },
  { bar: 8,  beat: 0,   midi: N.Bb5, durBeats: 1.0, v: "a" },
  { bar: 8,  beat: 1,   midi: N.D6,  durBeats: 1.0, v: "a" },
  { bar: 8,  beat: 2,   midi: N.Bb5, durBeats: 1.0, v: "a" },
  { bar: 9,  beat: 0,   midi: N.C6,  durBeats: 1.5, v: "a" },
  { bar: 9,  beat: 1.5, midi: N.A5,  durBeats: 1.5, v: "a" },
  // wow bars 12-13 — "woo woo woo whoa"
  { bar: 12, beat: 0,   midi: N.A5,  durBeats: 1.0, v: "u" },
  { bar: 12, beat: 1,   midi: N.G5,  durBeats: 1.0, v: "u" },
  { bar: 12, beat: 2,   midi: N.A5,  durBeats: 1.0, v: "u" },
  { bar: 13, beat: 0,   midi: N.F5,  durBeats: 3.0, v: "o" },
  // baba bars 14-17
  { bar: 14, beat: 0,   midi: N.A5,  durBeats: 0.5, v: "a" },
  { bar: 14, beat: 0.5, midi: N.G5,  durBeats: 0.5, v: "a" },
  { bar: 14, beat: 1.0, midi: N.A5,  durBeats: 1.0, v: "a" },
  { bar: 14, beat: 2.0, midi: N.F5,  durBeats: 1.0, v: "a" },
  { bar: 15, beat: 0,   midi: N.C6,  durBeats: 0.5, v: "a" },
  { bar: 15, beat: 0.5, midi: N.Bb5, durBeats: 0.5, v: "a" },
  { bar: 15, beat: 1.0, midi: N.C6,  durBeats: 1.0, v: "a" },
  { bar: 15, beat: 2.0, midi: N.A5,  durBeats: 1.0, v: "a" },
  { bar: 16, beat: 0,   midi: N.G5,  durBeats: 1.0, v: "a" },
  { bar: 16, beat: 1,   midi: N.F5,  durBeats: 1.0, v: "a" },
  { bar: 16, beat: 2,   midi: N.E5,  durBeats: 1.0, v: "a" },
  { bar: 17, beat: 0,   midi: N.F5,  durBeats: 3.0, v: "a" },
  // sleep bars 18-21
  { bar: 18, beat: 0,   midi: N.C5,  durBeats: 1.5, v: "a" },
  { bar: 18, beat: 1.5, midi: N.A4,  durBeats: 1.5, v: "a" },
  { bar: 19, beat: 0,   midi: N.G4,  durBeats: 1.0, v: "a" },
  { bar: 19, beat: 1,   midi: N.F4,  durBeats: 1.0, v: "a" },
  { bar: 19, beat: 2,   midi: N.F4,  durBeats: 1.0, v: "a" },
  { bar: 20, beat: 0,   midi: N.A4,  durBeats: 1.0, v: "a" },
  { bar: 20, beat: 1,   midi: N.G4,  durBeats: 1.0, v: "a" },
  { bar: 20, beat: 2,   midi: N.F4,  durBeats: 1.0, v: "a" },
  { bar: 21, beat: 0,   midi: N.F4,  durBeats: 3.0, v: "a" },
  // ending bars 22-23 — closing "ooh" echo of the hush motif
  { bar: 22, beat: 0,   midi: N.C5,  durBeats: 1.0, v: "o" },
  { bar: 22, beat: 1,   midi: N.A4,  durBeats: 1.0, v: "o" },
  { bar: 22, beat: 2,   midi: N.F4,  durBeats: 1.0, v: "o" },
  { bar: 23, beat: 0,   midi: N.F4,  durBeats: 3.0, v: "o" },
];
...
if (IS_MASTER) {
  ...
  writeFileSync(resolve(VTAKES_DIR, "manifest.json"), JSON.stringify({
    track: "marimbaba", lane: "marimba", bpm: BPM,
...
console.log(`→ marimbaba · ${events.length} marimba + ${connectors.length} connectors + ${bubbles.length} bubbles + phaser · ${totalSec.toFixed(1)} s · F major 3/4 @ ${BPM} BPM`);
...
// ── storyline-visualizer telemetry (marimbaba.struct.json) ────────────
// Emit the struct.json the preview-score visualizer reads: the 10 story
// sections + the marimba note events grouped by voice lane — the "sound
// elements" that ride the verlet string. Additive — the deterministic
// audio above is untouched; this just publishes its event schedule.
{
  ...
  // 10 story sections — two per .np musical section (faster panel changes)
  const SEC_BARS = [
    ["hush1", 0, 2], ["hush2", 2, 4],
    ["twinkle1", 4, 7], ["twinkle2", 7, 10],
    ["wow1", 10, 12], ["wow2", 12, 14],
    ["baba1", 14, 16], ["baba2", 16, 18],
    ["sleep1", 18, 22], ["sleep2", 22, TOTAL_BARS],
  ];
  const sections = SEC_BARS.map(([name, b0, b1], i) => ({
    name,
    startSec: +(b0 * BAR).toFixed(4),
    endSec: i === SEC_BARS.length - 1
      ? +structTotalSec.toFixed(4) : +(b1 * BAR).toFixed(4),
  }));
  const structPath = resolve(HERE, "..", "out", "marimbaba.struct.json");
  writeFileSync(structPath, JSON.stringify({
    _comment: "Storyline-visualizer telemetry, emitted by render-marimbaba.mjs: totalSec + 10 story sections + marimba note events per voice lane (the sound-elements riding the verlet string in preview-score.mjs).",
    meter: 3, bpm: BPM, scale: "major", rootMidi: 53,
    totalSec: +structTotalSec.toFixed(4),
    prerollSec: 0,
    sections,
    events: laneEvents,
  }, null, 2) + "\n");
  const nEv = Object.values(laneEvents).reduce((s, a) => s + a.length, 0);
  console.log(`✓ ${structPath} (${sections.length} sections · ${nEv} note events)`);
}

// ── optional DistroKid WAV master ─────────────────────────────────────
// --wav <path> writes a lossless 44.1 kHz / 16-bit stereo master from
// the same f32 buffer: pop master chain → loudnorm to -14 LUFS / -1.5
// dBTP (streaming-standard, matches the helpabeach single).
const wavIdx = argv.indexOf("--wav");
const wavOut = wavIdx >= 0 && argv[wavIdx + 1] ? expandHome(argv[wavIdx + 1]) : null;
if (wavOut) {
  mkdirSync(dirname(wavOut), { recursive: true });
  const wff = spawnSync("ffmpeg", ["-hide_banner", "-y", "-loglevel", "error",
    "-f", "f32le", "-ar", String(SR), "-ac", "2", "-i", rawPath,
    "-af", `${MASTER},loudnorm=I=-14:TP=-1.5:LRA=11`,
    "-ar", "44100", "-ac", "2", "-c:a", "pcm_s16le", wavOut], { stdio: "inherit" });
  if (wff.status !== 0) { try { unlinkSync(rawPath); } catch {}; console.error("✗ wav master failed"); process.exit(1); }
