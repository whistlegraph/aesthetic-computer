#!/usr/bin/env node
// marimba/bin/motion-score-marimbaba-yt.mjs — the marimbaba MOTION
// visualizer: the Seedance motion cut (gen-motion-marimbaba.mjs
// --assemble) as the live background, with the full preview-score
// chrome riding on top. Forked from preview-score-marimbaba-yt.mjs;
// the panel Ken-Burns / face-zoom / backlight / transition pipeline is
// replaced by a raw ffmpeg frame pipe from the motion mp4 — every
// other feature is inherited verbatim:
//
...
const TITLE  = flags.title || "Marimbaba";   // capital M in the side badges (jas, 2026-06-11)
...
// The motion cut is the background; FPS pinned to its 24 so background
// frames map 1:1 to output frames.
const MOTION = flags.motion || `${LANE}/out/${SLUG}-motion-yt.mp4`;
const OUT    = flags.out    || `${LANE}/out/${SLUG}-motion-score-yt.mp4`;
...
checkYwftAvailable();
for (const [name, p] of [["audio", AUDIO], ["struct", STRUCT], ["motion", MOTION]]) {
  if (!existsSync(p)) {
...
// ── motion background frame pipe ─────────────────────────────────────
// ffmpeg decodes the motion cut to raw RGBA frames at output size +
// fps; nextBgFrame() pulls exactly one frame's bytes with backpressure.
// On EOF (motion runs ~0.07s short of the struct total) the last frame
// holds.
const FRAME_BYTES = W * H * 4;
const bgProc = spawn("ffmpeg", [
  "-v", "error",
  ...(START_T > 0 ? ["-ss", String(START_T)] : []),
  "-i", MOTION,
  "-vf", `scale=${W}:${H},fps=${FPS}`,
  "-pix_fmt", "rgba", "-f", "rawvideo", "pipe:1",
]);
const bgChunks = [];
let bgBuffered = 0, bgDone = false, bgWake = null;
bgProc.stdout.on("data", (d) => {
  bgChunks.push(d); bgBuffered += d.length;
  if (bgBuffered > FRAME_BYTES * 8) bgProc.stdout.pause();
  bgWake?.();
});
bgProc.stdout.on("end", () => { bgDone = true; bgWake?.(); });
bgProc.on("error", (e) => { console.error(`✗ motion decode failed: ${e.message}`); process.exit(1); });
async function nextBgFrame() {
  const out = Buffer.allocUnsafe(FRAME_BYTES);
  let filled = 0;
  while (filled < FRAME_BYTES) {
    if (!bgChunks.length) {
      if (bgDone) return null;
      await new Promise((r) => { bgWake = r; bgProc.stdout.resume(); });
      bgWake = null;
      continue;
    }
    const head = bgChunks[0];
    const need = FRAME_BYTES - filled;
    if (head.length <= need) { head.copy(out, filled); filled += head.length; bgBuffered -= head.length; bgChunks.shift(); }
    else { head.copy(out, filled, 0, need); bgChunks[0] = head.subarray(need); bgBuffered -= need; filled = FRAME_BYTES; }
  }
  if (bgBuffered < FRAME_BYTES * 4) bgProc.stdout.resume();
  return out;
}
let lastBgFrame = null;
...
for (let f = startFrame; f < endFrame; f++) {
  ...
  // ── scene: the Seedance motion cut, frame-for-frame ────────────────
  // (replaces the panel Ken-Burns / face-zoom / backlight / transition
  // pipeline of preview-score — the footage carries its own camera.)
  const bgBuf = await nextBgFrame();
  if (bgBuf) {
    lastBgFrame = new ImageData(
      new Uint8ClampedArray(bgBuf.buffer, bgBuf.byteOffset, FRAME_BYTES), W, H);
  }
  if (lastBgFrame) ctx.putImageData(lastBgFrame, 0, 0);
