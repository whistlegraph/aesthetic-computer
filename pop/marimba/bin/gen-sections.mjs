...
import { COLORED_PENCIL_TOOTH, FRAMING_YT_LANDSCAPE } from "../../lib/mediums.mjs";
...
`LANDSCAPE OVERRIDE — recompose this beat's scene for the wide frame: PULL THE CAMERA BACK so the WHOLE late-night study / setting stretches horizontally — desk, lamp, bookshelf, window, armchair, doorway visible across the width. keep the medium and the calm somber late-night lullaby mood — only the framing widens.
...
const SECTION_VARIANTS = {
  hush1:
`BEAT — HUSH 1 (jeffrey approaches) · EMOTION: quiet ANTICIPATION, a little wistful. the opening establishing panel, OUTSIDE at night. jeffrey — and EXACTLY ONE jeffrey, he appears ONCE in this image, outside only — walks up a quiet garden path toward Bill Gates's front door, hands in pockets, thoughtful and expectant, seen mostly from behind or in profile as he heads for the door. one ground-floor window glows warm amber — the study, where BILL GATES SITS ALONE at the white typewriter, a small distant lamplit figure (grey hair, sage-green sweater), the ONLY person inside. NOBODY sits beside Gates; the chair next to him is EMPTY — jeffrey has not arrived yet, he is still outside on the path. exactly TWO people total in the whole image: one jeffrey outside, one Gates inside the window. dark trees, a cool blue night, a sliver of moon. jeffrey small in the frame, arriving at the door. WIDE establishing shot, low angle from across the path.`,
  hush2:
`BEAT — HUSH 2 (the doorway hello) · EMOTION: warm, shy GREETING — Bill Gates SURPRISED and quietly PLEASED, lonely-glad that someone came. jeffrey has stepped into the doorway of the hushed late-night STUDY, raising one hand in a small hello. across the lamplit room Bill Gates twists round from the WHITE SELECTRIC TYPEWRITER on the desk, eyebrows lifted in mild surprise, a relieved smile breaking. the typewriter shown from the side, paper-bail and rolled paper occluded by the type ball and carriage. desk lamp the one warm pool of light; bookshelves, a worn armchair, a dark night window. MEDIUM-WIDE room shot from near the doorway.`,
  twinkle1:
`BEAT — TWINKLE 1 (companionable, from behind) · EMOTION: warm COMPANIONABLE ease, read entirely through posture — shoulders loose, heads tilted gently toward each other. the camera sits BEHIND the two men: the backs of their heads and shoulders fill the near frame (jeffrey's tousled brown hair and light-blue shirt on the left, Bill's grey hair and sage-green sweater on the right), the WHITE SELECTRIC TYPEWRITER on the desk beyond their shoulders, the warm lamp glow spilling between their silhouettes. NEITHER FACE IS VISIBLE — no profiles, no glasses-edges, just the two backs leaning companionably. the page and platen are fully hidden behind their shoulders and the typewriter's carriage body from this angle.`,
  twinkle2:
`BEAT — TWINKLE 2 (the type ball, in close-up) · EMOTION: absorbed wonder, told entirely through the machine and a pointing hand. EXTREME CLOSE-UP macro on Bill's hand pointing at the silver chrome TYPE BALL on the Selectric mid-spin, his finger an inch away. the type ball mechanism, ribbon cartridge, and typebar housing FILL the frame edge to edge. NO faces anywhere in this panel: beyond the machine everything falls into soft warm out-of-focus darkness — at most two vague unfocused silhouette shapes, unreadable as people. the lamp light catches the chrome. no readable page surface.`,
  wow1:
