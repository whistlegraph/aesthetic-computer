...
const N = {
  ...
  C6: 84, D6: 86, F6: 89,
  // doziba — D major palette (D E F# G A B C#)
  D2: 38,
  Fs4: 66, B4: 71, Cs5: 73,
  Fs5: 78, B5: 83, Cs6: 85,
  Fs6: 90,
...
// ── [hush hush] — bars 0..3 — descending sigh (D major) ──────────────
// doziba: drowsy stepwise descent — A4 → G4 → Fs4 → (held) → G4 → Fs4 → E4
events.push(v("rosewood", 0, 0, N.A4, 1.0, 0.55));
events.push(v("rosewood", 0, 1, N.G4, 1.0, 0.55));
events.push(v("rosewood", 0, 2, N.Fs4, 1.0, 0.55));
events.push(v("rosewood", 1, 0, N.Fs4, 3.0, 0.45));
events.push(v("rosewood", 2, 0, N.G4, 1.5, 0.55));
events.push(v("rosewood", 2, 1.5, N.Fs4, 1.5, 0.55));
events.push(v("rosewood", 3, 0, N.E4, 3.0, 0.55));

// bass: gentle low rocker on beat 1 every bar — D major root
for (let b = 0; b < 4; b++) {
  events.push(v("bass", b, 0, N.D2, 3.0, 0.55));
}

// ── [twinkle] — bars 4..9 — drowsy descending waves (D major) ────────
events.push(v("rosewood", 4, 0, N.B5, 1.0, 0.60));
events.push(v("rosewood", 4, 1, N.A5, 1.0, 0.60));
events.push(v("rosewood", 4, 2.0, N.Fs5, 0.5, 0.65));
events.push(v("rosewood", 4, 2.5, N.A5, 0.5, 0.55));
events.push(v("rosewood", 5, 0, N.G5, 3.0, 0.55));
events.push(v("rosewood", 6, 0, N.Fs5, 1.0, 0.55));
events.push(v("rosewood", 6, 1, N.E5, 1.0, 0.55));
events.push(v("rosewood", 6, 2.0, N.G5, 0.5, 0.60));
events.push(v("rosewood", 6, 2.5, N.Fs5, 0.5, 0.50));
events.push(v("rosewood", 7, 0, N.D5, 3.0, 0.55));
events.push(v("rosewood", 8, 0, N.A5, 1.0, 0.60));
events.push(v("rosewood", 8, 1, N.G5, 1.0, 0.65));
events.push(v("rosewood", 8, 2, N.Fs5, 1.0, 0.55));
events.push(v("rosewood", 9, 0, N.E5, 1.5, 0.60));
events.push(v("rosewood", 9, 1.5, N.Cs5, 1.5, 0.55));

// bass pulse — D-A alternating roots, gives the rocking
const twinkleBass = [N.D2, N.F3, N.D2, N.F3, N.D2, N.F3];
for (let i = 0; i < 6; i++) {
  events.push(v("bass", 4 + i, 0, twinkleBass[i], 3.0, 0.45));
}

// vibraphone_off pad — D major sustained over bars 4-6, G major over 7-9
events.push({ preset: "vibraphone_off", startSec: 4 * BAR, midi: N.D4, durSec: 3 * BAR, gain: 0.18 });
events.push({ preset: "vibraphone_off", startSec: 4 * BAR, midi: N.Fs4, durSec: 3 * BAR, gain: 0.18 });
events.push({ preset: "vibraphone_off", startSec: 4 * BAR, midi: N.A4, durSec: 3 * BAR, gain: 0.18 });
events.push({ preset: "vibraphone_off", startSec: 7 * BAR, midi: N.G4, durSec: 3 * BAR, gain: 0.18 });
events.push({ preset: "vibraphone_off", startSec: 7 * BAR, midi: N.B4, durSec: 3 * BAR, gain: 0.18 });
...
// vibraphone WITH motor for the "wow" wobble feel — A major haze
events.push({ preset: "vibraphone", startSec: 10 * BAR, midi: N.A5, durSec: 2 * BAR, gain: 0.45 });
events.push({ preset: "vibraphone", startSec: 10 * BAR, midi: N.Cs6, durSec: 2 * BAR, gain: 0.40 });
events.push(v("rosewood", 12, 0, N.G5, 1.0, 0.55));
events.push(v("rosewood", 12, 1, N.Fs5, 1.0, 0.55));
events.push(v("rosewood", 12, 2, N.E5, 1.0, 0.55));
events.push(v("rosewood", 13, 0, N.D5, 3.0, 0.55));

// kalimba sparkles overlaid on the wow — soft, off-beat ornaments
events.push(v("kalimba", 10, 2.5, N.Cs6, 0.5, 0.30));
events.push(v("kalimba", 11, 1.0, N.A5, 0.5, 0.25));
events.push(v("kalimba", 12, 2.5, N.B5, 0.5, 0.30));
events.push(v("kalimba", 13, 1.5, N.Fs5, 0.5, 0.25));

// quiet bass under the wow — A major root
events.push(v("bass", 10, 0, N.D2, 3.0, 0.35));
events.push(v("bass", 12, 0, N.D2, 3.0, 0.35));

// ratatatata — a fast staccato 16th-note tumble across bar 11, a
// playful machine-gun burst against the slow held vibraphone.
const wowRatata = [N.Cs6, N.B5, N.A5, N.G5, N.Fs5, N.G5, N.A5, N.B5];
for (let i = 0; i < wowRatata.length; i++) {
  events.push(v("staccato", 11, i * 0.1875, wowRatata[i], 0.22, 0.34));
}

// ── [ba-ba-ba bap] — bars 14..17 — slinky-dog wobble (D major) ───────
events.push(v("rosewood", 14, 0, N.Fs5, 0.5, 0.60));
events.push(v("rosewood", 14, 0.5, N.E5, 0.5, 0.55));
events.push(v("rosewood", 14, 1.0, N.Fs5, 1.0, 0.60));
events.push(v("rosewood", 14, 2.0, N.D5, 1.0, 0.65));
events.push(v("rosewood", 15, 0, N.E5, 0.5, 0.60));
events.push(v("rosewood", 15, 0.5, N.D5, 0.5, 0.55));
events.push(v("rosewood", 15, 1.0, N.E5, 1.0, 0.60));
events.push(v("rosewood", 15, 2.0, N.Cs5, 1.0, 0.65));
events.push(v("rosewood", 16, 0, N.D5, 1.0, 0.55));
events.push(v("rosewood", 16, 1, N.Cs5, 1.0, 0.55));
events.push(v("rosewood", 16, 2, N.B4, 1.0, 0.50));
events.push(v("rosewood", 17, 0, N.A4, 3.0, 0.55));

// kalimba doubles the "bap" on bars 14, 15 (Octave up sparkle)
events.push(v("kalimba", 14, 2, N.Fs6, 1.0, 0.28));
events.push(v("kalimba", 15, 2, N.D5, 1.0, 0.28));

// bass: walk down — D, B, G, A
const babaBass = [N.D2, N.D2 - 3, N.D2 - 7, N.D2 + 7];
for (let i = 0; i < 4; i++) {
...
// bar 9 — descending D-major 16ths closing the twinkle section
run16(9, 0, [N.B5, N.A5, N.G5, N.Fs5, N.E5, N.D5,
             N.Cs5, N.D5, N.E5, N.Fs5, N.G5, N.A5], 0.30);
// bar 13 — ascending 16ths spilling out of the wow
run16(13, 0, [N.D5, N.E5, N.Fs5, N.G5, N.A5, N.G5,
              N.Fs5, N.E5, N.D5, N.E5, N.Fs5, N.A5], 0.28);
// bar 21 — fast 16ths winding down into the ending cadence
run16(21, 0, [N.A5, N.G5, N.Fs5, N.E5, N.D5, N.Cs5,
              N.B4, N.A4, N.G4, N.Fs4, N.G4, N.A4], 0.26);

// ── [sleep now] — bars 18..23 — settling descent (D major) ───────────
events.push(v("rosewood", 18, 0, N.A4, 1.5, 0.45));
events.push(v("rosewood", 18, 1.5, N.G4, 1.5, 0.40));
events.push(v("rosewood", 19, 0, N.Fs4, 1.0, 0.40));
events.push(v("rosewood", 19, 1, N.E4, 1.0, 0.40));
events.push(v("rosewood", 19, 2, N.E4, 1.0, 0.35));
events.push(v("rosewood", 20, 0, N.G4, 1.0, 0.40));
events.push(v("rosewood", 20, 1, N.Fs4, 1.0, 0.35));
events.push(v("rosewood", 20, 2, N.E4, 1.0, 0.35));
events.push(v("rosewood", 21, 0, N.D4, 3.0, 0.45));

// kalimba — one last twinkle, then silence
events.push(v("kalimba", 19, 2.5, N.B5, 0.5, 0.18));
events.push(v("kalimba", 21, 1.0, N.Fs5, 0.5, 0.15));

// soft bass alternating through the descent
events.push(v("bass", 18, 0, N.D2, 3.0, 0.35));
events.push(v("bass", 20, 0, N.D2, 3.0, 0.35));
...
events.push(v("rosewood", 22, 0,   N.A4, 1.0, 0.45));
events.push(v("rosewood", 22, 1,   N.G4, 1.0, 0.45));
events.push(v("rosewood", 22, 2,   N.Fs4, 1.0, 0.45));
events.push(v("rosewood", 23, 0,   N.E4, 1.5, 0.42));
// ratatatata — fast staccato 16th run scooping up to the cadence
const flourish = [N.D4, N.E4, N.Fs4, N.G4, N.A4, N.D5, N.Fs5, N.A5];
for (let i = 0; i < flourish.length; i++) {
  events.push(v("staccato", 23, 1.5 + i * 0.1875, flourish[i], 0.22, 0.40));
}
// bar 24-25: final rolled D major chord — staggered entries 30ms apart
const finalChord = [N.D2, N.D4 - 12, N.Fs4 - 12, N.A4 - 12, N.D4, N.Fs4, N.A4, N.D5];
...
// vibraphone_off halo under the final chord — D major
for (const m of [N.D4 - 12, N.Fs4 - 12, N.A4 - 12, N.D4]) {
  events.push({ preset: "vibraphone_off", startSec: 24 * BAR, midi: m,
                durSec: 2 * BAR, gain: 0.16, decayMul: 1.8 });
}
events.push(v("bass", 22, 0, N.D2, 3.0, 0.34));
...
// fill the gaps that the per-section pad calls left empty — D major
pushPad(0, 4,    [N.D4, N.Fs4, N.A4]);             // bars 0..3   D
pushPad(10, 4,   [N.E4, N.A4, N.Cs5]);             // bars 10..13 A
pushPad(14, 1,   [N.D4, N.Fs4, N.A4]);             // bar 14      D
pushPad(15, 1,   [N.D4, N.Fs4, N.B4]);             // bar 15      Bm
pushPad(16, 2,   [N.D4, N.G4, N.B4]);              // bars 16..17 G
pushPad(18, 4,   [N.D4, N.Fs4, N.A4]);             // bars 18..21 D
...
pushOsti(2,  [N.D4, N.Fs4, N.A4]);   // hush — D
pushOsti(3,  [N.D4, N.Fs4, N.A4]);
pushOsti(4,  [N.D4, N.Fs4, N.A4]);
pushOsti(5,  [N.D4, N.Fs4, N.A4]);
pushOsti(6,  [N.D4, N.Fs4, N.A4]);
pushOsti(7,  [N.G4, N.B4, N.D5]);    // G
pushOsti(8,  [N.G4, N.B4, N.D5]);
pushOsti(9,  [N.G4, N.B4, N.D5]);
// bars 10–13 — leave the wow section spacious, no ostinato
pushOsti(14, [N.D4, N.Fs4, N.A4]);   // D
pushOsti(15, [N.B4, N.D5, N.Fs5]);   // Bm
pushOsti(16, [N.G4, N.B4, N.D5]);    // G
pushOsti(17, [N.G4, N.B4, N.D5]);
pushOsti(18, [N.D4, N.Fs4, N.A4]);   // D
pushOsti(19, [N.D4, N.Fs4, N.A4]);
// bars 20–21 fade out the ostinato gradually
events.push(v("kalimba", 20, 0, N.D4, 0.9, OSTI_GAIN * 0.6));
events.push(v("kalimba", 20, 2, N.Fs4, 0.9, OSTI_GAIN * 0.45));
events.push(v("kalimba", 21, 1, N.D4, 0.9, OSTI_GAIN * 0.3));
...
const connectors = [
  // bar 1 hush tail: hum Fs4 → A4 → Fs4 (gentle breath shape)
  { startBar: 1, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN,
    points: [[0.0, N.Fs4], [1.2, N.A4], [2.5, N.Fs4]] },
  // bar 3 hush tail: E4 sustained, drifting up a tiny bit then back
  { startBar: 3, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN,
    points: [[0.0, N.E4], [1.5, N.Fs4], [2.5, N.E4]] },
  ...
  // bar 7 twinkle tail: D5 lifting briefly to E5
  { startBar: 7, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN * 0.85,
    points: [[0.0, N.D5], [1.3, N.E5], [2.5, N.D5]] },
  // section seam — between [twinkle] and [wow], glide E5 → A5 (where
  // the vibraphone takes over) across the bar 9→10 transition
  { startBar: 9, startBeat: 1.5, durBeats: 2.5, gain: SINE_GAIN * 0.9,
    points: [[0.0, N.Cs5], [2.5, N.A5]] },
  // bar 13 wow tail: D5 hum settling
  { startBar: 13, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN * 0.9,
    points: [[0.0, N.D5], [1.5, N.Cs5], [2.5, N.D5]] },
  // bar 17 baba tail: A4 → G4 → A4 (matches the bap resolution)
  { startBar: 17, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN,
    points: [[0.0, N.A4], [1.5, N.G4], [2.5, N.A4]] },
  // bar 21 sleep tail: D4 humming low
  { startBar: 21, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN,
    points: [[0.0, N.D4], [1.0, N.E4], [2.5, N.D4]] },
  // bars 22–25 — the goodnight, a drifting low D drone under the
  // composed cadence, pulling the room tone to the final chord.
  { startBar: 22, startBeat: 0, durBeats: 12.0, gain: SINE_GAIN * 1.1,
    points: [[0.0, N.D2 + 12], [4.0, N.D4], [8.0, N.D2 + 12], [12.0, N.D2 + 12]] },
];
...
setChord(0, 6,    26, 33, 38, 42);   // D1 A1 D2 Fs2 — D major
setChord(7, 9,    31, 38, 43, 47);   // G1 D2 G2 B2 — G major
setChord(10, 13,  33, 40, 45, 49);   // A1 E2 A2 Cs3 — A major
setChord(14, 14,  26, 33, 38, 42);   // D major
setChord(15, 15,  23, 30, 35, 38);   // B0 Fs1 B1 D2 — B minor
setChord(16, 17,  31, 38, 43, 47);   // G major
setChord(18, 21,  26, 33, 38, 42);   // D major (sleep)
setChord(22, TOTAL_BARS - 1, 26, 33, 38, 42);   // D major (goodnight + tail)
...
const voiceMelody = [
  ...
  // twinkle bars 4-9
  { bar: 4,  beat: 0,   midi: N.B5,  durBeats: 1.0, v: "a" },
  { bar: 4,  beat: 1,   midi: N.A5,  durBeats: 1.0, v: "a" },
  { bar: 4,  beat: 2.0, midi: N.Fs5, durBeats: 0.5, v: "a" },
  { bar: 4,  beat: 2.5, midi: N.A5,  durBeats: 0.5, v: "a" },
  { bar: 5,  beat: 0,   midi: N.G5,  durBeats: 3.0, v: "a" },
  { bar: 6,  beat: 0,   midi: N.Fs5, durBeats: 1.0, v: "a" },
  { bar: 6,  beat: 1,   midi: N.E5,  durBeats: 1.0, v: "a" },
  { bar: 6,  beat: 2.0, midi: N.G5,  durBeats: 0.5, v: "a" },
  { bar: 6,  beat: 2.5, midi: N.Fs5, durBeats: 0.5, v: "a" },
  { bar: 7,  beat: 0,   midi: N.D5,  durBeats: 3.0, v: "a" },
  { bar: 8,  beat: 0,   midi: N.A5,  durBeats: 1.0, v: "a" },
  { bar: 8,  beat: 1,   midi: N.G5,  durBeats: 1.0, v: "a" },
  { bar: 8,  beat: 2,   midi: N.Fs5, durBeats: 1.0, v: "a" },
  { bar: 9,  beat: 0,   midi: N.E5,  durBeats: 1.5, v: "a" },
  { bar: 9,  beat: 1.5, midi: N.Cs5, durBeats: 1.5, v: "a" },
  // wow bars 12-13 — "woo woo woo whoa"
  { bar: 12, beat: 0,   midi: N.G5,  durBeats: 1.0, v: "u" },
  { bar: 12, beat: 1,   midi: N.Fs5, durBeats: 1.0, v: "u" },
  { bar: 12, beat: 2,   midi: N.E5,  durBeats: 1.0, v: "u" },
  { bar: 13, beat: 0,   midi: N.D5,  durBeats: 3.0, v: "o" },
  // baba bars 14-17
  { bar: 14, beat: 0,   midi: N.Fs5, durBeats: 0.5, v: "a" },
  { bar: 14, beat: 0.5, midi: N.E5,  durBeats: 0.5, v: "a" },
  { bar: 14, beat: 1.0, midi: N.Fs5, durBeats: 1.0, v: "a" },
  { bar: 14, beat: 2.0, midi: N.D5,  durBeats: 1.0, v: "a" },
  { bar: 15, beat: 0,   midi: N.E5,  durBeats: 0.5, v: "a" },
  { bar: 15, beat: 0.5, midi: N.D5,  durBeats: 0.5, v: "a" },
  { bar: 15, beat: 1.0, midi: N.E5,  durBeats: 1.0, v: "a" },
  { bar: 15, beat: 2.0, midi: N.Cs5, durBeats: 1.0, v: "a" },
  { bar: 16, beat: 0,   midi: N.D5,  durBeats: 1.0, v: "a" },
  { bar: 16, beat: 1,   midi: N.Cs5, durBeats: 1.0, v: "a" },
  { bar: 16, beat: 2,   midi: N.B4,  durBeats: 1.0, v: "a" },
  { bar: 17, beat: 0,   midi: N.A4,  durBeats: 3.0, v: "a" },
  // sleep bars 18-21
  { bar: 18, beat: 0,   midi: N.A4,  durBeats: 1.5, v: "a" },
  { bar: 18, beat: 1.5, midi: N.G4,  durBeats: 1.5, v: "a" },
  { bar: 19, beat: 0,   midi: N.Fs4, durBeats: 1.0, v: "a" },
  { bar: 19, beat: 1,   midi: N.E4,  durBeats: 1.0, v: "a" },
  { bar: 19, beat: 2,   midi: N.E4,  durBeats: 1.0, v: "a" },
  { bar: 20, beat: 0,   midi: N.G4,  durBeats: 1.0, v: "a" },
  { bar: 20, beat: 1,   midi: N.Fs4, durBeats: 1.0, v: "a" },
  { bar: 20, beat: 2,   midi: N.E4,  durBeats: 1.0, v: "a" },
  { bar: 21, beat: 0,   midi: N.D4,  durBeats: 3.0, v: "a" },
  // ending bars 22-23 — closing "ooh" echo of the hush motif
  { bar: 22, beat: 0,   midi: N.A4,  durBeats: 1.0, v: "o" },
  { bar: 22, beat: 1,   midi: N.G4,  durBeats: 1.0, v: "o" },
  { bar: 22, beat: 2,   midi: N.Fs4, durBeats: 1.0, v: "o" },
  { bar: 23, beat: 0,   midi: N.E4,  durBeats: 3.0, v: "o" },
];
