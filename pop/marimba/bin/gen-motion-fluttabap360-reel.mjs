#!/usr/bin/env node
// marimba/bin/gen-motion-fluttabap360-reel.mjs — the fluttabap360
// METAMORPHOSIS REEL motion pass. Each reel beat-master panel
// (out/_fluttabap360-reel-beat-<name>.png, from
// gen-sections-fluttabap360-reel.mjs) becomes the source frame for a
// Seedance 2.0 image-to-video shot; --assemble then cuts the 46.2s
// portrait reel against ONE COMPLETE MUSICAL PHRASE of the track —
// mommywow → slinky → fly → ride (34.7s→80.9s), hush → climb → lift →
// outro, ending exactly on the pass boundary so the reel LOOPS — and
...
// pirouette in one continuous shot, fixing the two-similar-shots
// glitch). Morph clips ARRIVE on their end frame, so assembly trims
// them from the HEAD.
//
...
//
// Moderation: every human beat is staged from behind / three-quarter-
// behind / head-down; a FAILED shot is logged and SKIPPED. CUT beats
// with no clip fall back to Ken Burns at assembly.
//
// Usage:
//   node pop/marimba/bin/gen-motion-fluttabap360-reel.mjs --dry-run     # prompts + cost
//   node pop/marimba/bin/gen-motion-fluttabap360-reel.mjs               # gen all missing
//   node pop/marimba/bin/gen-motion-fluttabap360-reel.mjs --only molt --force  # re-roll
//   node pop/marimba/bin/gen-motion-fluttabap360-reel.mjs --assemble    # 46.2s loop cut + chrome
//   node pop/bin/audition-motion.mjs --lane marimba --slug fluttabap360-reel
...
const MEDIUM_MOTION = [
  `Diegetic light only — no glow filters, no neon, no motion-blur language, never cinematic, never a photograph. Hand-drawn colored-pencil + gouache on warm cream paper. Any human figure never looks at the camera. Motion is graceful; the painted-paper monarchs drift on their own gentle arcs.`,
...
`Extreme close-up on the splitting seam: the hand-cut paper edge TEARS slowly wider, fiber by fiber, flakes of painted paper peeling away and spinning down out of frame. The fat dew-drop clinging beside the seam acts as a tiny LENS — the warm light from inside the chrysalis refracting through it, splitting into painted glints that slide as the drop trembles. The sliver of pale-yellow fabric shifts inside. The camera pushes in slowly with a slight roll, tilting into the widening split.`,
  ...
  hatch: {
    dur: 4, exact: 2.7,
    motion:
`The keeper crawls slowly OUT of the split chrysalis along the thick branch, on hands and knees, gooey with birth: the glistening strands of pale-jade fluid stretching from his shoulders back to the husk sag, wobble and SNAP one by one as he pulls forward, fat drops refracting the morning sun as they fall away toward the lawn. His slicked-wet curls stay matted; the soaked paper-mache wings give one heavy shiver, flinging droplets that glint as they spin off. The cracked husk swings and slowly rotates behind him, fluid stringing from its torn lip. The camera tracks alongside with a gentle tilt and shift, keeping his lowered head away from view. Wet shine stays painted gouache — never a glow.`,
  },
  hatchpov: {
    dur: 4, exact: 3.0,
    motion:
`First-person POV, brand-new eyes (no face ever visible, no second figure anywhere): his own two hands grip the rough oak bark and pull forward hand over hand, knuckles flexing, a strand of jade goo on one wrist stretching and glinting in the sun. Beyond the hands the dawn park swims from blur into focus far below — dew refracting in tiny sparks across the lawn, the faint ground-grid resolving — and ONE monarch rises toward the camera-as-his-eyes, SPINNING slowly on its arc. The view sways and tilts with each hand-pull, like the first time a body is used.`,
  },
  growdance: {
    dur: 6, exact: 4.4,
    image: "grow",
    endImage: "dance",
    motion:
`One continuous shot, start frame to end frame: seen from behind, the keeper rises from his crouch to full height in one long unfurling stretch — spine straightening, arms spreading, the paper-mache monarch wings creaking OPEN to full span, drops of jade goo spinning off the wingtips — and the rise rolls WITHOUT A CUT into a full PIROUETTE: he spins on the ball of one foot, wings fanning bands of refracted sunlight, and lands out of the turn exactly in the ballet lift of the final frame — relevé, low arabesque, arms curved overhead. The camera ORBITS slowly counter to his spin. Three or four monarchs corkscrew upward around him, sharing the rotation. Joy arriving all at once.`,
  },
  laptops: {
    dur: 4, exact: 2.5,
    motion:
`Wide from behind and over his shoulder, his head in three-quarter profile: the keeper kneels at his arc of five different-colored open laptops and PLAYS them like a marimba with his BARE FINGERS — fingertips bouncing off the colored keycaps in a quick alternating pattern, wrists loose and percussive, hands hopping machine to machine — while he SINGS like a pop star into the antenna-mic: the bent-down antenna's felt bobble bobbing at his lips as his chin lifts and drops mid-note, mouth shaping the song, head rocking with the beat. The camera ORBITS slowly around the arc — and as the angle changes, the glossy sheen on each notepat screen slides and refracts across the tile grids, candy-colored tiles winking pressed-and-released chords. The monarchs perched along the screen-tops flutter up and resettle with each strike. His wings tip in time. Gleeful, belting, musical.`,
  },
  mallets: {
    dur: 4, exact: 2.75,
    motion:
`Extreme close-up: the keeper's bare fingertips BOUNCE off the colored keymap keycaps like a pair of marimba mallets — strike, rebound, strike, wrist loose — each hit brightening the matching candy-colored tile on the notepat screen, the grid winking through a chord. The screen's glossy sheen SPLITS and re-gathers around each bounce, painted glints sliding across the tilted glass. The camera tilts and shifts subtly with each strike, like a head keeping the beat. The monarch perched on the screen-top lip flutters and resettles in rhythm. The hand stays low in frame, face never shown.`,
  },
  laptopspov: {
    dur: 4, exact: 2.75,
    motion:
`First-person POV mid-performance (no face ever visible, no second figure anywhere): his own bare forearms and hands play down across the arc of five different-colored laptops — left fingertips striking one colored keyboard, right hand rebounding off another, alternating fast like a marimbist's mallets. The view BOBS and TILTS with the playing, and as it moves the sheen rolls in refracted bands across each screen, the notepat tile grids winking different chords under his fingers. The monarchs along the screen-tops hop and flutter with the rhythm. Flow-state loose, the whole instrument his.`,
  },
  molt: {
    dur: 6, exact: 5.4,
    motion:
`The transformation: facing away from camera beside his neat stack of closed laptops, the keeper's arms rise into one last port de bras — and his outline DISSOLVES upward into a swirl of orange-and-black wing fragments that SPINS around him like a slow rising cyclone, each fragment catching and refracting the gold light as it turns. The camera circles slowly COUNTER to the swirl's rotation. The fragments gather into a single LARGE monarch that lifts away spinning gently at the top of the frame, while the pale-yellow button-down settles soft and empty onto the lawn. Solemn wonder, not chaos. The new wings give their first full beat.`,
  },
  wing: {
    dur: 5, exact: 4.5,
    motion:
...
`The drone-monarch CORKSCREWS upward over the shrinking park — spinning slowly on its own axis as it climbs, wings beating, the sun sweeping turning bands of refracted light around the brushed-metal panels with every rotation, rivet dots glinting in sequence. Its six legs stay tucked neatly beneath the thorax, trailing back. The camera SPIRALS up with it, counter-rotating gently, the lawn and oak and faint ground-grid wheeling far below. The dotted signal-arc corkscrews behind it. The sky deepens toward indigo as they climb.`,
  ...
  spacepov: {
    dur: 4, exact: 3.5,
    motion:
`First-person POV through the drone-monarch's lens-eye (no human anywhere): the whole view BARREL-ROLLS slowly as the drone banks, its own brushed-metal wingtips framing the edges, starlight glinting and refracting off the panels in sliding sparks with each wingbeat. Between the wings Earth — the small blue-green hand-hatched marble — arcs gracefully across the rolling frame, the faint orbit-lines curving past. Stars stay fixed like bare paper under a rostrum camera. Vast, calm, leaving home.`,
  },
  space: {
    dur: 5, exact: 4.2,
    motion:
...
if (!flags.assemble) {
  ...
  for (const s of shots) {
    for (const p of [s.image, s.endImage].filter(Boolean)) {
      if (!existsSync(p)) { console.error(`✗ panel missing: ${p} — run gen-sections-fluttabap360-reel.mjs first`); process.exit(1); }
    }
  }
...
// Emits a chrome-less BASE at 1080×1920@30 plus meta-fluttabap360-reel.json,
// then hands off to chrome-reel-fluttabap360.mjs. CUT shots open on their
// input panel — trim the TAIL. MORPH shots arrive on their end panel in
// their final frames — trim the HEAD so the arrival lands whole.
...
const chrome = spawnSync("node", [`${HERE}/chrome-reel-fluttabap360.mjs`], { stdio: ["ignore", "inherit", "inherit"] });
