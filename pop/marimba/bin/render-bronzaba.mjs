...
// bronzaba — A minor pentatonic note table (pitches A C D E G only).
// A clean pentatonic ladder; melody/bass below are rewritten to use
// these names directly (no F-major leftovers).
const N = {
  A1: 33, A2: 45, C3: 48, D3: 50, E3: 52, G3: 55,
  A3: 57, C4: 60, D4: 62, E4: 64, G4: 67,
  A4: 69, C5: 72, D5: 74, E5: 76, G5: 79,
  A5: 81, C6: 84, D6: 86, E6: 88, G6: 91,
};
...
// ── [hush hush] — bars 0..3 — circular pentatonic drift on Am ─────────
// rosewood traces the bronzaba loop-cell A→C→D→E settling on the tonic
events.push(v("rosewood", 0, 0, N.A4, 1.0, 0.55));
events.push(v("rosewood", 0, 1, N.C5, 1.0, 0.55));
events.push(v("rosewood", 0, 2, N.D5, 1.0, 0.55));
events.push(v("rosewood", 1, 0, N.E5, 3.0, 0.45));
events.push(v("rosewood", 2, 0, N.D5, 1.5, 0.55));
events.push(v("rosewood", 2, 1.5, N.C5, 1.5, 0.55));
events.push(v("rosewood", 3, 0, N.A4, 3.0, 0.55));

// bass: gentle low A rocker on beat 1 every bar (Am tonic)
for (let b = 0; b < 4; b++) {
  events.push(v("bass", b, 0, N.A1, 3.0, 0.55));
}

// ── [twinkle] — bars 4..9 — hypnotic circular ostinato (Am→G) ────────
// the bronzaba loop-cell, repeated and rotated: a high pentatonic
// gamelan curl that turns back on itself every bar.
events.push(v("rosewood", 4, 0, N.E5, 1.0, 0.60));
events.push(v("rosewood", 4, 1, N.G5, 1.0, 0.60));
events.push(v("rosewood", 4, 2.0, N.A5, 0.5, 0.65));
events.push(v("rosewood", 4, 2.5, N.G5, 0.5, 0.55));
events.push(v("rosewood", 5, 0, N.E5, 3.0, 0.55));
events.push(v("rosewood", 6, 0, N.A5, 1.0, 0.55));
events.push(v("rosewood", 6, 1, N.G5, 1.0, 0.55));
events.push(v("rosewood", 6, 2.0, N.E5, 0.5, 0.60));
events.push(v("rosewood", 6, 2.5, N.G5, 0.5, 0.50));
events.push(v("rosewood", 7, 0, N.D5, 3.0, 0.55));   // chord turns to G
events.push(v("rosewood", 8, 0, N.G5, 1.0, 0.60));
events.push(v("rosewood", 8, 1, N.D6, 1.0, 0.65));
events.push(v("rosewood", 8, 2, N.G5, 1.0, 0.55));
events.push(v("rosewood", 9, 0, N.A5, 1.5, 0.60));
events.push(v("rosewood", 9, 1.5, N.G5, 1.5, 0.55));

// bass pulse — A-A then G-G, the slow i↔♭VII rock
const twinkleBass = [N.A1, N.A2, N.A1, N.A2, N.A1, N.A2];
for (let i = 0; i < 6; i++) {
  events.push(v("bass", 4 + i, 0, twinkleBass[i], 3.0, 0.45));
}

// vibraphone_off pad — Am sustained over bars 4-6, G major over 7-9
events.push({ preset: "vibraphone_off", startSec: 4 * BAR, midi: N.A3, durSec: 3 * BAR, gain: 0.18 });
...
events.push({ preset: "vibraphone_off", startSec: 7 * BAR, midi: N.G3, durSec: 3 * BAR, gain: 0.18 });
...
// vibraphone WITH motor for the "wow" wobble feel — G major dyad
events.push({ preset: "vibraphone", startSec: 10 * BAR, midi: N.D5, durSec: 2 * BAR, gain: 0.45 });
events.push({ preset: "vibraphone", startSec: 10 * BAR, midi: N.G5, durSec: 2 * BAR, gain: 0.40 });
events.push(v("rosewood", 12, 0, N.G5, 1.0, 0.55));
events.push(v("rosewood", 12, 1, N.E5, 1.0, 0.55));
events.push(v("rosewood", 12, 2, N.G5, 1.0, 0.55));
events.push(v("rosewood", 13, 0, N.A5, 3.0, 0.55));

// kalimba sparkles overlaid on the wow — soft, off-beat ornaments
events.push(v("kalimba", 10, 2.5, N.D6, 0.5, 0.30));
events.push(v("kalimba", 11, 1.0, N.E6, 0.5, 0.25));
events.push(v("kalimba", 12, 2.5, N.C6, 0.5, 0.30));
events.push(v("kalimba", 13, 1.5, N.A5, 0.5, 0.25));

// quiet bass under the wow
events.push(v("bass", 10, 0, N.G3, 3.0, 0.35));
events.push(v("bass", 12, 0, N.G3, 3.0, 0.35));

// ratatatata — a fast staccato 16th-note tumble across bar 11, a
// playful machine-gun burst against the slow held vibraphone.
const wowRatata = [N.G5, N.A5, N.G5, N.E5, N.D5, N.E5, N.G5, N.A5];
for (let i = 0; i < wowRatata.length; i++) {
  events.push(v("staccato", 11, i * 0.1875, wowRatata[i], 0.22, 0.34));
}

// ── [ba-ba-ba bap] — bars 14..17 — slinky pentatonic wobble ──────────
...
events.push(v("rosewood", 14, 2.0, N.E5, 1.0, 0.65));
events.push(v("rosewood", 15, 0, N.D6, 0.5, 0.60));
events.push(v("rosewood", 15, 0.5, N.C6, 0.5, 0.55));
events.push(v("rosewood", 15, 1.0, N.D6, 1.0, 0.60));
events.push(v("rosewood", 15, 2.0, N.A5, 1.0, 0.65));
events.push(v("rosewood", 16, 0, N.G5, 1.0, 0.55));
events.push(v("rosewood", 16, 1, N.E5, 1.0, 0.55));
events.push(v("rosewood", 16, 2, N.D5, 1.0, 0.50));
events.push(v("rosewood", 17, 0, N.A5, 3.0, 0.55));

// kalimba doubles the "bap" on bars 14, 15 (Octave up sparkle)
events.push(v("kalimba", 14, 2, N.E6, 1.0, 0.28));
events.push(v("kalimba", 15, 2, N.A5, 1.0, 0.28));

// bass: slow i↔♭VII rock — A, G, A, G
const babaBass = [N.A1, N.G3, N.A1, N.G3];
for (let i = 0; i < 4; i++) {
...
run16(9, 0, [N.A5, N.G5, N.E5, N.D5, N.C5, N.A4,
             N.C5, N.D5, N.E5, N.G5, N.A5, N.C6], 0.30);
// bar 13 — ascending 16ths spilling out of the wow
run16(13, 0, [N.E5, N.G5, N.A5, N.C6, N.D6, N.C6,
              N.A5, N.G5, N.E5, N.G5, N.A5, N.C6], 0.28);
// bar 21 — fast 16ths winding down into the ending cadence
run16(21, 0, [N.A5, N.G5, N.E5, N.D5, N.C5, N.A4,
              N.G4, N.E4, N.D4, N.E4, N.G4, N.A4], 0.26);

// ── [sleep now] — bars 18..23 — settling pentatonic descent ──────────
events.push(v("rosewood", 18, 0, N.E5, 1.5, 0.45));
events.push(v("rosewood", 18, 1.5, N.D5, 1.5, 0.40));
events.push(v("rosewood", 19, 0, N.C5, 1.0, 0.40));
events.push(v("rosewood", 19, 1, N.A4, 1.0, 0.40));
events.push(v("rosewood", 19, 2, N.A4, 1.0, 0.35));
events.push(v("rosewood", 20, 0, N.D5, 1.0, 0.40));
events.push(v("rosewood", 20, 1, N.C5, 1.0, 0.35));
events.push(v("rosewood", 20, 2, N.A4, 1.0, 0.35));
events.push(v("rosewood", 21, 0, N.A4, 3.0, 0.45));

// kalimba — one last twinkle, then silence
events.push(v("kalimba", 19, 2.5, N.C6, 0.5, 0.18));
events.push(v("kalimba", 21, 1.0, N.E5, 0.5, 0.15));

// soft bass alternating through the descent — A then G
events.push(v("bass", 18, 0, N.A1, 3.0, 0.35));
events.push(v("bass", 20, 0, N.G3, 3.0, 0.35));
...
events.push(v("rosewood", 22, 0,   N.D5, 1.0, 0.45));
events.push(v("rosewood", 22, 1,   N.C5, 1.0, 0.45));
events.push(v("rosewood", 22, 2,   N.A4, 1.0, 0.45));
events.push(v("rosewood", 23, 0,   N.E5, 1.5, 0.42));
// ratatatata — fast staccato 16th run scooping up to the cadence
const flourish = [N.A4, N.C5, N.D5, N.E5, N.G5, N.A5, N.C6, N.D6];
for (let i = 0; i < flourish.length; i++) {
  events.push(v("staccato", 23, 1.5 + i * 0.1875, flourish[i], 0.22, 0.40));
}
// bar 24-25: final rolled A minor chord — staggered entries 30ms apart
const finalChord = [N.A1, N.A2, N.C4, N.E4, N.A4, N.C5, N.E5, N.A5];
...
// vibraphone_off halo under the final chord — A minor triad
for (const m of [N.A2, N.C4, N.E4, N.A4]) {
  events.push({ preset: "vibraphone_off", startSec: 24 * BAR, midi: m,
                durSec: 2 * BAR, gain: 0.16, decayMul: 1.8 });
}
events.push(v("bass", 22, 0, N.A1, 3.0, 0.34));
...
pushPad(0, 4,    [N.A3, N.C4, N.E4]);              // bars 0..3   Am
pushPad(10, 4,   [N.G3, N.D4, N.G4]);              // bars 10..13 G
pushPad(14, 1,   [N.A3, N.C4, N.E4]);              // bar 14      Am
pushPad(15, 1,   [N.G3, N.D4, N.G4]);              // bar 15      G
pushPad(16, 2,   [N.A3, N.C4, N.E4]);              // bars 16..17 Am
pushPad(18, 4,   [N.A3, N.C4, N.E4]);              // bars 18..21 Am
...
// chord = [root, third, fifth]; bronzaba rocks Am ↔ G
pushOsti(2,  [N.A4, N.C5, N.E5]);    // Am
pushOsti(3,  [N.A4, N.C5, N.E5]);
pushOsti(4,  [N.A4, N.C5, N.E5]);
pushOsti(5,  [N.A4, N.C5, N.E5]);
pushOsti(6,  [N.A4, N.C5, N.E5]);
pushOsti(7,  [N.G4, N.D5, N.G5]);    // G
pushOsti(8,  [N.G4, N.D5, N.G5]);
pushOsti(9,  [N.G4, N.D5, N.G5]);
// bars 10–13 — leave the wow section spacious, no ostinato
pushOsti(14, [N.A4, N.C5, N.E5]);    // Am
pushOsti(15, [N.G4, N.D5, N.G5]);    // G
pushOsti(16, [N.A4, N.C5, N.E5]);    // Am
pushOsti(17, [N.A4, N.C5, N.E5]);
pushOsti(18, [N.A4, N.C5, N.E5]);
pushOsti(19, [N.A4, N.C5, N.E5]);
// bars 20–21 fade out the ostinato gradually
events.push(v("kalimba", 20, 0, N.A4, 0.9, OSTI_GAIN * 0.6));
events.push(v("kalimba", 20, 2, N.C5, 0.9, OSTI_GAIN * 0.45));
events.push(v("kalimba", 21, 1, N.A4, 0.9, OSTI_GAIN * 0.3));
...
const connectors = [
  // bar 1 hush tail: hum A4 → C5 → A4 (gentle breath shape)
  { startBar: 1, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN,
    points: [[0.0, N.A4], [1.2, N.C5], [2.5, N.A4]] },
  // bar 3 hush tail: A4 sustained, drifting up a tiny bit then back
  { startBar: 3, startBeat: 0.5, durBeats: 2.5, gain: SINE_GAIN,
    points: [[0.0, N.A4], [1.5, N.C5], [2.5, N.A4]] },
...
// bronzaba — slow hypnotic two-chord rock: A minor (i) ↔ G major (♭VII)
// root, fifth, octave, third — low register
setChord(0, 6,    33, 40, 45, 48);   // A1 E2 A2 C3 — A minor
setChord(7, 9,    31, 38, 43, 47);   // G1 D2 G2 B2 — G major
setChord(10, 13,  31, 38, 43, 47);   // G major
setChord(14, 14,  33, 40, 45, 48);   // A minor
setChord(15, 15,  31, 38, 43, 47);   // G major
setChord(16, 17,  33, 40, 45, 48);   // A minor
setChord(18, 21,  33, 40, 45, 48);   // A minor (sleep)
setChord(22, TOTAL_BARS - 1, 33, 40, 45, 48);   // A minor (goodnight + tail)
...
const voiceMelody = [
  ...
  // twinkle bars 4-9 — the bronzaba loop-cell sung
  { bar: 4,  beat: 0,   midi: N.E5,  durBeats: 1.0, v: "a" },
  { bar: 4,  beat: 1,   midi: N.G5,  durBeats: 1.0, v: "a" },
  { bar: 4,  beat: 2.0, midi: N.A5,  durBeats: 0.5, v: "a" },
  { bar: 4,  beat: 2.5, midi: N.G5,  durBeats: 0.5, v: "a" },
  { bar: 5,  beat: 0,   midi: N.E5,  durBeats: 3.0, v: "a" },
  { bar: 6,  beat: 0,   midi: N.A5,  durBeats: 1.0, v: "a" },
  { bar: 6,  beat: 1,   midi: N.G5,  durBeats: 1.0, v: "a" },
  { bar: 6,  beat: 2.0, midi: N.E5,  durBeats: 0.5, v: "a" },
  { bar: 6,  beat: 2.5, midi: N.G5,  durBeats: 0.5, v: "a" },
  { bar: 7,  beat: 0,   midi: N.D5,  durBeats: 3.0, v: "a" },
  { bar: 8,  beat: 0,   midi: N.G5,  durBeats: 1.0, v: "a" },
  { bar: 8,  beat: 1,   midi: N.D6,  durBeats: 1.0, v: "a" },
  { bar: 8,  beat: 2,   midi: N.G5,  durBeats: 1.0, v: "a" },
  { bar: 9,  beat: 0,   midi: N.A5,  durBeats: 1.5, v: "a" },
  { bar: 9,  beat: 1.5, midi: N.G5,  durBeats: 1.5, v: "a" },
  // wow bars 12-13 — "woo woo woo whoa"
  { bar: 12, beat: 0,   midi: N.G5,  durBeats: 1.0, v: "u" },
  { bar: 12, beat: 1,   midi: N.E5,  durBeats: 1.0, v: "u" },
  { bar: 12, beat: 2,   midi: N.G5,  durBeats: 1.0, v: "u" },
  { bar: 13, beat: 0,   midi: N.A5,  durBeats: 3.0, v: "o" },
  // baba bars 14-17
  ...
  { bar: 14, beat: 2.0, midi: N.E5,  durBeats: 1.0, v: "a" },
  { bar: 15, beat: 0,   midi: N.D6,  durBeats: 0.5, v: "a" },
  { bar: 15, beat: 0.5, midi: N.C6,  durBeats: 0.5, v: "a" },
  { bar: 15, beat: 1.0, midi: N.D6,  durBeats: 1.0, v: "a" },
  { bar: 15, beat: 2.0, midi: N.A5,  durBeats: 1.0, v: "a" },
  { bar: 16, beat: 0,   midi: N.G5,  durBeats: 1.0, v: "a" },
  { bar: 16, beat: 1,   midi: N.E5,  durBeats: 1.0, v: "a" },
  { bar: 16, beat: 2,   midi: N.D5,  durBeats: 1.0, v: "a" },
  { bar: 17, beat: 0,   midi: N.A5,  durBeats: 3.0, v: "a" },
  // sleep bars 18-21
  { bar: 18, beat: 0,   midi: N.E5,  durBeats: 1.5, v: "a" },
  { bar: 18, beat: 1.5, midi: N.D5,  durBeats: 1.5, v: "a" },
  { bar: 19, beat: 0,   midi: N.C5,  durBeats: 1.0, v: "a" },
  { bar: 19, beat: 1,   midi: N.A4,  durBeats: 1.0, v: "a" },
  { bar: 19, beat: 2,   midi: N.A4,  durBeats: 1.0, v: "a" },
  { bar: 20, beat: 0,   midi: N.D5,  durBeats: 1.0, v: "a" },
  { bar: 20, beat: 1,   midi: N.C5,  durBeats: 1.0, v: "a" },
  { bar: 20, beat: 2,   midi: N.A4,  durBeats: 1.0, v: "a" },
  { bar: 21, beat: 0,   midi: N.A4,  durBeats: 3.0, v: "a" },
  // ending bars 22-23 — closing "ooh" echo of the loop-cell
  { bar: 22, beat: 0,   midi: N.D5,  durBeats: 1.0, v: "o" },
  { bar: 22, beat: 1,   midi: N.C5,  durBeats: 1.0, v: "o" },
  { bar: 22, beat: 2,   midi: N.A4,  durBeats: 1.0, v: "o" },
  { bar: 23, beat: 0,   midi: N.A4,  durBeats: 3.0, v: "o" },
];
...
{
  const falls = [
    { bar: 13, beat: 0,   fromMidi: N.D6, toMidi: N.A4, durBeats: 2.4, gain: 0.26, pan: +0.2 },
  ];
