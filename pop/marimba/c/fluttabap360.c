// fluttabap360.c — C renderer for the fluttabap360 pop track. Mirrors the
...
//
// Usage:  fluttabap360 <score.txt> --raw <out.f32> [--solo a,b]
...
static double gTargetSec = 360.0;     // hard form clamp
...
static double scene_amt(const char *sec, const char *v) {
  ...
  if (S("intro"))           { if(V("bell"))return .34; if(V("spark"))return .20; if(V("vib"))return .20; if(V("pad"))return .34; if(V("lead"))return .12; if(V("rise"))return .18; if(V("scream"))return .18; if(V("flute"))return .40; }
  else if (S("butterfly"))  { if(V("bell"))return .18; if(V("spark"))return .13; if(V("lead"))return .07; if(V("scream"))return .16; if(V("flute"))return .26; }
  else if (S("palofmine"))  { if(V("bell"))return .16; if(V("spark"))return .11; if(V("lead"))return .07; if(V("flute"))return .26; }
  else if (S("mommywow"))   { if(V("bell"))return .38; if(V("pad"))return .34; if(V("vib"))return .18; if(V("lead"))return .12; if(V("spark"))return .16; if(V("flute"))return .42; }
  else if (S("slinky"))     { if(V("scream"))return .14; if(V("flute"))return .22; }
  else if (S("fly"))        { if(V("bell"))return .08; if(V("flute"))return .24; }
  else if (S("ride"))       { if(V("scream"))return .38; if(V("scr"))return .34; if(V("bell"))return .10; if(V("rise"))return .14; }
  else if (S("cave"))       { if(V("scream"))return .28; if(V("vib"))return .20; if(V("pad"))return .22; if(V("lead"))return .10; if(V("flute"))return .34; }
  else if (S("progression")){ if(V("bell"))return .15; if(V("spark"))return .11; if(V("lead"))return .07; }
  else if (S("land"))       { if(V("bell"))return .24; if(V("pad"))return .20; if(V("lead"))return .10; }
  else if (S("button"))     { if(V("bell"))return .30; if(V("pad"))return .24; }
  #undef S
  #undef V
  return 0.0;
}
...
static void synth_pad(double start, double dur, double g, int nm, const double *midis,
                      double *bus) {
  const double dt = 1.0 / SR;
  long n0 = (long)floor(start * SR);
  long len = (long)ceil((dur + 0.5) * SR);
  long att = (long)floor(dur * 0.42 * SR); if (att < 1) att = 1;
...
int main(int argc, char **argv) {
  ...
  if (!scorePath || !rawPath) { fprintf(stderr, "usage: fluttabap360 <score.txt> --raw <out.f32> [--loop] [--solo a,b]\n"); return 1; }
  ...
  while (fgets(line, sizeof line, f)) {
    ...
    if (sscanf(line, "sub %lf %lf %lf %lf", &a, &b, &c, &d) == 4) {
      ...
      for (int j = 0; ok && j < nm; j++) { char *tt = strtok_r(NULL," \n",&save); if (!tt) { ok = 0; break; } midis[j] = atof(tt); }
      if (ok) { if (wanted("pad")) { synth_pad(st,du,gn,nm,midis, busPad); evN++; } } else skipN++;
  ...
  if (soloCount >= 0) {
    ...
    // FULL MIX — exact order + DSP from the JS render.
    place(busLead,   0.00, 1.00);
    place(busFlute,  0.16, 0.90);   // the flute — fluttabap finally has its flute
    place(busBass,   0.00, 0.66);
    place(busVib,   -0.24, 0.88);
    place(busDeep,   0.00, 0.92);
    place(busRise,   0.00, 0.85);
    place(busScr,    0.34, 0.60);
    place(busScream,-0.48, 0.48);
    place(busSpark,  0.44, 0.86);
    place(busBell,  -0.42, 0.84);
    place(busKick,   0.00, 1.02);
    place(busSub,    0.00, 1.00);
    place(busSnare,  0.04, 0.80);
    place(busHat,    0.34, 0.56);
    place(busShak,  -0.34, 0.48);
    reverb_in_place(busCave, NS, 0.50, 0.74);
    place(busCave,   0.00, 0.92);
    reverb_in_place(busStream, NS, 0.28, 0.55);
    place(busStream, -0.30, 0.30);
    place(busStream,  0.30, 0.30);
    ...
    reverb_in_place(spaceSend, NS, 1.0, 0.70);
  ...
  if (soloCount < 0) {
    ...
    if (!loopMode) {
      ...
      long fadeIn = (long)floor(6.0 * SR), fadeOut = (long)floor(1.6 * SR);
      for (long i = 0; i < fadeIn && i < trimN; i++) { double gg = pow((double)i / fadeIn, 1.7); outL[i] *= (float)gg; outR[i] *= (float)gg; }
      ...
      const double FLOOR = 0.45; long swell = (long)floor(3.5 * SR);
  ...
  fprintf(stderr, "[fluttabap360] %ld events (%ld unported skipped) → %ld frames\n", evN, skipN, outN);
  return 0;
}
