#!/usr/bin/env node
// hellsine/bin/preview-score.mjs — storyline visualizer for hellsine.
//
...
for (let i = 0; i < SECTIONS.length; i++) {
  ...
  const p = `${LANE}/out/${SLUG}-p-sec-${pad2(i)}-${safeName(SECTIONS[i].name)}.png`;
