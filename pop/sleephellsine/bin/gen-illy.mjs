#!/usr/bin/env node
// sleephellsine/bin/gen-illy.mjs — sleephellsine album cover. The
// hellsine group portrait recomposed as a kawaii peaceful FROZEN ice
// dreamscape: sleeping pixsies, ice-sine channels, snowflakes, moonlight,
// frosted bullet train, ice-sculpted horse. Pure lullaby winter dream.
//
// Prompt: pop/sleephellsine/sleephellsine.illy.txt
// Output: pop/sleephellsine/out/sleephellsine-cover.png
// Embed:  pop/sleephellsine/out/sleephellsine.mp3   (ID3v2 attached picture)
//
// Usage:
//   node pop/sleephellsine/bin/gen-illy.mjs            # cached if cover exists
//   node pop/sleephellsine/bin/gen-illy.mjs --force    # regenerate
...
const PROMPT_PATH = resolve(process.cwd(), _af("--prompt", `${LANE}/sleephellsine.illy.txt`));
const OUT_PATH    = resolve(process.cwd(), _af("--cover",  `${LANE}/out/sleephellsine-cover.png`));
const MP3_PATH    = resolve(process.cwd(), _af("--mp3",    `${LANE}/out/sleephellsine.mp3`));
mkdirSync(`${LANE}/out`, { recursive: true });

// jeffrey identity refs — same set as the other singles in the pixsies
// series. The model uses these to keep all the sleeping pixsies on-model.
...
console.log(`▸ sleephellsine cover · ${SIZE} · ${REFS.length} refs`);
const t0 = Date.now();
