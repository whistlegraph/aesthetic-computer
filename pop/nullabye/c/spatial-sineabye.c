...
static int presentationMode=0; // closer orbiting overview for room-scale playback
static int cylinderMode=0; // high-resolution light score tube, viewed side-on
...
// Presentation view score tubes. Upcoming events rise from below each source,
// cross its spectral body at onset, then accelerate upward as emitted notes.
// Ring radius is logarithmic pitch and the axial tail is authored duration.
static void note_ring3(unsigned char*p,V3 c,double r,V3 cam,V3 target,uint32_t color,double alpha){V3 first={c.x+r,c.y,c.z},prev=first;for(int i=1;i<=12;i++){double a=TAU*i/12;V3 q={c.x+cos(a)*r,c.y+sin(a)*r,c.z};line3(p,prev,q,cam,target,color,alpha);prev=q;} }
static void note_stream(unsigned char*p,int src,double t,V3 c,V3 cam,V3 target){double lookAhead=cylinderMode?12.0:4.2,after=cylinderMode?9.0:2.25,below=cylinderMode?14.0:3.6,above=cylinderMode?14.0:3.4,tubeRadius=cylinderMode?.18:.24;uint32_t base=S[src].color;
 for(int k=0;k<3;k++){double a=TAU*k/3+src*.19,x=cos(a)*tubeRadius,y=sin(a)*tubeRadius;line3(p,(V3){c.x+x,c.y+y,c.z-below},(V3){c.x+x,c.y+y,c.z+above},cam,target,base,.075);}
 for(int j=0;j<NE;j++){Event*e=&E[j];if(e->src!=src||e->g<.004)continue;double dt=e->t-t,flow,fade;if(dt>0&&dt<=lookAhead){flow=dt/lookAhead;flow=flow*flow*(3-2*flow);flow=-below*flow;fade=.22+.62*(1-dt/lookAhead);}else if(dt<=0&&dt>=-after){double u=-dt/after;flow=above*(1-pow(1-u,3));fade=.78*(1-u)*(1-u);}else continue;
  double f=sqrt(fmax(45,e->f0)*fmax(45,e->f1)),pitch=fmax(0,fmin(1,log(f/45.0)/log(14000.0/45.0))),r=.10+.18*pitch,tail=.10+.30*fmin(2.0,e->dur),amp=fmin(1.0,.24+1.55*sqrt(fabs(e->g)));uint32_t col=shade_color(base,.68+.48*pitch);V3 q={c.x,c.y,c.z+flow};note_ring3(p,q,r,cam,target,col,fade*amp);line3(p,(V3){q.x,q.y,q.z-tail},q,cam,target,col,fade*amp*.82);P2 qp=project(q,cam,target);if(qp.ok){int rr=dt<=0?3:2;glow(p,qp.x,qp.y,rr*3,col,.05+fade*amp*.09);dot(p,qp.x,qp.y,rr,col);}if(fabs(dt)<.055){P2 cp=project(c,cam,target);if(cp.ok)glow(p,cp.x,cp.y,20,col,.10*amp);}
 }
}
...
static void sound_shells(unsigned char*p,int src,int fr,V3 cam,V3 target,double spread,double cx,double cy){for(int q=1;q<=6;q++){int pf=fr-q*5;if(pf<0)continue;double ml,mr;source_meter(pf,src,&ml,&mr);double e=hypot(ml,mr);if(e<.00008)continue;double age=(fr-pf)/(double)FPS,et=pf/(double)FPS,fade=(1-age/1.35);fade*=fade;Source so=source_at(src,et);V3 c={cx+(so.x-cx)*spread,cy+(so.y-cy)*spread,so.z};double r=.16+age*2.35,a=(.025+fmin(.20,e*15))*fade;uint32_t col=shade_color(S[src].color,.72+q*.055);const float*wave=&sourceWave[(pf*NSRC+src)*WAVE_POINTS];double peak=0;for(int i=0;i<WAVE_POINTS;i++)peak=fmax(peak,fabs(wave[i]));wave_ring3plane(p,c,r,0,wave,peak,cam,target,col,a);wave_ring3plane(p,c,r,1,wave,peak,cam,target,col,a*.78);wave_ring3plane(p,c,r,2,wave,peak,cam,target,col,a*.62);}}
...
static void video(const char*wavp,const char*outp,double videoStart,double videoDuration){char cmd[2048];size_t ol=strlen(outp);int lossless=ol>=4&&!strcmp(outp+ol-4,".mov");const char*clean=globeMode?"-af 'highpass=f=30,equalizer=f=7600:t=q:w=.75:g=-4,lowpass=f=11800,alimiter=limit=.90:attack=6:release=110,volume=.88'":"",*pixels=(globeMode&&!acousticsView)?"-vf 'scale=240:240:flags=neighbor,scale=720:720:flags=neighbor'":"";if(lossless)
 ...
 int videoFrames=(int)floor(videoDuration*FPS+.5);for(int outFr=0;outFr<videoFrames;outFr++){double t=videoStart+outFr/(double)FPS;int fr=(int)floor(t*FPS);for(int y=0;y<H;y++){unsigned char v=brightMode?(unsigned char)(250-15*y/(double)H):(unsigned char)(24-10*y/(double)H);for(int x=0;x<W;x++){int o=(y*W+x)*3;if(cylinderMode){p[o]=(unsigned char)(247-5*y/(double)H);p[o+1]=(unsigned char)(242-3*y/(double)H);p[o+2]=(unsigned char)(249-2*y/(double)H);}else{p[o]=v;p[o+1]=(unsigned char)fmax(0,v-(brightMode?3:1));p[o+2]=(unsigned char)fmin(255,v+(brightMode?0:2));}}}int li=(int)(t*CTRL);Listener l=L[li];
  if(globeMode){globe_frame(p,fr,t,l);fwrite(p,1,W*H*3,ff);continue;}
  double cylinderProgress=cylinderMode?fmax(0,fmin(1,(t-videoStart)/fmax(.001,videoDuration))):0;
  projectionRoll=cylinderMode?M_PI*.5+.26*sin(TAU*cylinderProgress*3):0;
  ...
  V3 cam,target;double visualCx=0,visualCy=0,visualSpread=1;if(cameraMode){cam=(V3){l.x,l.y,1.62};target=(V3){la.x+(lb.x-la.x)*lookMix,la.y+(lb.y-la.y)*lookMix,la.z+(lb.z-la.z)*lookMix};}
  else{double cx=l.x,cy=l.y,cz=1.6;for(int s=0;s<NSRC;s++){Source q=source_at(s,t);cx+=q.x;cy+=q.y;cz+=q.z;}cx/=NSRC+1;cy/=NSRC+1;cz/=NSRC+1;visualCx=cx;visualCy=cy;if(presentationMode){double extent=0;for(int s=0;s<NSRC;s++){Source q=source_at(s,t);extent=fmax(extent,hypot(q.x-cx,q.y-cy));}visualSpread=fmax(1,fmin(3.4,4.6/fmax(.01,extent)));double displayExtent=extent*visualSpread,elapsed=t-videoStart,u,orbit,zoff;if(cylinderMode){double progress=cylinderProgress,elevation=.08+1.18*pow(sin(TAU*progress),2),baseDistance=fmax(9.5,fmin(15.5,displayExtent*1.55+4.2)),distance=baseDistance+6.2*sin(elevation);orbit=-.78+TAU*(1.65*progress+.055*sin(TAU*progress*4));double radius=distance*cos(elevation);zoff=distance*sin(elevation);cam=(V3){cx+radius*cos(orbit),cy+radius*sin(orbit),cz+zoff};target=(V3){cx,cy,cz};}else{if(elapsed<22){u=smooth01(elapsed/22);orbit=-.95+.40*u;zoff=6.6-1.2*u;}else if(elapsed<38){u=smooth01((elapsed-22)/16);orbit=-.55+.85*u;zoff=5.4+5.4*u;}else if(elapsed<56){u=smooth01((elapsed-38)/18);orbit=.30+1.10*u;zoff=10.8-1.3*u;}else if(elapsed<76){u=smooth01((elapsed-56)/20);orbit=1.40+.85*u;zoff=9.5-3.7*u;}else{double side=l.heading+M_PI*.5;while(side-2.25>M_PI)side-=TAU;while(side-2.25<-M_PI)side+=TAU;u=smooth01(fmin(1,(elapsed-76)/10));orbit=2.25+(side-2.25)*u;zoff=5.8-u;}double radius=fmax(6.5,fmin(13.2,displayExtent*1.45+2.4));cam=(V3){cx+radius*cos(orbit),cy+radius*sin(orbit),cz+zoff};target=(V3){cx,cy,cz};}}else{double orbitTime=ending_motion_time(t),orbit=-.72+.16*sin(orbitTime/19.0);cam=(V3){cx+19*cos(orbit),cy+19*sin(orbit),cz+12.5};target=(V3){cx,cy,cz*.55};}}
  ...
  // The ordinary overview retains its spatial scaffolding. Presentation mode
  // lets the authored bodies, score tubes, and measured waves carry the view.
  if(!presentationMode)for(int q=-8;q<=8;q++){line3(p,(V3){q,-8,0},(V3){q,8,0},cam,target,brightMode?0x9eafb2:0x536069,brightMode?.42:.28);line3(p,(V3){-8,q,0},(V3){8,q,0},cam,target,brightMode?0x9eafb2:0x536069,brightMode?.42:.28);}
  ...
  if(!presentationMode){for(int q=0;q<=5;q++)line3(p,(V3){1.2,-3,q*.48},(V3){1.2,2.5,q*.48},cam,target,0x806f68,.62);for(int q=0;q<=6;q++)line3(p,(V3){1.2,-3+q*.92,0},(V3){1.2,-3+q*.92,2.4},cam,target,0x806f68,.62);for(int q=0;q<42;q++){double rx=-8+fmod(q*3.71+t*.7,16),ry=-8+fmod(q*6.13+t*.43,16),rz=.3+fmod(q*1.77-t*1.9,5.5);line3(p,(V3){rx,ry,rz},(V3){rx-.12,ry-.08,rz-.38},cam,target,0x8faab5,.18);}}
  V3 lv={l.x,l.y,1.6};
  // Six-second fading breadcrumb: the path that the two follower voices chase.
  if(!cylinderMode)for(int q=1;q<=36;q++){int ia=li-(q-1)*CTRL/6,ib=li-q*CTRL/6;if(ib<0)break;V3 a={L[ia].x,L[ia].y,1.58},b={L[ib].x,L[ib].y,1.58};line3(p,a,b,cam,target,0xff6b9d,.42*(1-q/37.0));}
  for(int s=0;s<NSRC;s++){double ml,mr;source_meter(fr,s,&ml,&mr);double heard=hypot(ml,mr);Source so=source_at(s,t);if(presentationMode){so.x=visualCx+(so.x-visualCx)*visualSpread;so.y=visualCy+(so.y-visualCy)*visualSpread;}V3 sv={so.x,so.y,so.z};P2 sp=project(sv,cam,target);double gl,gr,dd;gains(s,t,&gl,&gr,&dd);
   ...
   if(!presentationMode)line3(p,lv,sv,cam,target,0x778087,.18);if(presentationMode)note_stream(p,s,t,sv,cam,target);if(!presentationMode){circle3(p,(V3){so.x,so.y,.025},1.35,cam,target,S[s].color,s==ts?.34:.08);if(s==ts&&t<104)circle3(p,(V3){so.x,so.y,.03},4.05,cam,target,S[s].color,.2);}
   sound_shells(p,s,fr,cam,target,visualSpread,visualCx,visualCy);
   ...
   {double trail=.18+fmin(.9,dd/12);V3 prev=sv;for(int q=1;q<=16;q++){double pt=fmax(0,t-q*trail/16),fade=(1-q/17.0)*(.08+fmin(.55,heard*18));Source old=source_at(s,pt);V3 ov={visualCx+(old.x-visualCx)*visualSpread,visualCy+(old.y-visualCy)*visualSpread,old.z};line3(p,prev,ov,cam,target,S[s].color,fade);prev=ov;}}
   if(!presentationMode&&heard>.00005)line3(p,lv,sv,cam,target,S[s].color,.03+fmin(.84,heard*28));
   ...
   spectral_volume(p,s,fr,t,sv,cam,target,s==ts);if(!cameraMode&&!presentationMode&&sp.ok)text3(p,sp.x+9,sp.y-7,S[s].name,1,brightMode?0x33484c:0xd5e5e2);
  }
  if(cameraMode){
