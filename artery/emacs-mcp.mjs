...
// toolchain/mcp/install-daemons.sh, which also points Claude at it with a
// local-scope http entry. handleMessage is stateless per call.
const port = httpPort(process.argv, 7766);
if (port) serveHttp({ handleMessage, port, banner: "🧠 emacs-mcp shared daemon" });
else serveStdio({ handleMessage, banner: "🧠 Emacs MCP Server started" });
