// login.mjs — Device code auth for Claude on AC Native
// Shows a short code + QR code, polls server until phone approves.
// On approval, saves Claude credentials to /mnt/config.json and launches claude.
...
function paint({ wipe, ink, box, write, qr, screen, system, wifi }) {
  ...
  write("claude login", { x: 10, y: 10, size: 2, font: "matrix" });
  ...
  if (state === "requesting") {
    ...
    // Big code display
    ink(T.fg, T.fg, T.fg);
    write("scan or visit:", { x: 10, y: 40, size: 1, font });
    ...
    if (system.fetchResult && frame > pollFrame + 30) {
      try {
        ...
        if (data.status === "approved" && data.credentials) {
          ...
          saveCredentials(system, data.credentials);
          state = "approved";
        }
        // Reset fetch result for next poll
      } catch (e) {
    ...
    // Jump to claude after brief pause
    if (frame % 120 === 0) {
...
function saveCredentials(system, credentials) {
  // Write Claude credentials to /mnt so the PTY can read them
  // The PTY reads /mnt/claude-credentials.json and sets CLAUDE_CODE_OAUTH_TOKEN
  try {
    system.writeFile("/mnt/claude-credentials.json", JSON.stringify(credentials));
  } catch (e) {
    console.log("[login] Failed to save credentials:", e);
  }
}
