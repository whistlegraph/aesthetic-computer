...
function releasePercussionHold(sound, hold) {
  ...
  for (const entry of hold.voices) {
    ...
    sound?.kill?.(entry.handle, entry.releaseFade ?? 0.08);
  }
}
...
//
// RECIPES are grounded in classic analog drum synthesis (primarily Roland
// TR-808) with frequencies and envelope shapes taken from the bridged-T
// filter math in Kurt Werner's "Boom Like an 808" and the Faust
// synths.lib reference (grame-cncm/faustlibraries). See reports/ for the
// research summary. Key principle: the iconic 808 frequencies (238 Hz
// snare, 540/800 Hz cowbell, 6-square hat cluster) are NOT jittered —
// they're what makes each drum sound like itself. Per-hit variation
// comes from TIMBRE jitter (volume balance, decay, attack, pan) not
// tonal jitter.
//
// Multi-burst drums (clap, tambourine) use the Faust trick of firing
// one voice per burst with staggered `attack` values (0.005 / 0.015 /
// 0.025 / 0.035) — the audio engine's attack envelope provides the
// sub-frame timing resolution that the setTimeout polyfill can't
// (sim() only flushes every ~16 ms).
//
...
const CHARACTER_SPRITES = {
  ...
  // 🎩 Piano Man — formal pianist with bowler hat, mustache, red
  // bowtie, sitting at a piano keyboard. The '=' row is the keys; on
  // play they flip to '+' for a highlight flash. M ↔ N swaps the
  // mustache to a slightly excited shape on note hits.
  piano: {
    palette: {
      H: [40, 40, 50], h: [25, 25, 35],
      F: [220, 180, 80],
      S: [240, 200, 170],
      M: [80, 50, 60], N: [120, 70, 80], L: [150, 90, 100],
      B: [60, 50, 70], b: [40, 35, 50],
      C: [180, 60, 60],
      "=": [240, 235, 220], "+": [255, 250, 200],
      c: [30, 25, 40],
      P: [10, 10, 20],
    },
    paletteLight: {
      H: [60, 60, 70], h: [40, 40, 55],
      F: [200, 160, 60],
      S: [240, 200, 170],
      M: [70, 40, 50], N: [110, 60, 70], L: [140, 80, 90],
      B: [80, 70, 90], b: [55, 45, 65],
      C: [180, 60, 60],
      "=": [240, 235, 220], "+": [240, 200, 100],
      c: [30, 25, 40],
      P: [10, 10, 20],
    },
    spriteA: [
  ...
  // 👩 Sample Sally — modern sampler artist with wavy brown hair, pink
  // skin, teal shirt, and a row of sampler pads. The pads are the '='
  // row; pressing notes flips them to '+' for a per-pad pulse glow.
  sample: {
    palette: {
      H: [120, 70, 40], h: [80, 45, 25],
      S: [245, 215, 195],
      M: [170, 80, 90], N: [220, 100, 120], L: [160, 60, 80],
      D: [80, 130, 160], d: [50, 90, 120],
      "=": [200, 50, 70], "+": [255, 100, 130],
      g: [50, 35, 25],
      P: [25, 15, 30],
    },
    paletteLight: {
      H: [110, 60, 30], h: [70, 40, 20],
      S: [245, 215, 195],
      M: [160, 70, 80], N: [200, 90, 110], L: [150, 50, 70],
      D: [60, 110, 150], d: [40, 80, 110],
      "=": [200, 50, 70], "+": [240, 80, 110],
      g: [40, 30, 20],
      P: [20, 10, 25],
    },
    spriteA: [
