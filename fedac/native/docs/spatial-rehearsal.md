...
## MIDI spatializer (September 24)

`midi-to-nsscore.mjs song.mid --name slug` turns any Standard MIDI File into a ring score for the same six seats: every note keeps its time, pitch, velocity and GM program (voiced by the native gm_synth, with a family wave as fallback, and trimmed by the measured GM levels), and gains a place in the room. `--place octave` (the default) makes one octave one lap of the ring, so scales run around the room and chords spread across it; `register` sweeps a source's range once around; `tracks` pins each source to its own side, or orbits them all with `--orbit S`; `orbit` gives each source its own slow orbit. Channel 10 becomes the house kit (kick front, snare rear, hats orbiting, cymbals on every seat). `--held <regex>` sends a source to the center laptop, `--max-dur` shortens sustained notes for dense files, `--rate` and `--transpose` do what they say, and MIDI markers become movements. The report prints the peak voices each laptop is asked for (the native pool holds 16, oldest stolen). First two: `rush-e.nsscore` (4:59, three pianos, 19,850 events, `--max-dur 1`) and `naruto-ai-bomb.nsscore` (2:32, a kit and twelve instruments). Deploy with `--score rush-e` like any ring score.
