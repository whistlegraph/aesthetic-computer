...
// The sine wavetable + bounded per-note stochasticism helpers that used to
// live here moved into the standalone gm_synth module (gm_synth.c). The GM
// voices own their own copies; this engine only drives them via
// gm_voice_init()/gm_voice_render(). The global organic knob is set through
// gm_set_organic() (see audio_set_organic below).
...
// ============================================================
// GM synthesis library (modal piano / FM e-piano / extended Karplus-Strong /
// modal bank / subtractive synth bass) was EXTRACTED to the standalone,
// dependency-free gm_synth module (src/gm_synth.{h,c}) so it can compile + be
// tested off-device (macOS, no ALSA) and later be shared into Menu Band. The
// program tables, GMProgramParams, gm_voice_init(), the generators, and
// gm_program_implemented() all live there now. This engine fills a per-voice
// GMVoice (ACVoice.gm) at note-on and renders it via gm_voice_render().
// ============================================================
...
static inline double generate_sample(ACVoice *v, double sample_rate) {
    ...
    case WAVE_MODAL:
    case WAVE_SYNTHBASS:
        // All GM voices render through the standalone gm_synth module. The
        // engine supplies the amplitude envelope + current (smoothed) frequency;
        // gm_synth owns the rest of the DSP state (ACVoice.gm) and dispatches on
        // its own stored engine type.
        s = gm_voice_render(&v->gm, sample_rate, compute_envelope(v), v->frequency);
        break;
    default:
        s = 0.0;
    }
    ...
    // synthesis voices (gmpiano/epiano/pluck/modal/synthbass) manage their own
    // phase state internally.
    if (v->type != WAVE_WHISTLE && v->type != WAVE_GUN && v->type != WAVE_HARP &&
        v->type != WAVE_PIANO && v->type != WAVE_GMPIANO && v->type != WAVE_EPIANO &&
        v->type != WAVE_PLUCK && v->type != WAVE_MODAL && v->type != WAVE_SYNTHBASS) {
...
uint64_t audio_synth(ACAudio *audio, WaveType type, double freq,
    ...
    if (type == WAVE_NOISE || type == WAVE_WHISTLE || type == WAVE_GUN ||
        ...
        type == WAVE_MODAL || type == WAVE_SYNTHBASS) {
...
// gm_program_implemented() now lives in the standalone gm_synth module
// (gm_synth.h / gm_synth.c) — it is the same 0-based program-range table.

// GM synthesis voice (docs/gm-synthesis/). Mirrors audio_synth_gun: do the base
// voice setup (slot alloc, envelope fields, per-trigger noise_seed) via
// audio_synth with a GM placeholder type so the seed is set, then run
// gm_voice_init() to fill the standalone GMVoice (ACVoice.gm) state + apply
// note-on stochasticism. Unimplemented programs return 0 so the JS caller can
// fall back to the normal `type`-based audio_synth path. v->type is kept as one
// of the GM WaveTypes so the render switch routes here and the phase-advance
// guard skips it; the *actual* engine is stored inside v->gm.engine and
// gm_voice_render() dispatches on it. We map the chosen engine back onto
// v->type for parity with the pre-extraction behavior (type checks elsewhere).
uint64_t audio_synth_gm(ACAudio *audio, int program, double freq,
    ...
    if (v) {
        double sr = (double)(audio->actual_rate ? audio->actual_rate : AUDIO_SAMPLE_RATE);
        if (gm_voice_init(&v->gm, program, freq, sr, v->noise_seed) < 0) {
            // gm_program_implemented() said yes but init disagreed — retire the
            // slot and report no-voice so the JS caller falls back cleanly.
            v->state = VOICE_INACTIVE;
            ok = 0;
        } else {
            // Reflect the chosen engine in v->type (render still routes all GM
            // types through gm_voice_render; this keeps type-based guards right).
            switch (v->gm.engine) {
            case GM_ENGINE_GMPIANO:   v->type = WAVE_GMPIANO;   break;
            case GM_ENGINE_EPIANO:    v->type = WAVE_EPIANO;    break;
            case GM_ENGINE_PLUCK:     v->type = WAVE_PLUCK;     break;
            case GM_ENGINE_MODAL:     v->type = WAVE_MODAL;     break;
            case GM_ENGINE_SYNTHBASS: v->type = WAVE_SYNTHBASS; break;
            default:                  break;  // stays WAVE_GMPIANO
            }
        }
    }
    pthread_mutex_unlock(&audio->lock);
...
void audio_set_organic(double amt) {
    gm_set_organic(clampd(amt, 0.0, 1.0));
}
