...
typedef struct {
    ...
    int    gun_phys_echo_w;
    // Harp — Karplus-Strong plucked string. Reuses whistle_bore_buf /
    // whistle_bore_w as the string delay line (voice can only be one
    // wave type at a time). `harp_lp1` is the two-point moving-average
    // damping filter state from Karplus & Strong (CMJ 7:2, 1983).
    double harp_lp1;
} ACVoice;
