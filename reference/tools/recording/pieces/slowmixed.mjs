// Mixed raytraced cubes and spheres with proper real-time animation speeds
// 2025.09.25.23.25.15
...
function initObjects() {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    // Animation properties based on REAL TIME (frameTime is in milliseconds)
    sphere.originalCenter = new Vec3(centerX, centerY, centerZ);
    // Over 5 seconds (5000ms), we want maybe 1/4 rotation max
    sphere.rotationSpeed = (Math.random() - 0.5) * 0.0003; // radians per ms
    sphere.floatSpeed = (Math.random() - 0.5) * 0.0002;    // frequency per ms
    sphere.floatAmplitude = 0.3 + Math.random() * 0.4;     // amplitude
    sphere.type = 'sphere';
  ...
  for (let i = 0; i < 8; i++) {
    ...
    // Over 5 seconds (5000ms), we want gentle movement
    cube.rotationSpeed = (Math.random() - 0.5) * 0.0002;  // radians per ms
    cube.floatSpeed = (Math.random() - 0.5) * 0.00015;    // frequency per ms
    cube.floatAmplitude = 0.2 + Math.random() * 0.3;      // amplitude
    cube.type = 'cube';
...
function updateObjects(frameTime) {
  // frameTime is in milliseconds, 60fps means each frame is ~16.67ms
  // For a 5-second video (300 frames), frameTime goes from 0 to 5000ms
  
  objects.forEach((obj, i) => {
    // Gentle floating - one cycle every ~8 seconds (8000ms)
    const floatCycleMs = 8000; // 8 seconds for one full cycle
    const floatPhase = (frameTime / floatCycleMs) * 2 * Math.PI;
    const floatOffset = Math.sin(floatPhase + i * 0.5) * obj.floatAmplitude;
    
    // Gentle orbital motion - one orbit every ~12 seconds (12000ms)
    const orbitCycleMs = 12000; // 12 seconds for one full orbit
    const orbitPhase = (frameTime / orbitCycleMs) * 2 * Math.PI;
    const radius = 0.3; // Smaller orbit
    const rotX = Math.cos(orbitPhase + i * 0.8) * radius;
    const rotZ = Math.sin(orbitPhase + i * 0.8) * radius;
...
function paint({ api, frameTime }) {
  ...
  // Debug: Log frameTime to understand the time progression
  if (Math.floor(frameTime / 1000) !== Math.floor((frameTime - 16.67) / 1000)) {
    console.log(`🕐 Time: ${frameTime}ms (${(frameTime/1000).toFixed(1)}s)`);
  }
