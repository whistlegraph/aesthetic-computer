...
// Falls back to the faded static mascot if the video is absent. `mood` nudges
// the backdrop's presence with the app's state (the wizard "leans in").
import AppKit
import AVFoundation

final class BackdropView: NSView {
    enum Mood { case idle, listening, recording, success, error }
    var mood: Mood = .idle { didSet { playerLayer.opacity = opacity(for: mood) } }
    private func opacity(for m: Mood) -> Float {
        switch m {
        case .idle: return 0.16
        case .listening: return 0.24
        case .recording: return 0.26
    ...
    override init(frame: NSRect) {
        fallbackImage = {
            let b = Bundle.module
            if let u = b.url(forResource: "wavewizard-mascot", withExtension: "png", subdirectory: "Assets")
                ?? b.url(forResource: "wavewizard-mascot", withExtension: "png"),
    ...
    private func setupVideo() {
        let b = Bundle.module
        guard let url = b.url(forResource: "wavewizard-backdrop", withExtension: "mp4", subdirectory: "Assets")
            ?? b.url(forResource: "wavewizard-backdrop", withExtension: "mp4") else { return }
        ...
        playerLayer.opacity = opacity(for: .idle)
