...
void update_sound_from_pixels(uint16_t scroll_x, uint16_t scroll_y) {
    ...
    // CH1 (Strip 0: top strip, pulse with sweep)
    if(ch1_count > 0) {
        uint8_t avg_y = ch1_y_sum / ch1_count;
        uint8_t vol = (ch1_count > 8) ? 0x0C : 0x07;
        uint8_t duty = (ch1_count > 5) ? 0x80 : 0x40;
        
        // Y position controls pitch within strip (0-35 range)
        uint16_t freq = 0x600 - (avg_y << 4);  // Higher on screen = higher pitch
        if(freq < 0x400) freq = 0x400;
        if(freq > 0x700) freq = 0x700;
        
        NR10_REG = 0x00;  // No sweep for now
        NR11_REG = duty;
        NR12_REG = (vol << 4) | 0x05;
        NR13_REG = freq & 0xFF;
    ...
    // CH2 (Strip 1: second strip, pulse)
    if(ch2_count > 0) {
        uint8_t avg_y = ch2_y_sum / ch2_count;
        uint8_t vol = (ch2_count > 8) ? 0x0C : 0x07;
        uint8_t duty = (ch2_count > 5) ? 0x80 : 0x40;
        
        // Y position controls pitch (36-71 range, normalized to 0-35)
        uint16_t freq = 0x600 - ((avg_y - 36) << 4);
        if(freq < 0x400) freq = 0x400;
        if(freq > 0x700) freq = 0x700;
    ...
    // CH3 (Strip 2: third strip, wavetable)
    if(ch3_count > 0) {
        uint8_t avg_y = ch3_y_sum / ch3_count;
        ...
        NR32_REG = (ch3_count > 10) ? 0x40 : 0x60;
        
        // Y position controls pitch (72-107 range, normalized to 0-35)
        uint16_t freq = 0x600 - ((avg_y - 72) << 4);
        if(freq < 0x400) freq = 0x400;
        if(freq > 0x700) freq = 0x700;
    ...
    // CH4 (Strip 3: bottom strip, noise)
    if(ch4_count > 0) {
        uint8_t avg_y = ch4_y_sum / ch4_count;
        uint8_t vol = (ch4_count > 8) ? 0x0D : 0x08;
        
        // Y position controls noise character (108-143 range, normalized to 0-35)
        uint8_t noise_param = (avg_y - 108) >> 2;  // 0-8 range
...
void seed_canvas_tiles(void) {
  ...
  // Add some starting patterns for each strip
  // Use simple repeating patterns to make sound interesting
  
  // Strip 0 (y: 0-35) - sparse dots
  for(uint16_t x = 0; x < CANVAS_UNIQUE_WIDTH; x += 8) {
    for(uint16_t y = 5; y < 35; y += 10) {
      plot_pixel(x, y);
      plot_pixel(x+1, y);
    }
  }
  
  // Strip 1 (y: 36-71) - horizontal lines
  for(uint16_t x = 0; x < CANVAS_UNIQUE_WIDTH; x += 4) {
    plot_pixel(x, 45);
    plot_pixel(x, 55);
    plot_pixel(x, 65);
  }
  
  // Strip 2 (y: 72-107) - diagonal pattern
  for(uint16_t x = 0; x < CANVAS_UNIQUE_WIDTH; x++) {
    uint16_t y = 72 + ((x * 35) / CANVAS_UNIQUE_WIDTH);
    plot_pixel(x, y);
    plot_pixel(x, y+1);
  }
  
  // Strip 3 (y: 108-143) - dense pattern
  for(uint16_t x = 0; x < CANVAS_UNIQUE_WIDTH; x += 3) {
    for(uint16_t y = 110; y < 143; y += 5) {
      plot_pixel(x, y);
    }
  }
}
...
void main(void) {
  ...
  // Draw horizontal guide lines for the 4 voice strips
  draw_strip_guides();
  ...
  for(uint8_t i = 0; i < 18; i++) {  // 18 sprites to cover 144 pixels
    ...
    while(1) {
        ...
        // Update sound every frame (60 Hz) for responsive audio
        update_sound_from_pixels(scroll_x, scroll_y);

        // Read joypad input for manual scroll control
        uint8_t joy = joypad();
        
        // Manual scroll control with D-pad
        target_vx = 0;
        target_vy = 0;
        
        if(joy & J_RIGHT) target_vx = 512;   // Scroll right
        if(joy & J_LEFT) target_vx = -512;   // Scroll left
        if(joy & J_DOWN) target_vy = 512;    // Scroll down
        if(joy & J_UP) target_vy = -512;     // Scroll up
        
        // Apply scroll velocity directly (no smoothing needed)
        scroll_vx = target_vx;
        scroll_vy = target_vy;
        
        // Convert velocity to integer scroll amount
        int8_t scroll_dx = scroll_vx >> 8;  // Get integer part
        int8_t scroll_dy = scroll_vy >> 8;  // Get integer part

        // Update scroll position
        scroll_x = (scroll_x + scroll_dx) & WORLD_SCROLL_MASK;
        scroll_y = (scroll_y + scroll_dy) & BG_SCROLL_MASK;
