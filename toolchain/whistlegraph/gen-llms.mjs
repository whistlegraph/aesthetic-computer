  ...
  return [
    ...
    table(
      ...
      [
        ...
        [
          `[\`GET /api/wg/verify\`](${SITE_URL}/api/wg/verify)`,
          "free",
          "Checks a license receipt: `?receipt=<token>&sig=<signature>`, both copied from the `verify` link inside a license. Confirms the receipt was issued here, is unedited, and is still in term.",
        ],
      ],
    ),
    "",
...
const llmsTxt = [
  ...
  `- [Verify a license](${SITE_URL}/api/wg/verify): \`/api/wg/verify?receipt=<token>&sig=<signature>\` — checks a receipt was issued here and is still in term. Free.`,
  "",
  `Generated ${generated}.`,
  "",
].join("\n");
...
const robotsTxt = [
  ...
  "#",
  `#   ${SITE_URL}/api/wg/verify           free — check a license receipt`,
  "",
].join("\n");
