#!/usr/bin/env node
// oskiewar-cli.mjs — call one oskiewar-mcp tool from a shell (a copy of venue-cli pointed at the displays).
//
...
const child = spawn(process.execPath, [join(dirname(fileURLToPath(import.meta.url)), "oskiewar-mcp.mjs")], { stdio: ["pipe", "pipe", "inherit"] });
let buf = "";
