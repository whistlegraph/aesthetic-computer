#!/bin/bash
# Safe, user-level macOS disk inventory/cleanup for the AC fleet Macs.
...
usage() {
  cat <<'EOF'
Usage: ac-disk-clean [--apply] [--thin-snapshots] [--install]
...
report() {
  ...
  echo "$(scutil --get LocalHostName 2>/dev/null || hostname) disk report — $(date '+%Y-%m-%dT%H:%M:%S%z')"
  echo
...
install_utility() {
  ...
  plist="$agent_dir/computer.aesthetic.disk-clean.plist"
  source=$(cd "$(dirname "$0")" && pwd)/$(basename "$0")
  uid=$(id -u)
  mkdir -p "$bin_dir" "$agent_dir" "$HOME/Library/Logs"
  if [[ "$source" != "$bin_dir/ac-disk-clean" ]]; then
    install -m 0755 "$source" "$bin_dir/ac-disk-clean"
  else
    chmod 0755 "$bin_dir/ac-disk-clean"
  fi
...
<plist version="1.0"><dict>
  <key>Label</key><string>computer.aesthetic.disk-clean</string>
  <key>ProgramArguments</key><array>
    <string>$bin_dir/ac-disk-clean</string><string>--apply</string>
  </array>
  <key>StartCalendarInterval</key><dict>
    <key>Weekday</key><integer>1</integer><key>Hour</key><integer>3</integer><key>Minute</key><integer>15</integer>
  </dict>
  <key>StandardOutPath</key><string>$HOME/Library/Logs/ac-disk-clean.log</string>
  <key>StandardErrorPath</key><string>$HOME/Library/Logs/ac-disk-clean.log</string>
</dict></plist>
EOF

  plutil -lint "$plist"
  launchctl bootout "gui/$uid/computer.aesthetic.disk-clean" >/dev/null 2>&1 || true
  launchctl bootstrap "gui/$uid" "$plist"
  echo "Installed $bin_dir/ac-disk-clean and weekly LaunchAgent."
}
