        ...
        <script>
          ...
          function anim(){if(!run||!c)return;f++;lb*=0.94;var chaos=0.3+bp*0.4+lb*0.3;var t=f*0.05;
            ...
            if(motd){x.save();var mAge=(performance.now()-motdStart)/1000;var maxW=W*0.9;var roughChars=Math.max(6,Math.floor(maxW/(7*S)));var linesMotd=wrapMotdText(motd,roughChars);var longest=0;for(var li=0;li<linesMotd.length;li++){if(linesMotd[li].length>longest)longest=linesMotd[li].length;}
              ...
              x.restore();}
            if(!isKidlisp&&pgLoaded){if(pgStart===null&&(imgFullLoaded||performance.now()-bootStart>900))pgStart=performance.now();var pgAge=pgStart?(performance.now()-pgStart)/1000:0,pgReady=pgStart&&pgAge>0.35;if(pgReady){var m=3*S,bSafe=8*S,aspect=pgImg.naturalWidth&&pgImg.naturalHeight?pgImg.naturalWidth/pgImg.naturalHeight:2,bW=Math.min(72*S,Math.max(48*S,W*0.28)),bH=bW/aspect,slide=(Math.sin(t*1.2)*2+Math.sin(t*2.4)*1)*S,bX=W-bW-m+sx*0.4*S+4*S+slide,bY=H-bH-m-bSafe+sy*0.4*S,jit=0.8+chaos*2.2,rev=Math.min(1,Math.max(0,(pgAge-0.35)/0.8));x.globalCompositeOperation='screen';x.globalAlpha=0.25*rev;x.filter='hue-rotate('+(18+Math.sin(t*5)*30)+'deg) saturate('+(1.5+chaos*0.6)+') blur(2px)';x.drawImage(pgImg,bX-jit*1.5*S,bY+jit*0.5*S,bW,bH);x.filter='hue-rotate('+(-18+Math.cos(t*4)*30)+'deg) saturate('+(1.5+chaos*0.6)+') blur(2px)';x.drawImage(pgImg,bX+jit*1.4*S,bY-jit*0.6*S,bW,bH);x.filter='none';x.globalCompositeOperation='screen';x.globalAlpha=(0.35+Math.sin(t*10)*0.15)*rev;x.filter='blur('+(10+chaos*6)+'px)';x.drawImage(pgImg,bX,bY,bW,bH);x.filter='none';x.globalCompositeOperation='lighter';x.globalAlpha=0.15*rev;x.filter='blur('+(20+chaos*10)+'px)';x.drawImage(pgImg,bX-4*S,bY-4*S,bW+8*S,bH+8*S);x.filter='none';x.globalCompositeOperation='source-over';x.globalAlpha=0.92*rev;x.drawImage(pgImg,bX,bY,bW,bH);x.globalCompositeOperation='screen';x.globalAlpha=(0.35+Math.sin(t*8)*0.2)*rev;x.fillStyle='rgb('+(255+Math.sin(t*12)*20|0)+','+(220+Math.cos(t*10)*30|0)+','+(80+Math.sin(t*15)*40|0)+')';x.fillRect(bX,bY,bW,bH);x.globalCompositeOperation='source-over';x.globalAlpha=1;}}
