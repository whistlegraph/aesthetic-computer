      ...
      <body class="native-cursor" ${lanHost ? " data-lan-host=" + lanHost : ""}>
        ...
        <script>
          ...
          function anim(){if(!run||!c)return;f++;lb*=0.94;var chaos=0.3+bp*0.4+lb*0.3;var t=f*0.05;
            ...
            if(uH){var hAge=(performance.now()-hST)/1000,hFade=Math.min(1,hAge*2),hPulse=0.8+0.2*Math.sin(hAge*3);x.save();x.font='bold '+(5*S)+'px monospace';var hY=tYbase+16*S,hX=tX;x.globalAlpha=hFade*hPulse*0.3;x.fillStyle=isLightMode?'#7030a0':'#a855f7';x.fillText(uH,hX,hY);var ha=(0.4+chaos*0.5)*S;x.globalAlpha=hFade*hPulse*0.25;x.fillStyle=isLightMode?'#c04060':'#ff6b9d';x.fillText(uH,hX-ha,hY);x.fillStyle=isLightMode?'#40a080':'#6bffb8';x.fillText(uH,hX+ha*0.5,hY-ha*0.3);x.globalAlpha=hFade*hPulse;x.fillStyle=isLightMode?'rgb(100,60,140)':'rgb(240,210,255)';x.fillText(uH,hX+sx*0.1*S,hY+sy*0.1*S);x.restore();}
            var tSY=tYbase+15*S;x.font=(4*S)+'px monospace';var now=performance.now();
            for(var i=0;i<lines.length;i++){var y=tSY+i*4*S;if(y>H-3*S)break;var al=Math.max(0.4,1-i*0.08),la=(now-lines[i].time)/1000,lineBurst=Math.max(0,lines[i].burst-la*2),dt=lines[i].text;var isActive=i===0&&dt.indexOf('_')>-1;if(isActive){var fastBlink=Math.sin(t*12)>0;dt=fastBlink?dt:dt.replace(/_$/,' ');}else if(i===0&&f%30<15)dt=dt.replace(/_$/,' ');var lsh=(baseShk*0.4+lineBurst*2+chaos)*S;var lxx=tX+sx*0.5*S+(Math.random()-0.5)*lsh,lyy=y+sy*0.5*S+(Math.random()-0.5)*lsh;var aber=(0.8+chaos*2+lineBurst)*S;if(isActive){x.globalAlpha=0.5;x.fillStyle=isLightMode?'rgb(180,140,40)':'rgb(255,200,80)';x.fillText(dt,lxx-aber*0.8,lyy);x.fillStyle=isLightMode?'rgb(140,180,80)':'rgb(200,255,120)';x.fillText(dt,lxx+aber*0.4,lyy-aber*0.2);x.globalAlpha=1;x.fillStyle=isLightMode?'rgb(120,100,40)':'rgb(255,255,150)';x.fillText(dt,lxx,lyy);}else{if(aber>S){x.globalAlpha=al*0.4;x.fillStyle=isLightMode?'rgb(180,80,120)':'rgb(255,120,180)';x.fillText(dt,lxx-aber,lyy);x.fillStyle=isLightMode?'rgb(80,180,140)':'rgb(120,255,200)';x.fillText(dt,lxx+aber*0.5,lyy-aber*0.3);}x.globalAlpha=al;x.fillStyle=isLightMode?'rgb('+(100+i*4)+','+(80+i*3)+','+(120+i*2)+')':'rgb('+(220-i*6)+','+(180-i*8)+','+(240-i*4)+')';x.fillText(dt,lxx,lyy);}}
