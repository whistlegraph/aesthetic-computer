    ...
    <html>
      ...
      <body class="native-cursor" ${lanHost ? " data-lan-host=" + lanHost : ""}>
        ...
        <script>
          ...
          var SYN={kw:[197,134,192],fn:[220,220,170],str:[206,145,120],num:[181,206,168],cmt:[106,153,85],op:[212,212,212],tp:[78,201,176],vr:[156,220,254]};
          var COLS={'.mjs':'#1e3a5f','.js':'#2d4a3e','.lisp':'#3d2a4a',default:'#1a2433'};
          var PAGECOLS=['#0d1a2d','#0d2d1a','#2d0d1a','#1a0d2d','#1a2d0d','#2d1a0d','#0d1a1a','#1a1a2d'];
          var files=[],scrollYs={},bootStart=performance.now();
          ...
          function mkPage(name,toks,S){S=S||1;var pw=(53+Math.random()*60)*S,ph=Math.min(toks.length*3*S+9*S,100*S),px=Math.random()*(W-pw);return{x:px,y:H+7*S+Math.random()*27*S,w:pw,h:ph,toks:toks.slice(0,Math.floor((ph-9*S)/(3*S))),col:PAGECOLS[Math.floor(Math.random()*PAGECOLS.length)],name:name,a:0.6+Math.random()*0.3,s:0.5+Math.random()*1.3,rot:(Math.random()-0.5)*0.1,skew:(Math.random()-0.5)*0.05};}
          ...
          function addFile(name,src){if(shownFiles[name])return;shownFiles[name]=true;netAct=1;var ls=src.split('\\n').slice(0,100),ext=(name.match(/\\.[^.]+$/)||[''])[0];var toks=ls.map(tok);fileQ.push({name:name,toks:toks});files.push({name:name,lines:ls,ext:ext,toks:toks,total:ls.length,cH:(ls.length+3)*LH});scrollYs[name]=0;}
          ...
          function anim(){if(!run||!c)return;f++;lb*=0.94;var chaos=0.3+bp*0.4+lb*0.3;var t=f*0.05;
            ...
            var BGCOLS=['#1e3a5f','#2d4a3e','#3d2a4a','#4a2a2a','#2a3a4a','#3a4a2a','#2a2a4a','#4a3a2a'];
            var sHH=HH*S;
            var NAMECOLS=['#6eb5ff','#6effa5','#c49eff','#ff9e9e','#9ed4ff','#d4ff9e','#9e9eff','#ffd49e'];
            ...
            for(var fc=0;fc<maxCols;fc++){var fi=files[fc],pX=fc*colW,pY=0,bgC=BGCOLS[fc%BGCOLS.length],nameC=NAMECOLS[fc%NAMECOLS.length];
            x.fillStyle=bgC;x.globalAlpha=0.9;x.fillRect(pX,pY,colW,pH);x.globalAlpha=1;x.fillStyle='#1a2a3a';x.fillRect(pX,pH-sHH,colW,sHH);
            ...
            x.restore();if(f%4===0)scrollYs[fi.name]=(sY+baseLH);if(scrollYs[fi.name]>fi.toks.length*baseLH-pH+sHH+20*S)scrollYs[fi.name]=0;}}
            ...
            for(var i=0;i<pages.length;i++){var p=pages[i];var jx=(Math.random()-0.5)*S,jy=(Math.random()-0.5)*0.7*S;
              ...
              if(Math.random()>0.88){x.fillStyle='#fff';x.globalAlpha=0.2;x.fillRect(0,Math.random()*p.h,p.w,S);}
              x.fillStyle='#1a2a3a';x.fillRect(0,0,p.w,6*S);
              x.font='bold '+(3*S)+'px monospace';x.fillStyle='#6a9aca';x.globalAlpha=p.a;x.fillText(p.name,2*S,4*S);
              x.font=(2*S)+'px monospace';x.globalAlpha=p.a*0.85;
              for(var li=0;li<p.toks.length;li++){var ly=9*S+li*3*S;if(ly>p.h-2*S)break;
                x.fillStyle='#3a5a7a';x.fillText(String(li+1).padStart(3,' '),S,ly);
            ...
            x.globalAlpha=0.04+chaos*0.08;x.fillStyle='#000';for(var yy=0;yy<H;yy+=3*S)x.fillRect(0,yy,W,S);
            ...
            x.globalAlpha=0.05+chaos*0.1;for(var ni=0;ni<(15+chaos*35|0);ni++){x.fillStyle='rgb('+(Math.random()*100|0)+','+(Math.random()*50|0)+','+(Math.random()*180|0)+')';x.fillRect(Math.random()*W|0,Math.random()*H|0,S,S);}
            x.globalAlpha=0.03+chaos*0.08*(0.5+Math.random()*0.5);x.fillStyle='rgb('+(90+chaos*30)+','+(20+chaos*10)+','+(140+chaos*40)+')';x.fillRect(0,0,W,H);
            x.globalAlpha=1;var lS=21*S,lX=5*S,lY=5*S,tX=lX+lS+4*S;
            var pnkR=60+Math.sin(t*1.5)*30+chaos*20|0,pnkG=20+Math.sin(t*2.1)*15+chaos*10|0,pnkB=80+Math.sin(t*1.8)*40+chaos*30|0;
            ...
            x.globalAlpha=0.35;x.fillStyle='rgb(255,120,180)';x.fillText('Aesthetic',tX-aber*0.5,tYbase);x.fillStyle='rgb(120,255,200)';x.fillText('Aesthetic',tX+aber*0.3,tYbase-aber*0.2);x.globalAlpha=1;x.fillStyle='rgb(240,200,255)';x.fillText('Aesthetic',tX,tYbase);
            var dotX=tX+x.measureText('Aesthetic').width;var dotVis=netAct>0.1?(Math.sin(t*20+netAct*10)>0?1:0.2):1;var dotCol=netAct>0.3?'rgb(255,255,100)':netAct>0.1?'rgb(255,200,150)':'rgb(255,220,255)';x.globalAlpha=0.4*dotVis;x.fillStyle='rgb(255,180,220)';x.fillText('.',dotX-aber*0.3,tYbase+dotBounce);x.fillStyle='rgb(180,255,220)';x.fillText('.',dotX+aber*0.2,tYbase+dotBounce-aber*0.15);x.globalAlpha=dotVis;x.fillStyle=dotCol;x.fillText('.',dotX,tYbase+dotBounce);
            var compX=dotX+x.measureText('.').width;x.globalAlpha=0.35;x.fillStyle='rgb(255,120,180)';x.fillText('Computer',compX-aber*0.5,tYbase);x.fillStyle='rgb(120,255,200)';x.fillText('Computer',compX+aber*0.3,tYbase-aber*0.2);x.globalAlpha=1;x.fillStyle='rgb(240,200,255)';x.fillText('Computer',compX,tYbase);
            var secT=sec.toFixed(2)+'s';x.font='bold '+(4*S)+'px monospace';var secX=tX;x.globalAlpha=0.4;x.fillStyle='rgb(255,120,180)';x.fillText(secT,secX-aber+sx*0.1*S,tYbase+5*S);x.fillStyle='rgb(120,255,200)';x.fillText(secT,secX+aber*0.5+sx*0.1*S,tYbase+5*S-aber*0.3);x.globalAlpha=1;x.fillStyle='rgb(220,180,240)';x.fillText(secT,secX+sx*0.1*S+(Math.random()-0.5)*tss*0.2*S,tYbase+5*S+sy*0.1*S+(Math.random()-0.5)*tss*0.2*S);
            var d=new Date(),utcT=d.getUTCHours().toString().padStart(2,'0')+':'+d.getUTCMinutes().toString().padStart(2,'0')+':'+d.getUTCSeconds().toString().padStart(2,'0')+' UTC';x.font=(3*S)+'px monospace';x.globalAlpha=0.35;x.fillStyle='rgb(255,140,200)';x.fillText(utcT,tX-aber*0.6,tYbase+10*S);x.fillStyle='rgb(140,255,220)';x.fillText(utcT,tX+aber*0.3,tYbase+10*S-aber*0.2);x.globalAlpha=0.9;x.fillStyle='rgb(200,170,230)';x.fillText(utcT,tX,tYbase+10*S);x.globalAlpha=1;
          ...
          return{log:add,hide:function(){run=false;c.remove();},setHandle:setH,addFile:addFile,netPulse:netPulse};})();
