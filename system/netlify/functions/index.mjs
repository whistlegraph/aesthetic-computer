      ...
      <body class="native-cursor" ${lanHost ? " data-lan-host=" + lanHost : ""}>
        ...
        <script>
          ...
          function anim(){if(!run||!c)return;f++;lb*=0.94;var chaos=0.3+bp*0.4+lb*0.3;var t=f*0.05;
            ...
            x.globalAlpha=0.3;x.filter='blur(8px)';x.drawImage(logoImg,llx-4*S,lly-4*S,lS+8*S,lS+8*S);x.filter='none';
            x.globalAlpha=0.9;x.drawImage(logoImg,llx,lly,lS,lS);x.globalAlpha=1;
            ...
            if(motd){x.save();var mAge=(performance.now()-motdStart)/1000;var maxW=W*0.9;var roughChars=Math.max(6,Math.floor(maxW/(7*S)));var linesMotd=wrapMotdText(motd,roughChars);var longest=0;for(var li=0;li<linesMotd.length;li++){if(linesMotd[li].length>longest)longest=linesMotd[li].length;}
              ...
              x.restore();}
            if(!isKidlisp&&pgLoaded){if(pgStart===null&&(imgFullLoaded||performance.now()-bootStart>900))pgStart=performance.now();var pgAge=pgStart?(performance.now()-pgStart)/1000:0,pgReady=pgStart&&pgAge>0.35;if(pgReady){var m=3*S,bSafe=8*S,aspect=pgImg.naturalWidth&&pgImg.naturalHeight?pgImg.naturalWidth/pgImg.naturalHeight:2,bW=Math.min(44*S,Math.max(28*S,W*0.185)),bH=bW/aspect,bX=W-bW-m+4*S,bY=H-bH-m-bSafe,rev=Math.min(1,Math.max(0,(pgAge-0.35)/0.8));x.globalAlpha=0.3*rev;x.filter='blur(8px)';x.drawImage(pgImg,bX-4*S,bY-4*S,bW+8*S,bH+8*S);x.filter='none';x.globalAlpha=0.88*rev;x.drawImage(pgImg,bX,bY,bW,bH);x.globalAlpha=1;}}
            // Connection status subtle tint
            if(!sessionConnected){x.globalAlpha=0.08;x.fillStyle='rgb(255,100,140)';x.fillRect(0,0,W,H);x.globalAlpha=1;}
            if(connFlash>0.01){x.globalAlpha=connFlash*0.4;x.fillStyle='rgb(80,255,180)';x.fillRect(0,0,W,H);x.globalAlpha=1;}
