      ...
      <body class="native-cursor" ${lanHost ? " data-lan-host=" + lanHost : ""}>
        ...
        <script>
          ...
          function anim(){if(!run||!c)return;f++;lb*=0.94;var chaos=0.3+bp*0.4+lb*0.3;var t=f*0.05;
            ...
            var compX=dotX+x.measureText('.').width+1*S;x.globalAlpha=0.35;x.fillStyle=isKidlisp?(isLightMode?'rgb(180,100,20)':'rgb(255,180,80)'):(isLightMode?'rgb(180,80,120)':'rgb(255,120,180)');x.fillText('Computer',compX-aber*0.5,tYbase);x.fillStyle=isKidlisp?(isLightMode?'rgb(180,140,40)':'rgb(255,220,120)'):(isLightMode?'rgb(80,180,140)':'rgb(120,255,200)');x.fillText('Computer',compX+aber*0.3,tYbase-aber*0.2);x.globalAlpha=1;x.fillStyle=isKidlisp?(isLightMode?'rgb(120,60,0)':'rgb(255,240,200)'):(isLightMode?'rgb(100,60,120)':'rgb(240,200,255)');x.fillText('Computer',compX,tYbase);
            ...
            if(motd){x.save();var mAge=(performance.now()-motdStart)/1000;var maxW=W*0.9;var roughChars=Math.max(6,Math.floor(maxW/(7*S)));var linesMotd=wrapMotdText(motd,roughChars);var longest=0;for(var li=0;li<linesMotd.length;li++){if(linesMotd[li].length>longest)longest=linesMotd[li].length;}
              ...
              var breathScale=1+Math.sin(f*0.04)*0.08+Math.sin(f*0.07)*0.05;var kernWave=Math.sin(f*0.03)*0.3;var rainbowCols=['#ff0040','#ff8000','#ffff00','#00ff80','#00ffff','#0080ff','#8000ff','#ff00ff'];var charsShown=Math.min(motd.length,Math.floor(mAge*12));var shownSoFar=0;var startY=H/2-(linesMotd.length*lineH)/2+(Math.random()-0.5)*shk;
