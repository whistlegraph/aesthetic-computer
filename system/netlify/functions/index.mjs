      ...
      <body class="native-cursor" ${lanHost ? " data-lan-host=" + lanHost : ""}>
        ...
        <script>
          ...
          function anim(){if(!run||!c)return;f++;lb*=0.94;var chaos=0.3+bp*0.4+lb*0.3;var t=f*0.05;
            ...
            var pnkR=isKidlisp?(isLightMode?(180+Math.sin(t*1.5)*20|0):(100+Math.sin(t*1.5)*30|0)):(isLightMode?(120+Math.sin(t*1.5)*20|0):(60+Math.sin(t*1.5)*30|0)),pnkG=isKidlisp?(isLightMode?(120+Math.sin(t*2.1)*15|0):(60+Math.sin(t*2.1)*20|0)):(isLightMode?(80+Math.sin(t*2.1)*15|0):(20+Math.sin(t*2.1)*15|0)),pnkB=isKidlisp?(isLightMode?(60+Math.sin(t*1.8)*10|0):(20+Math.sin(t*1.8)*10|0)):(isLightMode?(160+Math.sin(t*1.8)*30|0):(80+Math.sin(t*1.8)*40|0));
            x.globalAlpha=0.4;x.fillStyle='rgb('+pnkR+','+pnkG+','+pnkB+')';x.fillRect(lX-2*S,lY-2*S,lS+4*S,lS+4*S);x.globalAlpha=1;
            x.imageSmoothingEnabled=imgFullLoaded;var logoImg=imgFullLoaded?imgFull:img;var llx=lX+sx*0.3*S,lly=lY+sy*0.3*S;
            var trCt=2;for(var tr=trCt;tr>=0;tr--){var trA=tr===0?1:0.15*(1-tr/trCt);var trO=tr*2*S;x.globalAlpha=trA;if(tr>0){var hue=tr*20;x.filter='hue-rotate('+hue+'deg) saturate(1.3)';}x.drawImage(logoImg,llx-trO*0.2,lly+trO*0.15,lS,lS);x.filter='none';}
            x.globalAlpha=0.15;x.globalCompositeOperation='multiply';x.fillStyle='rgb(200,100,220)';x.fillRect(llx-2*S,lly-2*S,lS+4*S,lS+4*S);x.globalCompositeOperation='source-over';x.globalAlpha=1;
            var sec=(performance.now()-bootStart)/1000;var tYbase=lY+S;
            x.font='bold '+(4*S)+'px monospace';netAct*=0.92;
            x.globalAlpha=1;x.fillStyle=isKidlisp?(isLightMode?'rgb(120,60,0)':'rgb(255,240,200)'):(isLightMode?'rgb(100,60,120)':'rgb(240,200,255)');x.fillText('Aesthetic',tX,tYbase);
            var dotX=tX+x.measureText('Aesthetic').width+1*S;var dotBlink=Math.sin(t*8)>0;var dotCol=dotBlink?'rgb(0,255,0)':'rgb(255,0,0)';x.globalAlpha=1;x.fillStyle=dotCol;x.fillText('.',dotX,tYbase);
            var compX=dotX+x.measureText('.').width+1*S;x.globalAlpha=1;x.fillStyle=isKidlisp?(isLightMode?'rgb(120,60,0)':'rgb(255,240,200)'):(isLightMode?'rgb(100,60,120)':'rgb(240,200,255)');x.fillText('Computer',compX,tYbase);
            ...
            var secT=sec.toFixed(2)+'s';x.font='bold '+(4*S)+'px monospace';var secX=tX;x.globalAlpha=0.6;x.fillStyle=isKidlisp?(isLightMode?'rgb(140,80,20)':'rgb(255,220,180)'):(isLightMode?'rgb(100,80,120)':'rgb(180,140,200)');x.fillText(secT,secX,tYbase+5*S);
            var d=new Date(),utcT=d.getUTCHours().toString().padStart(2,'0')+':'+d.getUTCMinutes().toString().padStart(2,'0')+':'+d.getUTCSeconds().toString().padStart(2,'0')+' UTC';x.font=(3*S)+'px monospace';x.globalAlpha=0.6;x.fillStyle=isKidlisp?(isLightMode?'rgb(120,80,40)':'rgb(255,220,180)'):(isLightMode?'rgb(120,100,140)':'rgb(200,170,230)');x.fillText(utcT,tX,tYbase+10*S);x.globalAlpha=1;
            if(uH){var hAge=(performance.now()-hST)/1000,hFade=Math.min(1,hAge*2),hPulse=0.8+0.2*Math.sin(hAge*3);x.save();x.font='bold '+(5*S)+'px monospace';var hY=tYbase+16*S,hX=tX;x.globalAlpha=hFade*hPulse;x.fillStyle=isLightMode?'rgb(100,60,140)':'rgb(240,210,255)';x.fillText(uH,hX,hY);x.restore();}
            var tSY=tYbase+15*S;x.font=(4*S)+'px monospace';var now=performance.now();
            for(var i=0;i<lines.length;i++){var y=tSY+i*4*S;if(y>H-3*S)break;var al=Math.max(0.4,1-i*0.08),dt=lines[i].text;var isActive=i===0&&dt.indexOf('_')>-1;if(isActive){var fastBlink=Math.sin(t*12)>0;dt=fastBlink?dt:dt.replace(/_$/,' ');}else if(i===0&&f%30<15)dt=dt.replace(/_$/,' ');var lxx=tX+sx*0.3*S,lyy=y+sy*0.3*S;x.globalAlpha=al;x.fillStyle=isLightMode?'rgb('+(100+i*4)+','+(80+i*3)+','+(120+i*2)+')':'rgb('+(220-i*6)+','+(180-i*8)+','+(240-i*4)+')';x.fillText(dt,lxx,lyy);}
            ...
            if(motd){x.save();var mAge=(performance.now()-motdStart)/1000;var maxW=W*0.9;var roughChars=Math.max(6,Math.floor(maxW/(7*S)));var linesMotd=wrapMotdText(motd,roughChars);var longest=0;for(var li=0;li<linesMotd.length;li++){if(linesMotd[li].length>longest)longest=linesMotd[li].length;}
              ...
              var breathScale=1+Math.sin(f*0.04)*0.04;var rainbowCols=['#ff0040','#ff8000','#ffff00','#00ff80','#00ffff','#0080ff','#8000ff','#ff00ff'];var charsShown=Math.min(motd.length,Math.floor(mAge*12));var shownSoFar=0;var startY=H/2-(linesMotd.length*lineH)/2;
              for(var li=0;li<linesMotd.length;li++){var line=linesMotd[li];var lineChars=Math.max(0,Math.min(line.length,charsShown-shownSoFar));shownSoFar+=line.length;if(lineChars<=0)continue;x.font='bold '+baseFS+'px YWFTProcessing-Bold, monospace';var totalW=0;for(var mi=0;mi<lineChars;mi++){totalW+=x.measureText(line[mi]).width;}
                var mX=(W-totalW*breathScale)/2;var mY=startY+li*lineH;for(var ri=0;ri<lineChars;ri++){var ch=line[ri],ci=Math.floor((ri+f*0.2+li*2)%rainbowCols.length),charAge=mAge-(shownSoFar-line.length+ri)/12,entryScale=Math.min(1,charAge*4);x.save();var charY=mY+Math.sin((ri+f+li)*0.25)*2*S-(1-entryScale)*20*S;x.translate(mX,charY);x.scale(breathScale*entryScale,breathScale*entryScale);x.font='bold '+baseFS+'px YWFTProcessing-Bold, monospace';var cw=x.measureText(ch).width;x.globalAlpha=0.9*entryScale;x.fillStyle=rainbowCols[ci];x.fillText(ch,0,0);x.restore();mX+=cw*breathScale*entryScale;}}
              if(motdHandle){var label='mood of the day from '+motdHandle;var labelFS=Math.max(3*S,baseFS*0.45);x.font='bold '+labelFS+'px monospace';var labelW=x.measureText(label).width;var labelX=(W-labelW)/2;var labelY=Math.min(H-3*S,startY+linesMotd.length*lineH+4*S);x.globalAlpha=0.6;x.fillStyle=isLightMode?'rgb(120,80,140)':'rgb(220,180,240)';x.fillText(label,labelX,labelY);x.globalAlpha=1;}
              x.restore();}
            if(!isKidlisp&&pgLoaded){if(pgStart===null&&(imgFullLoaded||performance.now()-bootStart>900))pgStart=performance.now();var pgAge=pgStart?(performance.now()-pgStart)/1000:0,pgReady=pgStart&&pgAge>0.35;if(pgReady){var m=3*S,bSafe=8*S,aspect=pgImg.naturalWidth&&pgImg.naturalHeight?pgImg.naturalWidth/pgImg.naturalHeight:2,bW=Math.min(44*S,Math.max(28*S,W*0.185)),bH=bW/aspect,slide=(Math.sin(t*1.2)*2+Math.sin(t*2.4)*1)*S,bX=W-bW-m+4*S+slide,bY=H-bH-m-bSafe,jit=0.6,rev=Math.min(1,Math.max(0,(pgAge-0.35)/0.8));x.globalCompositeOperation='screen';x.globalAlpha=0.18*rev;x.filter='hue-rotate(18deg) saturate(1.4)';x.drawImage(pgImg,bX-jit*1.2*S,bY+jit*0.4*S,bW,bH);x.filter='hue-rotate(-18deg) saturate(1.4)';x.drawImage(pgImg,bX+jit*1.1*S,bY-jit*0.5*S,bW,bH);x.filter='none';x.globalCompositeOperation='screen';x.globalAlpha=(0.25+Math.sin(t*10)*0.08)*rev;x.filter='blur(6px)';x.drawImage(pgImg,bX,bY,bW,bH);x.filter='none';x.globalCompositeOperation='source-over';x.globalAlpha=0.88*rev;x.drawImage(pgImg,bX,bY,bW,bH);x.globalAlpha=1;}}
            // Connection status VHS tint
            if(!sessionConnected){x.globalCompositeOperation='screen';x.globalAlpha=0.12+Math.sin(t*3)*0.04;x.fillStyle='rgb(255,60,100)';x.fillRect(0,0,W,H);x.globalCompositeOperation='source-over';}
            if(connFlash>0.01){x.globalCompositeOperation='screen';x.globalAlpha=connFlash*0.5;x.fillStyle='rgb(80,255,180)';x.fillRect(0,0,W,H);x.globalCompositeOperation='source-over';}
