    ...
    <html>
      ...
      <body class="native-cursor" ${lanHost ? " data-lan-host=" + lanHost : ""}>
        ...
        <script>
          ...
          if(klPieceName){(function(){var u=location.origin+'/api/store-kidlisp?code='+encodeURIComponent(klPieceName);fetch(u,{credentials:'omit'}).then(function(r){return r.ok?r.json():null}).then(function(d){if(d&&d.source){klSourceLines=d.source.split('\\n').slice(0,80);klSourceReady=true;}}).catch(function(){});})();}
          ...
          // Boot animation mode: 'spring' (turtle-graphics rainbows, default), 'serious' (clean/refined), or 'aesthetic' (VHS/glitch)
          ...
          // 🌈 Spring boot animation state — procedural turtle-graphics rainbows
          var SPRING_RAINBOWS=[],SPRING_INIT=false;
          var SPRING_FONTS_LIGHT=['serif','monospace','YWFTProcessing-Bold, monospace','cursive','Georgia, serif','Courier New, monospace'];
          // ROYGBIV hues (red, orange, yellow, green, blue, indigo, violet)
          var SPRING_RAINBOW_HUES=[0,28,54,120,210,255,290];
          function springInit(S){SPRING_RAINBOWS=[];var n=7;for(var i=0;i<n;i++){SPRING_RAINBOWS.push({px:Math.random(),py:0.28+Math.random()*0.55,r:(22+Math.random()*34)*S,phase:Math.random()*Math.PI*2,bob:0.6+Math.random()*1.3,tilt:(Math.random()-0.5)*0.28,vx:(0.0009+Math.random()*0.0019)*(Math.random()>0.5?1:-1),twinkle:Math.random()*Math.PI*2,puffHue:40+Math.random()*30});}SPRING_INIT=true;}
          function drawRainbow(ctx,W,H,b,t,S,isLightMode){
            // Drift horizontally, wrap around edges
            b.px+=b.vx;if(b.px>1.2)b.px=-0.2;if(b.px<-0.2)b.px=1.2;
            var cx=b.px*W,cy=b.py*H+Math.sin(t*0.9+b.phase)*b.bob*2.5*S,r=b.r;
            var tilt=b.tilt+Math.sin(t*0.45+b.phase)*0.07;
            ctx.save();ctx.translate(cx,cy);ctx.rotate(tilt);
            var bandW=Math.max(1.4,r*0.095);
            ctx.lineCap='round';
            // Soft halo behind the bow
            ctx.beginPath();ctx.arc(0,0,r+bandW*0.3,Math.PI,Math.PI*2);
            ctx.lineWidth=bandW*1.6;
            ctx.strokeStyle=isLightMode?'rgba(255,250,210,0.55)':'rgba(255,240,180,0.22)';
            ctx.stroke();
            // ROYGBIV arcs from outermost to innermost (turtle-style concentric strokes)
            for(var bi=0;bi<SPRING_RAINBOW_HUES.length;bi++){
              var hue=SPRING_RAINBOW_HUES[bi];var bandR=r-bi*bandW;if(bandR<bandW)break;
              ctx.beginPath();ctx.arc(0,0,bandR,Math.PI,Math.PI*2);
              ctx.lineWidth=bandW*0.92;
              ctx.strokeStyle='hsla('+hue+',88%,'+(isLightMode?52:62)+'%,0.85)';
              ctx.stroke();
            }
            // Little cloud puffs at each foot
            var puffY=bandW*0.2,puffR=bandW*1.9;
            for(var side=-1;side<=1;side+=2){
              var fx=side*(r-bandW*3.5);
              ctx.beginPath();ctx.arc(fx,puffY,puffR,0,Math.PI*2);
              ctx.fillStyle=isLightMode?'hsla('+b.puffHue+',60%,92%,0.9)':'hsla('+b.puffHue+',40%,82%,0.55)';
              ctx.fill();
              ctx.beginPath();ctx.arc(fx+side*puffR*0.7,puffY-puffR*0.45,puffR*0.75,0,Math.PI*2);
              ctx.fillStyle=isLightMode?'hsla('+b.puffHue+',55%,95%,0.85)':'hsla('+b.puffHue+',35%,78%,0.45)';
              ctx.fill();
              ctx.beginPath();ctx.arc(fx-side*puffR*0.55,puffY-puffR*0.25,puffR*0.6,0,Math.PI*2);
              ctx.fillStyle=isLightMode?'hsla('+b.puffHue+',55%,94%,0.75)':'hsla('+b.puffHue+',35%,75%,0.4)';
              ctx.fill();
            }
            // Twinkle sparkle at the crown
            var twA=0.45+Math.sin(t*3+b.twinkle)*0.45;
            ctx.globalAlpha=Math.max(0,twA);
            ctx.fillStyle=isLightMode?'#fff4b0':'#fffce0';
            ctx.beginPath();ctx.arc(0,-r+bandW*0.5,Math.max(1,1.5*S),0,Math.PI*2);ctx.fill();
            // Tiny cross-sparkle rays
            ctx.strokeStyle=ctx.fillStyle;ctx.lineWidth=Math.max(0.5,0.6*S);
            var rays=Math.max(2.5,2.2*S);
            ctx.beginPath();ctx.moveTo(-rays,-r+bandW*0.5);ctx.lineTo(rays,-r+bandW*0.5);
            ctx.moveTo(0,-r+bandW*0.5-rays);ctx.lineTo(0,-r+bandW*0.5+rays);ctx.stroke();
            ctx.globalAlpha=1;
            ctx.restore();}
          ...
          var L5_COLS_DARK=[[246,214,72],[255,238,87],[255,190,0],[200,170,50],[255,215,0],[220,200,80],[255,230,120]];
          var L5_COLS_LIGHT=[[139,101,0],[180,140,0],[160,120,0],[120,90,0],[200,160,0],[150,110,0],[170,130,0]];
          function getKidlispCols(){return isDeviceMode?KIDLISP_COLS_DEVICE:isL5?(isLightMode?L5_COLS_LIGHT:L5_COLS_DARK):(isLightMode?KIDLISP_COLS_LIGHT:KIDLISP_COLS_DARK);}
          // Listen for theme changes and kidlisp-reload from parent (kidlisp.com)
          window.addEventListener('message',function(e){if(e.data&&e.data.type==='kidlisp-theme'){isLightMode=e.data.theme==='light';}if(e.data&&e.data.type==='kidlisp-reload'&&e.data.codeId&&!klPieceName){var cn=e.data.codeId;if(cn.startsWith('$'))cn=cn.slice(1);klPieceName=cn;if(!klSourceReady&&cn){var u2=location.origin+'/api/store-kidlisp?code='+encodeURIComponent(cn);fetch(u2,{credentials:'omit'}).then(function(r){return r.ok?r.json():null}).then(function(d){if(d&&d.source){klSourceLines=d.source.split('\\n').slice(0,80);klSourceReady=true;}}).catch(function(){});}}});
          ...
          function anim(){if(!run||!c)return;f++;var sf=Math.floor(f*0.5);lb*=0.94;touchGlitch*=0.92;var chaos=0.3+bp*0.4+lb*0.3+touchGlitch*0.5;var t=sf*0.05;
            ...
            if(isNotebook){
              ...
              var nbFS=Math.floor(8*S);x.font='bold '+nbFS+'px monospace';var nbTxt=window.acSTARTING_PIECE||'prompt';x.fillStyle=isLightMode?'rgba(0,100,180,0.7)':'rgba(0,180,255,0.7)';x.fillText(nbTxt,4*S,10*S);
            ...
            if(isKidlisp||isL5||isDeviceMode){var klBg=isDeviceMode?'#000000':isL5?(isLightMode?'rgba(255,253,242,0.95)':'rgba(10,10,10,0.95)'):(isLightMode?'rgba(247,247,247,0.95)':'rgba(42,37,32,0.95)');x.fillStyle=klBg;x.fillRect(0,0,W,H);if(!isDeviceMode){x.globalAlpha=isLightMode?0.015:0.03;x.fillStyle=isLightMode?'#888':'#000';for(var yy=0;yy<H;yy+=3*S)x.fillRect(0,yy,W,S);}x.globalAlpha=1;var klCols=getKidlispCols();var dS=isDeviceMode?Math.max(1,densityParam):1;var embedPad=isKidlisp?8*S*dS:0;
              ...
              x.globalAlpha=1;
              // Title text - shows piece name if loading a piece
              {var klTitleFS=isDeviceMode?Math.floor(6*S*dS):Math.floor(6*S);x.font='bold '+klTitleFS+'px monospace';var klTitle=isL5?(klPieceName||'L5'):klPieceName?('$'+klPieceName):'KidLisp';x.globalAlpha=0.7+Math.sin(f*0.04)*0.15;var klTci=Math.floor(f*0.05)%klCols.length;var klTc=klCols[klTci];x.fillStyle='rgb('+klTc[0]+','+klTc[1]+','+klTc[2]+')';x.fillText(klTitle,10*S*dS,10*S*dS);x.globalAlpha=1;}
              // KidLisp source code display (scrolling faded source when available)
              if(klSourceReady&&klSourceLines.length>0){var srcFS=Math.floor(3.5*S);x.font=srcFS+'px monospace';var srcLH=Math.floor(srcFS*1.4);var srcStartY=Math.floor(16*S)+embedPad;var srcX=10*S;var srcScroll=(f*0.3)%(klSourceLines.length*srcLH);var maxSrcLines=Math.min(klSourceLines.length,Math.floor((H-srcStartY)/(srcLH))+2);for(var si=0;si<maxSrcLines;si++){var srcIdx=(si+Math.floor(srcScroll/srcLH))%klSourceLines.length;var srcLine=klSourceLines[srcIdx];if(!srcLine&&srcLine!=='')continue;var srcYPos=srcStartY+si*srcLH-(srcScroll%srcLH);if(srcYPos<srcStartY-srcLH||srcYPos>H)continue;var srcFade=1-Math.abs(srcYPos-H*0.35)/(H*0.4);srcFade=Math.max(0,Math.min(0.25,srcFade*0.25));x.globalAlpha=srcFade;var srcCi=si%klCols.length;var srcC=klCols[srcCi];x.fillStyle='rgb('+srcC[0]+','+srcC[1]+','+srcC[2]+')';x.fillText(srcLine.slice(0,80),srcX,srcYPos);}x.globalAlpha=1;}
              // Device mode: show centered initialization text (larger font for low density)
              if(isDeviceMode){var initText=klPieceName?(isL5?klPieceName:('$'+klPieceName)):'initializing...';var initFS=densityParam===1?Math.max(24,Math.floor(H/20)):6*S*dS;x.font='bold '+initFS+'px monospace';var initW=x.measureText(initText).width;var initX=(W-initW)/2;var initY=H/2;var initPulse=0.5+Math.sin(f*0.1)*0.3;x.globalAlpha=initPulse;x.fillStyle='rgb(255,255,255)';x.fillText(initText,initX,initY);x.globalAlpha=1;}
              ...
              x.globalAlpha=1;requestAnimationFrame(anim);return;}
            // 🌈 Spring mode — yellowish, light, with procedural turtle-graphics rainbows (default)
            if(isSpring){
              ...
              // Draw all rainbows (sorted by y for fake depth — farther ones higher up)
              var sortedBd=SPRING_RAINBOWS.slice().sort(function(a,b){return a.py-b.py;});
              for(var fi=0;fi<sortedBd.length;fi++){drawRainbow(x,W,H,sortedBd[fi],t,S,isLightMode);}
            ...
            if(isSerious){var sbg=isLightMode?'#ffffff':'#000000';x.fillStyle=sbg;x.fillRect(0,0,W,H);
              ...
              // MOTD — centered
              if(motd){x.save();var mAge=(performance.now()-motdStart)/1000;var mFade=Math.min(1,mAge*0.5);var maxMotdW=W*0.85;var roughChars=Math.max(6,Math.floor(maxMotdW/(7*S)));var linesMotd=wrapMotdText(motd,roughChars);var longest=0;for(var li=0;li<linesMotd.length;li++){if(linesMotd[li].length>longest)longest=linesMotd[li].length;}var baseFS=Math.min(14*S,Math.max(6*S,maxMotdW/Math.max(8,longest*1.15)));var lineH=baseFS*1.3;var maxMH=H*0.5;if(linesMotd.length*lineH>maxMH){baseFS=Math.max(5*S,maxMH/(linesMotd.length*1.3));lineH=baseFS*1.3;}var startY=H/2-(linesMotd.length*lineH)/2;x.font='bold '+baseFS+'px YWFTProcessing-Bold, monospace';x.fillStyle=isLightMode?'#222':'#eee';x.globalAlpha=mFade*0.85;x.textAlign='center';for(var li=0;li<linesMotd.length;li++){x.fillText(linesMotd[li],W/2,startY+li*lineH);}if(motdHandle){var label='mood of the day from '+motdHandle;var labelFS=Math.max(3*S,baseFS*0.4);x.font='bold '+labelFS+'px monospace';x.globalAlpha=mFade*0.5;x.fillStyle=isLightMode?'#555':'#bbb';x.fillText(label,W/2,Math.min(H-3*S,startY+linesMotd.length*lineH+4*S));}x.textAlign='left';x.globalAlpha=1;x.restore();}
