// 🎠 whistlegraph.org per-code link previews (Open Graph)
//
...
// code paths here, and we return index.html with that work's own og:title,
// og:image (its thumbnail) and og:video (the take's mp4) injected. Humans still
// get the full app; only the <head> is rewritten.
//
...
function load() {
  ...
  if (im !== cache.indexMtime || gm !== cache.graphsMtime || !cache.byCode) {
