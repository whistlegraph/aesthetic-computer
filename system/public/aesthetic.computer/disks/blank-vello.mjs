// GPU Backend Test - Vello
// Testing Vello WASM renderer (WebGPU compute-based)
...
/* 📝 Engineering Notes
  ...
  Vello is a GPU compute-centric 2D renderer by Linebender.
  Uses WebGPU compute shaders for parallel rendering.
  Very fast: 177fps for 30k paths on M1 Max.
  
  REQUIREMENTS:
  - WebGPU browser support (Chrome, experimental Firefox/Safari)
  - vello_bindings.wasm in lib/gpu/wasm/
  
  To build WASM:
  1. Create Rust wrapper project with wasm-bindgen
  2. wasm-pack build --target web
  3. Copy to lib/gpu/wasm/
*/
...
function boot({ api }) {
  // Use Vello backend via the gpu abstraction
  api.gpu = api.gpu || {};
  api.gpu.backend = "vello";
  api.webgpu.enabled = true;
  
  // Show DOM stats overlay (always on top)
  api.stats.show({ backend: "VELLO", lines: lineCount, extra: "↑↓ adjust  P toggle" });
  
  console.log("🎨 GPU Backend: Vello (WebGPU compute)");
}
...
function paint({ wipe, ink, line, screen }) {
  ...
  // Dark animated background with purple/violet tint (Linebender vibes)
  const bgR = Math.floor(20 + Math.sin(frame * 0.005) * 10);
  const bgG = Math.floor(12 + Math.sin(frame * 0.007) * 8);
  const bgB = Math.floor(35 + Math.sin(frame * 0.009) * 15);
  wipe(bgR, bgG, bgB);
  ...
  for (let i = 0; i < lineCount; i++) {
    ...
    // Purple-violet gradient (Linebender brand colors)
    const hue = (0.75 + t * 0.15 + frame * 0.001) % 1;
