// Paintings, 2026.02.27.12.25.00
// User painting portfolio page.
    ...
    if (!shortCode) {
      try {
        ...
        if (metadata?.code) {
          ...
          storePaintingCode(slug, normalizedHandle, shortCode);
        } else {
...
function storePaintingCode(slug, handle, code) {
  try {
    const runtimeApi = globalThis?.api || globalThis?.acAPI;
    runtimeApi?.code?.store?.(slug, handle, code);
  } catch (_) {
    // Best effort only.
  }
}
