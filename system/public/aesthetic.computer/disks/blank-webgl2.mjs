// GPU Backend Test - WebGL2
// Testing WebGL2 renderer performance
...
/* 📝 Engineering Notes
  ...
  This uses WebGL2 as the rendering backend (wide browser support).
*/
...
function boot({ api }) {
  // Use WebGL2 backend via the gpu abstraction
  api.gpu = api.gpu || {};
  api.gpu.backend = "webgl2";
  api.webgpu.enabled = true; // Enable GPU rendering path
  
  // Show DOM stats overlay (always on top)
  api.stats.show({ backend: "WEBGL2", lines: lineCount, extra: "↑↓ adjust  P toggle" });
  
  console.log("🎨 GPU Backend: WebGL2");
}
...
function paint({ wipe, ink, line, screen }) {
  ...
  // Dark animated background with blue tint (to distinguish from WebGPU)
  const bgR = Math.floor(10 + Math.sin(frame * 0.005) * 8);
  const bgG = Math.floor(15 + Math.sin(frame * 0.007) * 10);
  const bgB = Math.floor(30 + Math.sin(frame * 0.009) * 15);
  wipe(bgR, bgG, bgB);
  ...
  for (let i = 0; i < lineCount; i++) {
    ...
    // Cyan-blue gradient (to distinguish from WebGPU's rainbow)
    const hue = (0.5 + t * 0.2 + frame * 0.001) % 1;
