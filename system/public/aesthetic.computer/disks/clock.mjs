...
import { buildColoredMelodyString, computeCurrentNoteIndex } from "../lib/melody-highlighter.mjs";
...
function getCurrentNoteIndex(melodyState, trackIndex = 0) {
  // Delegate to the shared pure helper (gated on clock's hasFirstSynthFired).
  return computeCurrentNoteIndex(melodyState, trackIndex, hasFirstSynthFired);
}
...
function paint({
  ...
  if (syncedDate) {
    ...
    function buildColoredMelodyStringUnified(melodyString, melodyState) {
      // Thin wrapper over the shared melody highlighter (lib/melody-highlighter.mjs).
      // Feeds clock's runtime state in; preserves the original side effect of
      // clearing recentlyMutated* once the mutation flash expires.
      if (!melodyString) return "";
      const now = performance.now();
      const result = buildColoredMelodyString(melodyString, melodyState, {
        now,
        timingHasStarted: hasFirstSynthFired,
        getCurrentNoteIndex,
        recentlyMutated: {
          noteIndex: recentlyMutatedNoteIndex,
          trackIndex: recentlyMutatedTrackIndex,
        },
        mutationFlash: { active: now - mutationFlashTime < 300 },
        specialCharFlash: now - specialCharFlashTime < 200,
        triggeredAsteriskPositions,
        getStampleStatus: (code) => {
          const c = stampleCache.get(code);
          return c ? { loaded: c.loaded, loading: c.loading, error: c.error } : null;
        },
      });
      if (!(now - mutationFlashTime < 300)) {
