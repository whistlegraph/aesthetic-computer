// GPU Backend Test - ThorVG
// Testing ThorVG WASM renderer (experimental)
...
/* 📝 Engineering Notes
  ...
  ThorVG is a high-performance vector graphics library.
  Requires thorvg.wasm to be installed in lib/gpu/wasm/
  
  To get the WASM:
  1. Build from source: https://github.com/thorvg/thorvg
  2. Or use a pre-built: https://nicholasflamy.github.io/nicholasflamy.github.io/
*/
...
function boot({ api }) {
  // Use ThorVG backend via the gpu abstraction
  api.gpu = api.gpu || {};
  api.gpu.backend = "thorvg";
  api.webgpu.enabled = true;
  
  // Show DOM stats overlay (always on top)
  api.stats.show({ backend: "THORVG", lines: lineCount, extra: "↑↓ adjust  P toggle" });
  
  console.log("🎨 GPU Backend: ThorVG (WASM)");
}
...
function paint({ wipe, ink, line, screen }) {
  ...
  // Dark animated background with orange tint
  const bgR = Math.floor(30 + Math.sin(frame * 0.005) * 15);
  const bgG = Math.floor(15 + Math.sin(frame * 0.007) * 10);
  const bgB = Math.floor(10 + Math.sin(frame * 0.009) * 8);
  wipe(bgR, bgG, bgB);
  ...
  for (let i = 0; i < lineCount; i++) {
    ...
    // Orange-red gradient
    const hue = (0.05 + t * 0.1 + frame * 0.001) % 1;
