    ...
    for (let s = 0; s < output[0].length; s += 1) {
      ...
      if (glitchEnabled) {
        if (this.#glitchHoldCounter <= 0) {
          ...
          const rateSwing = (Math.random() * 0.3 - 0.15) * (0.4 + glitchJitter); // gentle wobble
          const holdSamples = Math.max(
            1,
            Math.floor(this.#glitchHoldSamples * (1 + jitter + rateSwing)),
          );
          this.#glitchHoldCounter = holdSamples;
          // Occasionally refresh the held sample for a soft flutter
        ...
        // Mild flutter only; avoid crunchy skips
        const skipChance = 0.001 + glitchJitter * 0.01;
