...
export function request(command) {
  return new Promise((resolvePromise, reject) => {
    ...
    client.on("timeout", () => client.destroy(new Error("JukeWizard control timed out")));
    client.on("error", (error) => reject(new Error(`JukeWizard is not reachable at ${SOCKET}: ${error.message}`)));
    client.on("end", () => {
      try {
        const value = JSON.parse(response.trim());
        if (!value.ok) reject(new Error(value.error || "JukeWizard command failed"));
...
export function parse(argv) {
  ...
  if (["status", "list", "pause", "toggle", "next", "previous", "prev"].includes(name)) {
  ...
  throw new Error("usage: jukewizard {status|list [limit]|play [path|--title title|--index n]|select ...|pause|toggle|seek seconds|speed 0.5..1.5|next|previous}");
}
