...
@MainActor private final class Shot {
  ...
  init(owner: Int, heavy: Bool, direction: CGFloat, position: CGPoint) {
    self.owner = owner; self.damage = heavy ? 15 : 8; self.direction = direction
    let radius: CGFloat = heavy ? 18 : 10
    node.path = CGPath(ellipseIn: CGRect(x: -radius, y: -radius, width: radius * 2, height: radius * 2), transform: nil)
    node.fillColor = owner == 1 ? pink : ink
    node.strokeColor = pink; node.lineWidth = heavy ? 6 : 4
...
@MainActor private final class Fighter {
  let root = SKNode()
  private let body = SKShapeNode(rectOf: CGSize(width: 76, height: 108), cornerRadius: 30)
  private let face = SKShapeNode(circleOfRadius: 27)
  private let fist = SKShapeNode(circleOfRadius: 16)
  private let shadow = SKShapeNode(ellipseOf: CGSize(width: 92, height: 19))
  let number: Int
  ...
  init(number: Int, inverted: Bool, facing: CGFloat) {
    ...
    let fill = inverted ? ink : pink
    let outline = inverted ? pink : ink
    shadow.fillColor = ink; shadow.strokeColor = ink
    shadow.position.y = -57; root.addChild(shadow)
    body.fillColor = fill; body.strokeColor = outline; body.lineWidth = 6
    root.addChild(body)
    face.fillColor = outline; face.strokeColor = fill; face.lineWidth = 6; face.position.y = 48; root.addChild(face)
    let eyeA = SKShapeNode(circleOfRadius: 4), eyeB = SKShapeNode(circleOfRadius: 4)
    for (eye, x) in [(eyeA, CGFloat(-9)), (eyeB, CGFloat(9))] {
      eye.fillColor = fill; eye.strokeColor = fill; eye.position = CGPoint(x: x, y: 52); root.addChild(eye)
    }
    fist.fillColor = fill; fist.strokeColor = outline; fist.lineWidth = 5
    fist.position = CGPoint(x: 44 * facing, y: 2); root.addChild(fist)
  }

  var hitbox: CGRect { CGRect(x: root.position.x - 38, y: root.position.y - 54, width: 76, height: 108) }
  var attackbox: CGRect {
    ...
    return CGRect(x: facing > 0 ? root.position.x + 22 : root.position.x - 22 - reach,
                  y: root.position.y - 28, width: reach, height: 62)
  }
  ...
  func tick(buttons: Buttons, opponentX: CGFloat) -> Int? {
    ...
    let extensionAmount: CGFloat = attackFrames > 0 ? 12 : 0
    fist.position.x = facing * (44 + extensionAmount)
...
private final class FightScene: SKScene {
  ...
  private let desktopStage: ClosedRange<CGFloat> = 500...780
  private let desktopFloor: CGFloat = 260
  ...
  override func didMove(to view: SKView) {
    ...
    if desktopMode {
      for fighter in [p1, p2] {
        ...
        fighter.visualScale = 0.52; fighter.root.setScale(fighter.visualScale)
      }
    }
  ...
  private func addDesktopStage() {
    let platform = SKShapeNode(rectOf: CGSize(width: desktopStage.upperBound - desktopStage.lowerBound, height: 14), cornerRadius: 7)
    platform.fillColor = pink; platform.strokeColor = pink; platform.lineWidth = 5
    platform.position = CGPoint(x: arenaWidth / 2, y: desktopFloor - 35)
    platform.zPosition = -1; addChild(platform)
  }

  private func showMenu(_ show: Bool) {
    menuOpen = show; menuCard.isHidden = !show
    status.text = show ? "CLICK TRAIN  •  ESC closes menu" : "TRACKPAD: MOVE • TAP SHOOTS • 2-FINGER CLICK HEAVY • SWIPE UP JUMPS"
  }

  private func resetRound() {
    p1.health = 100; p2.health = 100
    let y = desktopMode ? desktopFloor : 122
    p1.root.position = CGPoint(x: desktopMode ? 565 : 390, y: y)
    p2.root.position = CGPoint(x: desktopMode ? 715 : 890, y: y)
  ...
  private func fire(from fighter: Fighter, damage: Int) {
    let heavy = damage > 8
    let shot = Shot(owner: fighter.number, heavy: heavy, direction: fighter.facing,
                    position: CGPoint(x: fighter.root.position.x + fighter.facing * 72, y: fighter.root.position.y + 5))
    if desktopMode { shot.node.setScale(0.52) }
    shots.append(shot); addChild(shot.node)
    tone.play(frequency: heavy ? 115 : 230, duration: heavy ? 0.11 : 0.045, volume: heavy ? 0.24 : 0.14)
  }

  private func updateShots() {
    for shot in shots {
      shot.node.position.x += shot.direction * (shot.damage > 8 ? 13 : 18)
      shot.node.zRotation += shot.direction * 0.22
      ...
      defender.velocity.dx = shot.direction * (shot.damage > 8 ? 14 : 8)
      defender.velocity.dy = shot.damage > 8 ? 8 : 4; defender.grounded = false; defender.flashFrames = 8
      shot.node.removeFromParent()
      tone.play(frequency: shot.damage > 8 ? 72 : 105, duration: 0.09, volume: 0.25)
      if defender.health == 0 {
        roundOver = true; status.text = "PLAYER \(shot.owner) WINS  •  RETURN TO REMATCH"
        tone.play(frequency: 440, duration: 0.35, volume: 0.22)
      }
    }
    shots.removeAll { $0.node.parent == nil || $0.node.position.x < -40 || $0.node.position.x > arenaWidth + 40 }
  }
