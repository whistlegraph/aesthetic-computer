...
@MainActor private final class Shot {
  ...
  init(owner: Int, heavy: Bool, direction: CGVector, position: CGPoint) {
    self.owner = owner; self.damage = heavy ? 15 : 8; self.direction = direction
    let radius: CGFloat = heavy ? 9 : 6
    let orbitDark = SKShapeNode(circleOfRadius: radius)
    orbitDark.fillColor = .clear; orbitDark.strokeColor = contrastDark; orbitDark.lineWidth = lineDark
    let orbitLight = SKShapeNode(circleOfRadius: radius)
    orbitLight.fillColor = .clear; orbitLight.strokeColor = contrastLight; orbitLight.lineWidth = lineLight
    let orbit = SKShapeNode(circleOfRadius: radius)
    orbit.fillColor = .clear; orbit.strokeColor = accent; orbit.lineWidth = lineSpot
    node.addChild(orbitDark); node.addChild(orbitLight); node.addChild(orbit)
    node.position = position
  }

  var hitbox: CGRect { node.frame.insetBy(dx: -3, dy: -3) }
}

@MainActor private final class Asteroid {
  ...
  init(index: Int, position: CGPoint) {
    radius = CGFloat(9 + (index * 7) % 11)
    health = radius > 14 ? 2 : 1
    velocity = CGVector(dx: CGFloat((index * 17) % 7 - 3) * 0.34,
                        dy: CGFloat((index * 23) % 9 - 4) * 0.26)
    let path = CGMutablePath()
    let points = 8
    for point in 0..<points {
      let angle = CGFloat(point) / CGFloat(points) * .pi * 2
      let wobble = 0.76 + CGFloat((index * 19 + point * 31) % 25) / 100
      let p = CGPoint(x: cos(angle) * radius * wobble, y: sin(angle) * radius * wobble)
      point == 0 ? path.move(to: p) : path.addLine(to: p)
    }
    path.closeSubpath()
    node.path = path; node.fillColor = .clear
    node.strokeColor = contrastDark; node.lineWidth = lineDark
    let lightTrace = SKShapeNode(path: path)
    lightTrace.fillColor = .clear; lightTrace.strokeColor = contrastLight; lightTrace.lineWidth = lineLight
    let accentTrace = SKShapeNode(path: path)
    accentTrace.fillColor = .clear; accentTrace.strokeColor = accent; accentTrace.lineWidth = lineSpot
    node.addChild(lightTrace); node.addChild(accentTrace)
    node.position = position; node.zRotation = CGFloat(index) * 0.37
  }

  var hitbox: CGRect { node.frame.insetBy(dx: 3, dy: 3) }
}

@MainActor private final class ThrustParticle {
  let node = SKNode()

  init(position: CGPoint, velocity: CGVector) {
    let path = CGMutablePath()
    path.move(to: CGPoint(x: -2.5, y: 0)); path.addLine(to: CGPoint(x: 2.5, y: 0))
    let dark = SKShapeNode(path: path); dark.strokeColor = contrastDark; dark.lineWidth = lineDark
    let light = SKShapeNode(path: path); light.strokeColor = contrastLight; light.lineWidth = lineLight
    let spot = SKShapeNode(path: path); spot.strokeColor = accent; spot.lineWidth = lineSpot
    node.addChild(dark); node.addChild(light); node.addChild(spot)
    node.position = position; node.zRotation = atan2(velocity.dy, velocity.dx); node.zPosition = -1
    let body = SKPhysicsBody(circleOfRadius: 2.5)
    body.velocity = velocity; body.linearDamping = 0.16; body.restitution = 0.35; body.friction = 0
    body.categoryBitMask = 0; body.collisionBitMask = 0; body.contactTestBitMask = 0
    body.affectedByGravity = true; node.physicsBody = body
    node.run(.sequence([.wait(forDuration: 0.72), .fadeOut(withDuration: 0.38), .removeFromParent()]))
  }
}
...
@MainActor private final class Fighter {
  ...
  private let hullDark = SKShapeNode()
  private let hullLight = SKShapeNode()
  ...
  init(number: Int, inverted _: Bool, facing: CGFloat) {
    ...
    self.heading = facing > 0 ? 0 : .pi
    let outline = accent
    let hullPath = CGMutablePath()
    hullPath.move(to: CGPoint(x: 36, y: 0))
    hullPath.addLine(to: CGPoint(x: -25, y: 20))
    hullPath.addLine(to: CGPoint(x: -25, y: -20))
    hullPath.closeSubpath()
    hullDark.path = hullPath; hullDark.fillColor = .clear; hullDark.strokeColor = contrastDark; hullDark.lineWidth = lineDark
    hullLight.path = hullPath; hullLight.fillColor = .clear; hullLight.strokeColor = contrastLight; hullLight.lineWidth = lineLight
    hull.path = hullPath; hull.fillColor = outline; hull.strokeColor = outline; hull.lineWidth = lineSpot
    for node in [hullDark, hullLight, hull] {
      node.lineJoin = .round
      node.lineCap = .butt
      node.miterLimit = 1
    }
    ship.addChild(hullDark); ship.addChild(hullLight); ship.addChild(hull)
    root.addChild(ship)
    updatePose()
  }

  var hitbox: CGRect {
    let width = 82 * visualScale, height = 58 * visualScale
    return CGRect(x: root.position.x - width / 2, y: root.position.y - height / 2,
                  width: width, height: height)
  }
  var attackbox: CGRect {
    let reach: CGFloat = attackFrames > 0 ? (attackFrames > 7 ? 82 : 60) : 0
    return CGRect(x: facing > 0 ? root.position.x + 22 : root.position.x - 22 - reach,
                  y: root.position.y - 24, width: reach, height: 48)
  }
  ...
  func tick(buttons: Buttons, opponentX: CGFloat) -> Int? {
    ...
    updatePose()
    if gestureBoostFrames > 0 { gestureBoostFrames -= 1 }
    let rootScale = visualScale * (flashFrames % 2 == 1 ? 1.025 : 1)
    if rootScale != lastRootScale { root.setScale(rootScale); lastRootScale = rootScale }
    let rootAlpha: CGFloat = flashFrames % 2 == 1 ? 0.62 : 1
    if rootAlpha != lastRootAlpha { root.alpha = rootAlpha; lastRootAlpha = rootAlpha }
    return fired
  }
...
private final class FightScene: SKScene {
  ...
  private var controllerRefreshFrames = 0
  private var particleFrame = 0
  private var particleSequence = 0
  private let desktopFloor: CGFloat = arenaHeight / 2 - 10
  private var asteroids: [Asteroid] = []
  private var asteroidImpactCooldown = [1: 0, 2: 0]
  private var introProgress: CGFloat = 1
  private var introLastTime: TimeInterval = 0
  private var introFinished = true
  private var introCancelling = false
  private var introNextBeep = 0
  private let introFlash = SKShapeNode(rectOf: CGSize(width: arenaWidth, height: arenaHeight))
  private var introWarpStars: [(node: SKShapeNode, vector: CGVector, phase: CGFloat)] = []
  ...
  override func didMove(to view: SKView) {
    ...
    if desktopMode {
      ...
      for fighter in [p1, p2] {
        fighter.floorY = desktopFloor; fighter.flightMode = true
        fighter.setVisualScale(0.24)
      }
      addCornerVignette()
      addAsteroids()
    }
  ...
  private func addAsteroids() {
    let positions = [CGPoint(x: 160, y: 160), CGPoint(x: 310, y: 570), CGPoint(x: 610, y: 125),
                     CGPoint(x: 690, y: 610), CGPoint(x: 980, y: 520), CGPoint(x: 1135, y: 190)]
    for (index, position) in positions.enumerated() {
      let asteroid = Asteroid(index: index, position: position)
      asteroid.node.zPosition = -2; asteroids.append(asteroid); addChild(asteroid.node)
    }
  }

  private func addCornerVignette() {
    ...
    guard let context = CGContext(data: nil, width: width, height: height, bitsPerComponent: 8,
          ...
          let gradient = CGGradient(colorsSpace: colorSpace,
                                    colors: [contrastDark.withAlphaComponent(0.42).cgColor,
                                             contrastDark.withAlphaComponent(0).cgColor] as CFArray,
                                    locations: [0, 1]) else { return }
    for corner in [CGPoint(x: 0, y: 0), CGPoint(x: width, y: 0),
                   CGPoint(x: 0, y: height), CGPoint(x: width, y: height)] {
      context.drawRadialGradient(gradient, startCenter: corner, startRadius: 0,
                                 endCenter: corner, endRadius: 280, options: [])
    }
    guard let image = context.makeImage() else { return }
    let texture = SKTexture(cgImage: image); texture.filteringMode = .linear
    let shade = SKSpriteNode(texture: texture, size: CGSize(width: arenaWidth, height: arenaHeight))
    shade.position = CGPoint(x: arenaWidth / 2, y: arenaHeight / 2); shade.zPosition = -7
    addChild(shade)
  }

  private func showMenu(_ show: Bool) {
    menuOpen = show; menuCard.isHidden = !show
    status.text = show && !desktopMode ? "PAUSED" : ""
    lifetime.event("menu_changed", roundID: roundID, fields: ["sceneId": sceneID, "open": show])
  }
  ...
  private func resetRound(trigger: String, beginNewLifetime: Bool) {
    ...
    p1.root.position = CGPoint(x: desktopMode ? arenaWidth / 2 : 390, y: y)
    p2.root.position = CGPoint(x: desktopMode ? 1040 : 890, y: y)
    p1.velocity = .zero; p2.velocity = .zero; roundOver = false; updateBars()
    for shot in shots { shot.node.removeFromParent() }; shots.removeAll()
    lifetime.event("round_reset", roundID: roundID, fields: [
      "sceneId": sceneID, "trigger": trigger, "removedShots": removedShots,
      "sceneChildren": children.count,
    ])
  }

  private func startRematch(trigger: String) {
    resetRound(trigger: trigger, beginNewLifetime: true)
    showMenu(false)
  ...
  private func tick() {
    ...
    if desktopMode {
      if previewSling {
        p1.velocity = .zero
        p1.applyGesture(direction: -0.32, boost: 0.08, grabbed: true)
      } else {
        let trackpad = TrackpadFlightInput.shared.consume()
        p1.applyGesture(direction: trackpad.direction, boost: trackpad.boost,
                        grabbed: trackpad.grabbed)
        now[0].light = now[0].light || trackpad.fire
      }
    }
    if let damage = p1.tick(buttons: edges(now[0], previous[0]), opponentX: p2.root.position.x) { fire(from: p1, damage: damage) }
    let second = now[1]
    if let damage = p2.tick(buttons: edges(second, previous[1]), opponentX: p1.root.position.x) { fire(from: p2, damage: damage) }
  ...
  private func fire(from fighter: Fighter, damage: Int) {
    let heavy = damage > 8
    let direction = desktopMode
      ? CGVector(dx: cos(fighter.heading), dy: sin(fighter.heading))
      : CGVector(dx: fighter.facing, dy: 0)
    let shot = Shot(owner: fighter.number, heavy: heavy, direction: direction,
                    position: CGPoint(x: fighter.root.position.x + direction.dx * 58,
                                      y: fighter.root.position.y + direction.dy * 58))
    if desktopMode { shot.node.setScale(fighter.visualScale) }
    shots.append(shot); addChild(shot.node)
    lifetime.event("attack_fired", roundID: roundID, fields: [
      "sceneId": sceneID, "owner": fighter.number, "damage": damage,
      "shotCount": shots.count, "sceneChildren": children.count,
    ])
    tone.play(frequency: heavy ? 330 : 620, endFrequency: heavy ? 420 : 780,
              duration: heavy ? 0.11 : 0.055, volume: heavy ? 0.16 : 0.11)
  }

  private func emitThrust(from fighter: Fighter) {
    guard desktopMode, fighter.emitsThrust else { return }
    particleSequence += 1
    let dx = cos(fighter.heading), dy = sin(fighter.heading)
    let jitter = CGFloat((particleSequence * 37) % 17 - 8) * 1.7
    let speed = CGFloat(88 + (particleSequence * 19) % 25)
    let velocity = CGVector(dx: fighter.velocity.dx * 28 - dx * speed - dy * jitter,
                            dy: fighter.velocity.dy * 28 - dy * speed + dx * jitter)
    let particle = ThrustParticle(position: fighter.exhaustPosition, velocity: velocity)
    addChild(particle.node)
  }

  private func updateShots() {
    for shot in shots {
      let speed: CGFloat = shot.damage > 8 ? 13 : 18
      shot.node.position.x += shot.direction.dx * speed
      shot.node.position.y += shot.direction.dy * speed
      shot.node.zRotation += 0.08
      if hitAsteroid(with: shot) { continue }
      let defender = shot.owner == 1 ? p2 : p1
      guard shot.hitbox.intersects(defender.hitbox) else { continue }
      let healthBefore = defender.health
      defender.health = max(0, defender.health - shot.damage)
      defender.velocity.dx += shot.direction.dx * (shot.damage > 8 ? 14 : 8)
      defender.velocity.dy += shot.direction.dy * (shot.damage > 8 ? 14 : 8)
      defender.grounded = false; defender.flashFrames = 8
      shot.node.removeFromParent()
      lifetime.event("attack_hit", roundID: roundID, fields: [
        "sceneId": sceneID, "attacker": shot.owner, "defender": defender.number,
        "damage": Int(shot.damage), "healthBefore": Int(healthBefore),
        "healthAfter": Int(defender.health),
      ])
      tone.play(frequency: shot.damage > 8 ? 246 : 392, endFrequency: shot.damage > 8 ? 196 : 330,
                duration: 0.085, volume: 0.15)
      if defender.health == 0 {
        roundOver = true; status.text = "PLAYER \(shot.owner) WINS  •  RETURN TO REMATCH"
        lifetime.finishRound("knockout", roundID: roundID, fields: [
          "sceneId": sceneID, "winner": shot.owner, "loser": defender.number,
          "sceneChildren": children.count, "remainingShots": shots.count,
        ])
        tone.play(frequency: 659, endFrequency: 988, duration: 0.30, volume: 0.16)
        if desktopMode { DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) { NSApp.terminate(nil) } }
        break
      }
    }
    shots.removeAll {
      $0.node.parent == nil || $0.node.position.x < -40 || $0.node.position.x > arenaWidth + 40 ||
        $0.node.position.y < -40 || $0.node.position.y > arenaHeight + 40
    }
  }
