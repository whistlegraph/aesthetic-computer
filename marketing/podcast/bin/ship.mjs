...
const s3cp = (src, name, type) => run("aws", [
  "s3", "cp", src, `${BUCKET}/${name}`,
  "--endpoint-url", ENDPOINT, "--acl", "public-read", "--content-type", type,
]);
s3cp(mp3, `${siteName}.mp3`, "audio/mpeg");
if (existsSync(cover)) s3cp(cover, `${siteName}-cover.png`, "image/png");
...
// ── finish ───────────────────────────────────────────────────────────────
console.log(`\n\x1b[32m✓ ${slug} shipped\x1b[0m — episode is live on Buzzsprout + CDN.`);
