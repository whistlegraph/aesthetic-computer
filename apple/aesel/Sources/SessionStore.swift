...
final class SessionStore {
    ...
    private static func sanitized(_ input: [String: String]) throws -> [String: String] {
        var result = input
        if let text = result["session"], let data = text.data(using: .utf8) {
            ...
            result["session"] = String(data: try JSONSerialization.data(withJSONObject: record), encoding: .utf8)
        }
        return result
    }
    ...
    func write(key: String, value: String) throws {
        ...
        try JSONEncoder().encode(values).write(to: backup, options: .atomic)
